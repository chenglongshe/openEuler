...
| **前置条件** | 宿主机运行 VM FusionOS-23T10（19 vCPU），vcpu0-15 绑定 `5-47,53-63`，vcpu16-18 绑定 `0-63`，Host 侧 vaffinity 服务已启动 |

**步骤**：
1. 在 Host 侧执行 `vaffinity discover --vm FusionOS-23T10`
...
| **前置条件** | 宿主机运行 vm1，Guest 内已启动 `vaffinity-guest`，vCPU 已被自动发现 |

**步骤**：
1. 在 Host 侧执行 `vaffinity status --vm vm1`，确认所有 vCPU 均为 `range` 模式
2. 在 vm1 Guest 内执行 `taskset -c 2 stress --cpu 1 --timeout 60s`（将压测进程绑定到 vCPU 2）
3. 等待 2 秒后，在 Host 侧执行 `vaffinity status --vm vm1`
...
**步骤**：
1. 在 Host 侧执行 `vaffinity status --vm vm1`，确认 vcpu2 为 `exclusive` 模式
2. 在 Host 侧执行 `vaffinity map`，记录 vcpu2 独占的 pCPU 编号（如 pCPU 8）
3. 在 vm1 Guest 内终止 stress 进程（`kill` 或等待超时结束）
4. 等待 2 秒后，在 Host 侧再次执行 `vaffinity status --vm vm1`
5. 在 Host 侧执行 `vaffinity map`
...
| **前置条件** | 宿主机运行 vm1 和 vm2，两者 cpuset 有重叠，Guest 内均已启动 `vaffinity-guest` |

**步骤**：
1. 在 vm1 Guest 内执行 `taskset -c 0 stress --cpu 1 --timeout 120s`
2. 等待 2 秒，在 Host 侧执行 `vaffinity map`，记录 vm1:vcpu0 绑定的 pCPU（如 pCPU 5）
3. 在 vm2 Guest 内执行 `taskset -c 0 stress --cpu 1 --timeout 120s`
4. 等待 2 秒，在 Host 侧执行 `vaffinity map`
...
**步骤**：
1. 在 Host 侧执行 `vaffinity status`（查看所有 VM）
2. 在 Host 侧执行 `vaffinity map`（查看全局 pCPU 映射）
3. 在 Host 侧执行 `vaffinity status --vm vm1 --format json`
...
**步骤**：
1. 在 vm-a Guest 内执行 `vaffinity-guest start`，确认 `vaffinity-guest status` 显示 `running`
2. 在 vm-b Guest 内执行 `vaffinity-guest start`，确认 `vaffinity-guest status` 显示 `running`
3. 分别在 vm-a 和 vm-b 内执行 `taskset -c 1 stress --cpu 1 --timeout 60s`
4. 在 Host 侧执行 `vaffinity status`
...
| **前置条件** | Host 侧 vaffinity 服务已启动且日志正常输出 |

**步骤**：
1. 在 vm1 Guest 内执行 `taskset -c 2 stress --cpu 1 --timeout 30s`（触发绑核）
2. 等待 2 秒，在 Host 侧执行 `vaffinity log --vm vm1`
3. 等待 30 秒（stress 超时结束），在 Host 侧再次执行 `vaffinity log --vm vm1`
