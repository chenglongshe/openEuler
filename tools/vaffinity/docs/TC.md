...
| **前置条件** | 宿主机运行 vm1（4 vCPU，cpuset 0-15），Guest 内已加载截获模块（kprobe 或 eBPF），Host 侧 vaffinity 服务已启动 |
...
2. 等待 2 秒，在 Host 侧执行 `vaffinity map`，记录 vm1:vcpu0 绑定的 pCPU（如 pCPU 0）
...
|------|------|
| **关联** | US-08 |
| **前置条件** | Host 侧 vaffinity 服务已启动且日志正常输出到 journald |

**步骤**：
1. 在 Host 侧执行 `journalctl -u vaffinity --since "now"` 清空日志视图
2. 在 vm1 Guest 内执行 `taskset -c 2 stress --cpu 1 --timeout 30s`（触发绑核）
3. 等待 2 秒，在 Host 侧执行 `journalctl -u vaffinity --since "1 min ago"`
4. 等待 30 秒（stress 超时结束），在 Host 侧再次执行 `journalctl -u vaffinity --since "1 min ago"`
