...
1. **观测可视化域**：提供按 VM/vCPU/线程归因的实时绑核观测能力，支撑现场快速锁定异常 VM 与线程。运维人员可通过 `vaffinity status --vm <domain>` 直接查看每个 vCPU 的绑核模式、绑定的 pCPU、操作来源等状态，通过 `vaffinity map` 查看全局 pCPU 独占/共享分布。

2. **透明绑核透传域**：实现 Guest 侧绑核感知 → Host 侧自动执行的端到端透传。当虚拟机内业务应用绑核时，系统自动将对应 vCPU 从范围绑核切换为 1:1 独占绑核；业务解绑后自动恢复。**无需手动注册 VM**——vCPU 绑核范围在虚拟机启动时已由 libvirt 初始化，`vaffinity` 通过 `virsh vcpupin <domain>` 自动发现每个 vCPU 的 CPU 亲和性范围。

3. **统一管理编排域**：提供覆盖 vCPU 绑核的一体化管理入口（`vaffinity` 命令行），支撑主机 CPU 资源的统一查看、变更与协同操作，包括自动发现（`discover`）、状态查询（`status`）、全局映射（`map`）、批量解绑（`unpin-all`）、事件日志（`log`）等。
...
4. **部署约束**：Guest 侧以 eBPF CO-RE 程序 + 用户态通知代理形式部署；VMM 侧通过 `vaffinity` 命令行工具与 systemd 服务管理；支持一次编译、多版本 Guest 内核适配。
...
- GIVEN 宿主机上运行虚拟机 FusionOS-23T10（19 vCPU，vcpu0-15 cpuset `5-47,53-63`，vcpu16-18 cpuset `0-63`），管理员在 Host 侧启动 `vaffinity-listener` 服务
...
- GIVEN 宿主机上运行多台 VM，管理员在 Host 侧执行 `vaffinity status` 确认所有 VM 均为 `range` 模式
- WHEN vm1 Guest 内某业务程序调用 `taskset -c 2 <workload>` 绑核到 vCPU 2
- THEN 管理员执行 `vaffinity status --vm vm1`，**可观察到** vcpu2 已变为 `exclusive` 模式并绑定到具体 pCPU，`source` 字段标记为 `guest-pin`，可直接定位到触发绑核的 VM 与 vCPU

#### 一致性

**场景 3：不同主机上通过同一管理入口操作呈现一致体验**

- GIVEN 在不同宿主机上均部署了 `vaffinity` 工具与 `vaffinity-listener` 服务
- WHEN 管理员在各主机上分别执行 `vaffinity discover`、`vaffinity status`、`vaffinity map` 等命令
...
- WHEN 管理员在 Guest 内触发绑核（`taskset -c 2`），随后在 Host 侧执行 `vaffinity status --vm vm1` 查看状态，再在 Guest 内终止业务进程，最后在 Host 侧执行 `vaffinity status --vm vm1` 和 `vaffinity log --vm vm1`
...
- GIVEN 运维人员首次接触 `vaffinity` 工具
- WHEN 运维人员参照 README.md 和 `man vaffinity` 文档，按步骤完成宿主机侧安装、启动监听服务、执行自动发现、查看 VM 状态
...
- Guest 侧用户态通知代理（`vaffinity-guest`，Python + VSOCK/virtio-serial 传输）
- Guest 侧 systemd 服务（`vaffinity-guest.service`）
- VMM 侧管理工具（`vaffinity`，含自动发现、状态查询、全局映射、绑核执行、日志审计）
- VMM 侧 systemd 服务（`vaffinity-listener.service`、`vaffinity-restore.service`）
