...
static uint32_t tracker_buffer_size = 8388608;  /* 8MB for default */
...
static void _dump_trace(void)
{
	...
	pos = 0;
	ret = kernel_write(filp, rtracker.data, rtracker.pos, &pos);
	if (ret < 0)
...
__bpf_kfunc void bpf_get_dpath_mark(unsigned long addr, unsigned long off,
					  unsigned long len)
{
	const struct file *file = (const struct file *)addr;
...
BTF_ID_FLAGS(func, bpf_get_dpath_mark)
BTF_SET8_END(bpf_rio_tracker_ids)

static const struct btf_kfunc_id_set kfuncs = {
	.owner = THIS_MODULE,
...
static __init int rio_tracker_init(void)
{
	...
	mutex_init(&rtracker.lock);
	rtracker.pos = 0;

	/* register self-defined bpf helper */
	ret = register_btf_kfunc_id_set(BPF_PROG_TYPE_UNSPEC, &kfuncs);
	if (ret) {
