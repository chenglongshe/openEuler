...
#
# Test suite for vmca (Host-side handler + Guest-side interceptor)
# Based on patent: 一种优化虚拟机内业务绑核性能的方法 (Inventor: 张海亮)

"""
Unit and integration tests for vmca.
...
Run with: python3 -m pytest tests/test_vmca.py -v
"""
...
from vmca_guest import (  # noqa: E402
    AffinityEvent,
    EbpfInterceptor,
    NotificationAgent,
    SimulatedTransport,
...
class TestEventLog(unittest.TestCase):
    ...
    def setUp(self):
        ...
        import vmca
        self._orig_log_file = vmca.LOG_FILE
        vmca.LOG_FILE = self.log_file

    def tearDown(self):
        import shutil
        import vmca
        vmca.LOG_FILE = self._orig_log_file
        shutil.rmtree(self.tmpdir, ignore_errors=True)
