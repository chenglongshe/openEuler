...
| 位置 | 接口 | 说明 |
|------|------|------|
| Host 侧 | `vmca discover [--vm <domain>]` | 自动发现 VM 的 vCPU 绑核信息（从 virsh vcpupin 获取） |
| Host 侧 | `vmca status [--vm <domain>] [--format json\|table]` | 查看 vCPU 绑核状态（模式、pCPU、来源） |
| Host 侧 | `vmca map [--format json\|table]` | 查看全局 CPU 映射表 |
...
- GIVEN 宿主机上运行多台 VM
- WHEN 管理员启动 `vmca-listener` 服务
- THEN **可观察到**服务启动日志中包含各 VM 的自动发现记录，`vmca status` 可查看所有已发现 VM 的状态

**AC-01-03：状态查询支持 VM 粒度定位**

- GIVEN 宿主机运行 vm1 和 vm2，vm1 的 vcpu2 为 `exclusive` 模式
- WHEN 管理员执行 `vmca status --vm vm1`
...
- Guest 内安装 `vmca-guest` 包
- Host 侧 `vmca-listener` 服务运行中
...
| 位置 | 接口 | 说明 |
|------|------|------|
| Guest 侧 | `vmca-guest start [--mode vsock\|virtio-serial]` | 启动 eBPF 绑核感知模块 |
| Guest 侧 | `vmca-guest stop` | 停止感知模块 |
| Guest 侧 | `vmca-guest status` | 查看感知模块运行状态 |

### 【验收准则】

**AC-02-01：Guest 内业务绑核 → Host 侧 vCPU 自动切换为 1:1 独占绑核**

- GIVEN 宿主机运行 vm1，Guest 内启动 `vmca-guest start`，Host 侧 `vmca status --vm vm1` 确认所有 vCPU 为 `range` 模式
- WHEN 管理员在 vm1 Guest 内运行 `taskset -c 2 <workload>`
- THEN 管理员在 Host 侧执行 `vmca status --vm vm1`，**可观察到** vcpu2 变为 `exclusive` 模式并绑定到具体 pCPU

**AC-02-02：Guest 内业务解除绑核 → vCPU 自动恢复为范围绑核**

- GIVEN 承接 AC-02-01，vm1 的 vcpu2 为 `exclusive` 模式
- WHEN 管理员在 Guest 内终止该业务进程
- THEN Host 侧执行 `vmca status --vm vm1`，**可观察到** vcpu2 恢复为 `range` 模式；`vmca map` 显示原独占 pCPU 恢复为 `shared`

**AC-02-03：eBPF CO-RE 跨 Guest 内核版本兼容**

- GIVEN 同一编译产物的 eBPF 截获程序分别部署到 openEuler 22.03（5.10）和 24.03（6.6）的 Guest
- WHEN 在各 Guest 内启动 `vmca-guest start` 并运行绑核业务
...
| 位置 | 接口 | 说明 |
|------|------|------|
| Host 侧 | `vmca map [--format json\|table] [--limit N]` | 查看全局 CPU 映射表（每个 pCPU 的状态与归属） |
| Host 侧 | `vmca unpin-all --vm <domain>` | 批量恢复指定 VM 所有 vCPU 为范围绑核 |
...
- THEN `vmca map` 显示 vm2 的 vCPU 绑定到另一个不冲突的 pCPU，**不与** vm1 的 pCPU N 冲突
...
- WHEN 管理员执行 `vmca unpin-all --vm vm1`
...
| 位置 | 接口 | 说明 |
|------|------|------|
| Host 侧 | `vmca log [--vm <domain>] [--tail N]` | 查看绑核事件日志 |
...
┌─────────────────── Guest 侧 ───────────────────┐     ┌──────────── Host / VMM 侧 ────────────┐
│                                                  │     │                                        │
│  业务应用绑核 → eBPF 截获 → ring buffer          │     │  vmca-listener 启动时               │
...
**关键设计决策：** 无需手动注册 VM。vCPU 的绑核范围在虚拟机启动时已由 libvirt 初始化，`vmca` 通过 `virsh vcpupin <domain>` 自动获取每个 vCPU 的 CPU 亲和性范围（每个 vCPU 可以有不同的 cpuset），后续只需基于此进行动态修改。
