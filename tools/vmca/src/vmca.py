...
#
# vmca: VMM-side handler for transparent in-VM pinning passthrough
# Based on patent: 一种优化虚拟机内业务绑核性能的方法 (Inventor: 张海亮)
#
# Copyright (c) 2026 openEuler Contributors

"""
vmca — VMM-side transparent vCPU pinning optimization tool.
...
============
  ...
  Guest VM  ──VSOCK/virtio-serial──▶  vmca listener daemon (host)
                                           │
                                     ┌─────▼──────────┐
                                     │ Notification    │
                                     │ receiver        │
...
PID_FILE = "/run/vmca-listener.pid"
VSOCK_PORT = 11200                  # Same as guest side
LOG_FORMAT = "[%(levelname)s] %(message)s"

logger = logging.getLogger("vmca")
...
class GuestPinNotification:
    ...
    vmca-guest agent running inside the VM.
    """
    ACTION_PIN = "pin"
    ACTION_UNPIN = "unpin"
...
def main():
    ...
    logging.getLogger("vmca").setLevel(level)
