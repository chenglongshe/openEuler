...
#
# vmca-guest: Guest-side sched_setaffinity interceptor (eBPF + async agent)
# Based on patent: 一种优化虚拟机内业务绑核性能的方法 (Inventor: 张海亮)
#
# Copyright (c) 2026 openEuler Contributors

"""
vmca-guest — Guest-side eBPF interceptor & notification agent.
...
the host-side vmca listener daemon through a notification channel
...
logger = logging.getLogger("vmca-guest")

# ---------------------------------------------------------------------------
# Configuration
# ---------------------------------------------------------------------------

PID_FILE = "/run/vmca-guest.pid"
VSOCK_HOST_CID = 2                  # CID 2 = host in VSOCK
VSOCK_PORT = 11200                  # Port the host listener binds to
VIRTIO_SERIAL_PATH = "/dev/virtio-ports/vmca"
POLL_INTERVAL_SEC = 0.1             # Ring buffer polling interval (100 ms)
BPF_OBJ_SEARCH_PATHS = [
    "/usr/lib64/vmca/vmca_intercept.bpf.o",
    "/usr/lib/vmca/vmca_intercept.bpf.o",
    os.path.join(os.path.dirname(__file__), "bpf", "vmca_intercept.bpf.o"),
]
...
class EbpfInterceptor:
    ...
    In production this loads the CO-RE BPF object (vmca_intercept.bpf.o)
    ...
    def attach(self):
        ...
        In production:
            bpf_obj = bpf_object__open_file("vmca_intercept.bpf.o")
...
class VirtioSerialTransport(NotificationTransport):
    ...
    QEMU exposes a virtio-serial port (e.g. /dev/virtio-ports/vmca).
...
class NotificationAgent:
    ...
    vmca listener through the configured transport.
    """
...
def main():
    ...
    if args.command == "start":
        ...
        if getattr(args, "foreground", False):
            ...
            print("[INFO] Starting vmca-guest agent …")
