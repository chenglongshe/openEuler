...
struct xsched_entity_cfs {
	...
	/* Clean execution time of scheduling entity */
	u64 exec_start;
...
struct xsched_entity {
	...
	/* Entity execution statistics */
	ktime_t last_process_time;
			       ...
			       struct xsched_cu *xcu)
{
	...
	if (!xcu) {
		XSCHED_ERR("Tried to get pending kicks with xcu=NULL.\n");
		return NULL;
	}

	if (!class) {
		XSCHED_ERR("Tried to get pending kicks with class=NULL.\n");
		return NULL;
	}
...
{
	if (!xse) {
		XSCHED_ERR("Tried to get pending kicks with xse=NULL\n");
		return NULL;
	}

	if (!xse->xcu) {
		XSCHED_ERR("Tried to get pending kicks with xse->xcu=NULL\n");
		return NULL;
	}
...
static inline int xsched_dec_pending_kicks_xse(struct xsched_entity *xse)
{
	...
	if (!kicks_class) {
		XSCHED_ERR(
			"Could find atomic counter for class based pending kicks.\n");
		return -EINVAL;
	}

	if (!atomic_read(kicks_class)) {
		XSCHED_ERR("Tried to decrement pending kicks beyond 0!\n");
		return -EINVAL;
	}
