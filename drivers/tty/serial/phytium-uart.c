...
#define REG_IMSC_OEIM		(1 << 10)
#define REG_IMSC_BEIM		(1 << 9)
#define REG_IMSC_PEIM		(1 << 8)
#define REG_IMSC_FEIM		(1 << 7)
#define REG_IMSC_RTIM		(1 << 6)
...
#define REG_ICR_OEIS		(1 << 10)
#define REG_ICR_BEIS		(1 << 9)
#define REG_ICR_PEIS		(1 << 8)
#define REG_ICR_FEIS		(1 << 7)
#define REG_ICR_RTIS		(1 << 6)
...
static void phytium_modem_status(struct phytium_uart_port *pup)
{
	...
	status = phytium_uart_read(pup, REG_FR);
	status &= (REG_FR_DCD|REG_FR_DSR|REG_FR_CTS);
...
static irqreturn_t phytium_uart_interrupt(int irq, void *dev_id)
{
	...
	if (status) {
		do {
			phytium_uart_write(status & ~(REG_ICR_TXIS|
			REG_ICR_RTIS|REG_ICR_RXIS), pup, REG_ICR);
...
static int phytium_startup(struct uart_port *port)
{
	...
	pup->old_status = phytium_uart_read(pup, REG_FR) & (REG_FR_DCD|
		REG_FR_DSR|REG_FR_CTS);
...
static void
phytium_set_termios(struct uart_port *port,
