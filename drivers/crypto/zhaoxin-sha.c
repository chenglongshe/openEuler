 ...
 * multiple-parts hash supported by Zhaoxin Processor.
 */
...
static int padlock_sha256_update_zhaoxin(struct shash_desc *desc, const u8 *data,
				unsigned int len)
{
	struct sha256_state *sctx = shash_desc_ctx(desc);
	unsigned int partial, done;
...
static int padlock_sha_export_zhaoxin(struct shash_desc *desc,
				void *out)
{
...
static int padlock_sha_import_zhaoxin(struct shash_desc *desc,
				const void *in)
{
	int statesize = crypto_shash_statesize(desc->tfm);
	void *sctx = shash_desc_ctx(desc);
