...
static ssize_t store_smart_grid_governor(struct kobject *kobj,
					 ...
					 const char *buf, size_t count)
{
	...
	if (cpufreq_parse_governor(save_string, &new_policy)) {
		ret = -EINVAL;
		goto fail;
	}
	if (new_policy.governor)
	...
	strscpy(sg_zone.governor_name[current_level], save_string,
		CPUFREQ_NAME_LEN);
...
static void smart_grid_work_handler(struct work_struct *work)
{
	...
	for (gov_index = SMART_GRID_ZONE_NR - 1; gov_index >= 0; gov_index--) {
		if (cpufreq_parse_governor(sg_zone.governor_name[gov_index],
					   &new_policy))
			continue;
		...
		for_each_cpu(cpu, sched_grid_zone_cpumask(gov_index)) {
			...
			if (policy->governor == new_policy.governor) {
				cpufreq_cpu_put(policy);
				continue;
			}
...
static void sg_zone_set_enable(void)
{
	...
	for (gov_index = 0; gov_index < SMART_GRID_ZONE_NR; gov_index++) {
		if (!gov_index)
			strscpy(sg_zone.governor_name[gov_index],
...
static ssize_t store_smart_grid_governor_enable(struct kobject *kobj,
						struct kobj_attribute *attr,
						const char *buf, size_t count)
{
	unsigned int enable;
