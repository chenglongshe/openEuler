...
#define MODULE_NAME "XSched"
#define XSCHED_LOG(level, tag, fmt, ...) \
	pr_##level(pr_fmt(MODULE_NAME tag fmt), ##__VA_ARGS__)

#define XSCHED_ERR(fmt, ...)   XSCHED_LOG(err,   " [ERROR]: ",   fmt, ##__VA_ARGS__)
#define XSCHED_WARN(fmt, ...)  XSCHED_LOG(warn,  " [WARNING]: ", fmt, ##__VA_ARGS__)
#define XSCHED_INFO(fmt, ...)  XSCHED_LOG(info,  " [INFO]: ",    fmt, ##__VA_ARGS__)
#define XSCHED_DEBUG(fmt, ...) XSCHED_LOG(debug, " [DEBUG]: ",   fmt, ##__VA_ARGS__)
...
#define XSCHED_WAIT_TIMEOUT 500

/*
 * A default kick slice for RT class XSEs.
 */
#define XSCHED_RT_KICK_SLICE 10

extern atomic_t pending_task_count;
...
enum xcu_sched_type {
	XSCHED_TYPE_RT,
	XSCHED_TYPE_NUM,
	XSCHED_DEFAULT_TYPE = XSCHED_TYPE_RT
};
...
/* Xsched class. */
struct xsched_class {
	const enum xcu_sched_type class_id;
	size_t kick_slice;
