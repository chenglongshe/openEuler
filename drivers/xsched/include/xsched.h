...
#define XSCHED_ERR(fmt, ...)                                                   \
	pr_err(pr_fmt(MODULE_NAME " [ERROR]: " fmt), ##__VA_ARGS__)

#define XSCHED_WARN(fmt, ...)                                                  \
	pr_warn(pr_fmt(MODULE_NAME " [WARNING]: " fmt), ##__VA_ARGS__)

#define XSCHED_INFO(fmt, ...)                                                  \
	pr_info(pr_fmt(MODULE_NAME " [INFO]: " fmt), ##__VA_ARGS__)

/*
 * Debug specific prints for XSched
 */
#define XSCHED_DEBUG(fmt, ...)                                                 \
	pr_debug(pr_fmt(MODULE_NAME " [DEBUG]: " fmt), ##__VA_ARGS__)
...
#define XSCHED_KICK_SLICE 2

/*
 * A default kick slice for RT class XSEs.
 */
#define XSCHED_RT_KICK_SLICE XSCHED_KICK_SLICE
...
int xsched_xcu_unregister(struct xcu_group *group, int phys_id);
