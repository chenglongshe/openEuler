...
static int virtcca_get_all_cc_dev_info(struct device *dev, struct tmi_dev_delegate_params *params)
{
	...
	if (params->num_dev >= MAX_DEV_PER_PORT) {
		ret = -EINVAL;
		pr_err("virtcca_get_all_cc_dev_info nums overflow\n");
		return ret;
	}
	return ret;
}
...
static int virtcca_attach_each_dev_to_nvm(struct device *dev, void *domain)
{
	...
	if (is_cc_root_bd(root_bd)) {
		dev_err(smmu->dev,
		"CoDA: the security device under the root port 0x%x is not allowed to assign to the normal VM\n", root_bd);
		spin_unlock(&pcipc_enable_lock);
		return -EINVAL;
	}
	spin_unlock(&pcipc_enable_lock);
	...
	for (i = 0; i < master->num_streams; i++) {
		...
		if (ret)
			pr_err("CoDA: attach device to nvm, add device 0x%x to CoDA linked list failed\n", sid);
	}
 ...
 * 1NVM scenarioDelete the device information corresponding to the NVM from the CoDA management linked list
 * 2CVM scenarioSet the device that has already been assigned to the CVM in the CoDA management linked list
	...
	for (i = 0; i < master->num_streams; i++) {
		...
		if (is_cc_dev(sid)) {
			if (get_g_coda_dev_vm_type(sid) == CC_DEV_CVM_TYPE) {
				ret = add_cc_dev_obj(sid, 0, get_root_bd(dev), true, CC_DEV_HOST_TYPE);
				if (ret)
					pr_err("CoDA: detach device from vm, add cc device 0x%x failed\n", sid);
			}
		} else {
	...
	bool iommu_secure)
{
	...
	if (iommu_secure)
		...
		ret = iommu_group_for_each_dev(group, (void *)domain, virtcca_attach_each_dev_to_nvm);
