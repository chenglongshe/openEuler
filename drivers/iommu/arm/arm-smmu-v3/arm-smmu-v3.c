...
#ifdef CONFIG_HISI_VIRTCCA_HOST
static inline bool is_s_smmu_msi_index(struct msi_desc *desc)
{
	return desc->msi_index == S_EVTQ_MSI_INDEX ||
			desc->msi_index == S_GERROR_MSI_INDEX;
}

static inline void arm_s_smmu_set_irq(struct arm_smmu_device *smmu)
{
	if (is_s_smmu_enable(smmu)) {
	...
	struct arm_smmu_device *smmu, struct arm_smmu_master *master, struct device *dev)
{
	...
	if (!is_s_smmu_enable(smmu)) {
		dev_err(smmu->dev, "Security SMMU not initialized for the device\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HISI_VIRTCCA_HOST
	arm_s_smmu_init_one_queue(smmu, q, qsz, name);
#endif
...
#ifdef CONFIG_HISI_VIRTCCA_HOST
	arm_smmu_enable_s_irq(smmu, IRQ_CTRL_EVTQ_IRQEN | IRQ_CTRL_GERROR_IRQEN);
#endif
...
#ifdef CONFIG_HISI_VIRTCCA_HOST
	arm_smmu_s_idr1_support_secure(smmu);
#endif
...
static void arm_smmu_device_remove(struct platform_device *pdev)
{
	...
	arm_smmu_id_free(smmu->s_smmu_id);
#endif
}
