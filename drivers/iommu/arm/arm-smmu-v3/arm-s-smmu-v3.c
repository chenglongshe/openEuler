...
void s_queue_write(struct arm_smmu_device *smmu, u64 *src, size_t n_dwords)
{
	...
	if (smmu->virtcca_id != ARM_SMMU_INVALID_ID) {
		if (n_dwords == 2) {
			...
			if (forward) {
				if (tmi_smmu_queue_write(cmd0, cmd1, smmu->virtcca_id))
							  ...
							  size_t qsz, const char *name)
{
	if (smmu->virtcca_id != ARM_SMMU_INVALID_ID) {
		...
		if (!params_ptr) {
			smmu->virtcca_id = ARM_SMMU_INVALID_ID;
			return;
		}
		...
		if (!strcmp(name, "cmdq")) {
			...
			params_ptr->size = qsz;
			params_ptr->smmu_id = smmu->virtcca_id;
...
irqreturn_t arm_smmu_s_evtq_thread(int irq, void *dev)
{
	...
	if (smmu->virtcca_id != ARM_SMMU_INVALID_ID)
		tmi_handle_s_evtq(smmu->virtcca_id);
...
void arm_smmu_disable_s_irq(struct arm_smmu_device *smmu)
{
	int ret;
...
void arm_smmu_enable_s_irq(struct arm_smmu_device *smmu, u32 irqen_flags)
{
	int ret;
...
void arm_s_smmu_setup_unique_irqs(struct arm_smmu_device *smmu)
{
	...
	irq = smmu->s_evtq_irq;
	if (smmu->virtcca_id != ARM_SMMU_INVALID_ID && irq) {
		...
		if (master && master->num_streams >= 0) {
			for (i = 0; i < master->num_streams; ++i) {
				...
				ret = tmi_dev_attach(sid, virtcca_cvm->rd,
					arm_smmu_domain->smmu->virtcca_id);
				if (ret) {
	...
	struct device *dev)
{
	...
	if (ret) {
		pr_info("S_SMMU: failed to Delegate device to secure\n");
		return ret;
	}
...
void arm_smmu_map_init(struct arm_smmu_device *smmu, resource_size_t ioaddr)
{
	...
	if (tmi_smmu_pcie_core_check(ioaddr))
		smmu->virtcca_id = arm_smmu_id_alloc();
	else
		smmu->virtcca_id = ARM_SMMU_INVALID_ID;
...
void arm_s_smmu_device_reset(struct arm_smmu_device *smmu)
{
	int ret;
	u64 rv;
	u32 reg, enables;
...
void arm_smmu_s_idr1_support_secure(struct arm_smmu_device *smmu)
{
	u32 ret;
	u64 rv;
