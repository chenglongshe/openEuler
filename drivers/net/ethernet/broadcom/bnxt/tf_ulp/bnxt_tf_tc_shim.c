...
static void bnxt_set_custom_dflt_rss_indir_tbl(struct bnxt *bp)
{
	...
	for (i = 0; i < max_entries; i++) {
		if (j != max_rings) {
			bp->rss_indir_tbl[i] = j++;
		} else {
			j = 2;
			bp->rss_indir_tbl[i] = j++;
		}
		netdev_dbg(bp->dev, "i:%d bp->rss_indir_tbl[i]:%d\n", i, bp->rss_indir_tbl[i]);
	}
...
int bnxt_set_custom_vnic_rss(struct bnxt *bp)
{
	bnxt_set_custom_dflt_rss_indir_tbl(bp);
	return bnxt_hwrm_vnic_set_rss_p5(bp, 0, true);
}

int bnxt_get_vnic_info_idx(struct bnxt *bp)
{
	int idx;
...
static void bnxt_clear_queue_vnic(struct bnxt *bp, uint16_t vnic_id)
{
	int i, nr_ctxs;
	...
	if (bp->flags & BNXT_FLAG_TPA)
		bnxt_hwrm_vnic_set_tpa(bp, vnic_id, 0);

	bnxt_hwrm_vnic_free_one(bp, vnic_id);

	nr_ctxs = bnxt_get_nr_rss_ctxs(bp, bp->rx_nr_rings);
	for (i = 0; i < nr_ctxs; i++) {
		if (bp->vnic_info[vnic_id].fw_rss_cos_lb_ctx[i] != INVALID_HW_RING_ID) {
			bnxt_hwrm_vnic_ctx_free_one(bp, vnic_id, i);
			bp->rsscos_nr_ctxs--;
		}
	}
}

int32_t bnxt_vnic_queue_action_free(struct bnxt *bp, uint16_t vnic_id)
{
	struct bnxt_vnic_info *vnic_info;
	int32_t rc = -EINVAL;
	uint16_t vnic_idx = vnic_id;
...
static int
bnxt_setup_queue_vnic(struct bnxt *bp, uint16_t vnic_id, uint16_t q_index)
{
	int rc, nr_ctxs, i;

	/* It's a queue action, so only one queue */
	rc = bnxt_hwrm_vnic_alloc(bp, vnic_id, q_index, 1);
	if (rc) {
		rc = -EINVAL;
		goto cleanup;
	}

	rc = bnxt_hwrm_vnic_cfg(bp, vnic_id, q_index);
	if (rc) {
		rc = -EINVAL;
		goto vnic_free;
	}

	rc = bnxt_hwrm_vnic_set_tpa(bp, vnic_id, bp->flags & BNXT_FLAG_TPA);
	if (rc) {
		rc = -EINVAL;
		goto vnic_free;
	}
