...
{
#ifdef CONFIG_BNXT_CUSTOM_FLOWER_OFFLOAD
	return bnxt_pmd_bd_act_set(parms->ulp_ctx->bp, port_id, action);
#else
	netdev_dbg(parms->ulp_ctx->bp->dev,
		   "Warning: Not support bd action\n");
	return -1;
#endif
}
			 ...
			 void *error)
{
	...
	tfp = bnxt_ulp_cntxt_tfp_get(ulp_ctx, ulp_flow_db_shared_session_get(res));
	if (!tfp) {
			  ...
			  void *error)
{
	...
	while (!rc) {
		...
		if (trc) {
			 ...
			 */
			netdev_dbg(ulp_ctx->bp->dev,
				   "Flow[%d][0x%x] Res[%d][%0llu] failed rc=%d.\n",
