...
const char *ulp_mapper_resource_func_names[] = {
	...
	[BNXT_ULP_RESOURCE_FUNC_IDENTIFIER] = "Idenitifer table",
		     ...
		     u8 *ptr, u32 size)
{
	...
	netdev_dbg(ulp_ctx->bp->dev, "%s", lbuffer);
				       ...
				       u32 count_list)
{
	...
	netdev_dbg(params->ulp_ctx->bp->dev, "Default computed fields\n");
	for (idx = 0; idx < count_list; idx++) {
		netdev_dbg(params->ulp_ctx->bp->dev, "\t%s =\n",
			   field_names[idx]);
...
void ulp_parser_hdr_info_dump(struct ulp_tc_parser_params *params)
{
	...
	if (ULP_BITMAP_ISSET(hdr_bitmap->bits, BNXT_ULP_FLOW_DIR_BITMASK_EGR))
		netdev_dbg(ulp_ctx->bp->dev, "It is a Egress Flow - %x\n",
			   params->dir_attr);
	else
		netdev_dbg(ulp_ctx->bp->dev, "It is a Ingress Flow - %x\n",
			   params->dir_attr);
...
void ulp_matcher_act_field_dump(struct bnxt_ulp_context *ulp_ctx, u32 idx,
				u32 jdx, u32 mask_id)
{
	netdev_dbg(ulp_ctx->bp->dev, "Match failed template=%d,field=%s,mask=%s\n",
		   idx,
...
void ulp_mapper_ident_field_dump(struct bnxt_ulp_context *ulp_ctx,
				 ...
				 int id)
{
	netdev_dbg(ulp_ctx->bp->dev, "%-16s alloc %-16s, dir= %s, id = 0x%x\n",
...
{
	netdev_dbg(ulp_ctx->bp->dev, "%-16s [%s][0x%0x],keysz=%-3d resultsz=%-3d\n",
		   name,
...
void ulp_mapper_result_dump(struct bnxt_ulp_context *ulp_ctx, const char *name,
			    ...
			    struct ulp_blob *result)
{
	netdev_dbg(ulp_ctx->bp->dev, "%-16s [%s], bitlen=%-3d\n",
		   name,
		   (tbl->direction == TF_DIR_RX) ? "RX" : "TX",
		   result->write_idx);
...
{
	netdev_dbg(ulp_ctx->bp->dev, "%s ins %s[%s] scope=0x%02x keysz=%d recsz=%d\n",
		   name,
	...
	netdev_dbg(ulp_ctx->bp->dev, "FlowHdl= %llu FlowID= %llu\n",
		   iparms->flow_handle, iparms->flow_id);

	netdev_dbg(ulp_ctx->bp->dev, "Key Size %d, Data Size %d\n",
...
{
	netdev_dbg(ulp_ctx->bp->dev, "%s [%s] keysz=%u recsz=%u\n",
		   name,
		   (iparms->dir == CFA_DIR_RX) ? "RX" : "TX",
		   iparms->key_sz_bits,
		   iparms->lkup_key_sz_words);

	netdev_dbg(ulp_ctx->bp->dev, "FlowHdl=%llu\n", *iparms->flow_handle);
			   ...
			   struct bnxt_ulp_mapper_tbl_info *tbl, u32 idx)
{
	...
	if (tbl->resource_func == BNXT_ULP_RESOURCE_FUNC_INDEX_TABLE)
		...
		sub_type = "N/A";
	netdev_dbg(ulp_ctx->bp->dev, "Processing table %-16s:%-16s: %u\n",
...
{
	netdev_dbg(ulp_ctx->bp->dev, "Generic Tbl[%s][%s] - Dump Key\n",
