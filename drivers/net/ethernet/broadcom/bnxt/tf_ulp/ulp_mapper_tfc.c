			      ...
			      void *error)
{
	...
	ulp_blob_pad_align(&key, align_len_bits);
	...
	align_len_bits = dparms->em_blk_align_bits;
	ulp_blob_pad_align(&data, align_len_bits);
			     ...
			     void *error)
{
	...
	if (rc) {
		netdev_dbg(ulp->bp->dev, "Failed to delete EM entry, res = %llu\n",
			   res->resource_hndl);
	} else {
			       ...
			       void *error)
{
	...
	if (write) {
		...
		netdev_dbg(bp->dev, "CMM table[%d][%s][%llu] write successful\n",
			      ...
			      void *error)
{
	...
	if (rc) {
		netdev_dbg(ulp_ctx->bp->dev, "Failed to delete CMM entry,res = %llu\n",
			   res->resource_hndl);
	} else {
		netdev_dbg(ulp_ctx->bp->dev, "Deleted CMM entry,res = %llu\n", res->resource_hndl);
	}
	return rc;
}
