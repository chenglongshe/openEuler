				   ...
				   struct xdp_buff *xdp)
{
	...
	for (i = 0; i < num_frags ; i++) {
		...
		frag_tx_buf = &txr->tx_buf_ring[RING_TX(prod)];
		...
		txbd->tx_bd_len_flags_type = cpu_to_le32(flags);
		txbd->tx_bd_haddr = page_pool_get_dma_addr(skb_frag_page(frag));
...
void bnxt_tx_int_xdp(struct bnxt *bp, struct bnxt_napi *bnapi)
{
	...
	while (RING_TX(tx_cons) != tx_hw_cons) {
