...
int tfc_act_alloc(struct tfc *tfcp, u8 tsid, struct tfc_cmm_info *cmm_info, u16 num_contig_rec)
{
	...
	if (rc) {
		...
		cmm = kmalloc(qparms.db_size, GFP_KERNEL);
	...
	aparms.num_contig_records = num_contig_rec;
		...
		u16 data_sz_words)
{
	...
	for (i = 0; i < buff_len; i++) {
		if (i % 16 == 0)
			netdev_dbg(bp->dev, "\n%04d: ", i);
		netdev_dbg(bp->dev, "%02x", tx_msg[i]);
	}
	netdev_dbg(bp->dev, "\n");
#endif
	...
	for (i = 0; i < buff_len; i++) {
		if (i % 16 == 0)
			netdev_dbg(bp->dev, "\n%04d: ", i);
		netdev_dbg(bp->dev, "%02x", rx_msg[i]);
	}
	netdev_dbg(bp->dev, "\n");
#endif
		 ...
		 const struct tfc_cmm_info *cmm_info)
{
	...
	fparms.num_contig_records = record_size;
