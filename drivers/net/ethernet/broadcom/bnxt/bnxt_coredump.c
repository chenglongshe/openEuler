...
static void bnxt_fill_empty_seg(struct bnxt *bp, void *buf, u32 len)
{
	struct bnxt_coredump_segment_hdr seg_hdr;

	bnxt_fill_coredump_seg_hdr(bp, &seg_hdr, NULL, len, 0, 0, 0, 0);
...
err:
	...
	if (buf) {
		...
		bnxt_fill_coredump_record(bp, buf + offset,
					  start_time, start_utc, coredump.total_segs + 2, rc);
	}
...
int bnxt_get_coredump(struct bnxt *bp, u16 dump_type, void *buf, u32 *dump_len)
{
	if (dump_type == BNXT_DUMP_DRIVER) {
		bnxt_start_logging_coredump(bp, buf);
		return 0;
	}
...
int bnxt_hwrm_get_dump_len(struct bnxt *bp, u16 dump_type, u32 *dump_len)
{
	...
	if (dump_type == BNXT_DUMP_DRIVER) {
		*dump_len = bnxt_get_loggers_coredump_size(bp);
		return 0;
	}
...
u32 bnxt_get_coredump_length(struct bnxt *bp, u16 dump_type)
{
	...
	if (bnxt_hwrm_get_dump_len(bp, dump_type, &len)) {
		if (dump_type != BNXT_DUMP_CRASH && dump_type != BNXT_DUMP_DRIVER)
