...
{
	u32 cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_read_clr_cmd);
	...
	cmd->host_address_1 = (u32)rd_parms->host_address;
	cmd->host_address_2 = (u32)(rd_parms->host_address >> 32);
...
static int compose_mpc_read_msg(u8 *cmd_buff, u32 *cmd_buff_len,
				struct cfa_mpc_cache_axs_params *parms)
{
	u32 cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_read_cmd);
	struct cfa_mpc_cache_read_params *rd_parms = &parms->read;
...
static int compose_mpc_write_msg(u8 *cmd_buff, u32 *cmd_buff_len,
				 struct cfa_mpc_cache_axs_params *parms)
{
	u32 cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_write_cmd) +
...
static int compose_mpc_evict_msg(u8 *cmd_buff, u32 *cmd_buff_len,
				 struct cfa_mpc_cache_axs_params *parms)
{
	u32 cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_invalidate_cmd);
				     ...
				     struct cfa_mpc_em_op_params *parms)
{
	...
	cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_em_search_cmd) +
				     ...
				     struct cfa_mpc_em_op_params *parms)
{
	...
	cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_em_insert_cmd) +
		   e->data_size * MPC_CFA_CACHE_ACCESS_UNIT_SIZE;
	if (e->data_size < 1 || e->data_size > 4) {
		ASSERT_RTNL();
		return -EINVAL;
...
static int compose_mpc_em_delete_msg(u8 *cmd_buff, u32 *cmd_buff_len,
				     struct cfa_mpc_em_op_params *parms)
{
	u32 cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_em_delete_cmd);
...
static int compose_mpc_em_chain_msg(u8 *cmd_buff, u32 *cmd_buff_len,
				    struct cfa_mpc_em_op_params *parms)
{
	u32 cmd_size = sizeof(struct mpc_header) + sizeof(struct cfa_mpc_em_chain_cmd);
