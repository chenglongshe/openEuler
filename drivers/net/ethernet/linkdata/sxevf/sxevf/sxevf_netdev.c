...
STATIC s32 sxevf_mac_addr_set(struct net_device *netdev, void *p)
{
	...
	if (ret) {
		LOG_ERROR_BDF(
			"add vf mac addr:%pM to pf filter fail.(ret:%d)\n",
			addr->sa_data, ret);
		goto l_out;
	}
...
static void sxevf_dcb_configure(struct sxevf_adapter *adapter)
{
	...
	if (ret) {
		LOG_ERROR_BDF(
			"get pf ring cfg info fail, use default_tc ring num."
			"(err:%d)\n",
			ret);
		goto l_end;
	}
...
int sxevf_open(struct net_device *netdev)
{
	...
	if (test_bit(SXEVF_HW_STOP, &hw->state)) {
		...
		if (test_bit(SXEVF_HW_STOP, &hw->state)) {
			...
			LOG_DEV_ERR(
				"open process reset vf still fail.(err:%d)\n",
				ret);
			goto l_reset;
		}
	}
	...
	if (ret) {
		LOG_ERROR_BDF(
			"rx config failed, reset and wait for next insmod\n");
		goto l_free_tx;
	}
...
{
	netdev->features = NETIF_F_SG | NETIF_F_SCTP_CRC | NETIF_F_RXCSUM |
			   NETIF_F_HW_CSUM;
