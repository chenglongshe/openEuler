...
s32 sxevf_dev_reset(struct sxevf_hw *hw)
{
	...
	if (!retry) {
		ret = -SXEVF_ERR_RESET_FAILED;
		LOG_ERROR_BDF(
			"retry use up, vflr has not reset done.(err:%d)\n",
			ret);
		goto l_out;
	}
	...
	if (msg.msg_type == (SXEVF_RESET | SXEVF_MSGTYPE_ACK)) {
		...
		LOG_ERROR_BDF(
			"pf handle vf reset msg fail, rcv msg:0x%x.(err:%d)\n",
			msg.msg_type, ret);
		goto l_out;
	}
...
STATIC int sxevf_config_dma_mask(struct sxevf_adapter *adapter)
{
	...
	if (dma_set_mask_and_coherent(&adapter->pdev->dev,
		...
		LOG_ERROR_BDF(
			"device[pci_id %u] 64 dma mask and coherent set failed\n",
			adapter->pdev->dev.id);
		...
		if (ret) {
			LOG_DEV_ERR(
				"device[pci_id %u] 32 dma mask and coherent set failed\n",
				adapter->pdev->dev.id);
		}
	}
...
STATIC int sxevf_pci_init(struct sxevf_adapter *adapter)
{
	...
	if (!adapter->hw.reg_base_addr) {
		ret = -EIO;
		LOG_ERROR_BDF(
			"device[pci_id %u] ioremap[bar_base_paddr = 0x%llx, len = %zu] failed\n",
	...
	LOG_INFO_BDF(
		"bar_base_paddr = 0x%llx, bar len = %zu, reg_base_addr = %p\n",
...
STATIC struct sxevf_adapter *sxevf_adapter_create(struct pci_dev *pdev)
{
	...
	if (!netdev) {
		LOG_ERROR(
			"max:%d device[pci_id %u] sxe net device alloc failed\n",
...
STATIC s32 sxevf_resume(struct device *dev)
{
	...
	if (ret) {
		LOG_DEV_ERR("enable pci device from suspend fail.(err:%d)",
			    ret);
		goto l_end;
	}
