...
STATIC bool sxevf_ack_poll(struct sxevf_hw *hw)
{
	...
	if (!retry) {
		LOG_ERROR_BDF(
			"send msg to pf, retry:%d but don't check pf ack, "
			"init mbx retry to 0.\n",
			mbx->retry);
		mbx->retry = 0;
		ret = false;
...
static s32 sxevf_send_msg_to_pf(struct sxevf_hw *hw, u32 *msg, u16 msg_len)
{
	...
	if (!mbx->retry) {
		...
		LOG_ERROR_BDF(
			"msg:0x%x len:%d send fail due to retry:0.(err:%d)\n",
			msg[0], msg_len, ret);
		goto l_out;
	}
	...
	if (ret) {
		LOG_ERROR_BDF(
			"msg:0x%x len:%d send lock mailbox fail.(err:%d)\n",
			msg[0], msg_len, ret);
		goto l_out;
	}
	...
	if (!sxevf_ack_poll(hw)) {
		...
		LOG_ERROR_BDF(
			"msg:0x%x len:%d send done, but don't poll ack.\n",
			msg[0], msg_len);
		goto l_out;
	}
...
STATIC s32 __sxevf_mc_addr_sync(struct sxevf_hw *hw, struct net_device *netdev)
{
	...
	LOG_INFO_BDF("msg_type:0x%x len:%zu mc_cnt:%d msg "
		     "result:0x%x.(ret:%d)\n",
		     msg.msg_type, SXEVF_MSG_NUM(sizeof(msg)), mc_cnt, result,
		     ret);
...
s32 sxevf_uc_addr_sync(struct sxevf_hw *hw, struct net_device *netdev)
{
	...
	if (uc_cnt) {
		...
		LOG_INFO_BDF(
			"dev uc list null, send msg to pf to clear vf mac list.\n");
