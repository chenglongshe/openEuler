...
#define SXEVF_STAT(m)                                                          \
	SXEVF_STATS, sizeof(((struct sxevf_adapter *)0)->m),                   \
		offsetof(struct sxevf_adapter, m)

#define SXEVF_NETDEV_STAT(m)                                                   \
	NETDEV_STATS, sizeof(((struct net_device_stats *)0)->m),               \
...
static void sxevf_get_ringparam(
...
#ifdef HAVE_ETHTOOL_EXTENDED_RINGPARAMS
static int sxevf_set_ringparam(
	struct net_device *netdev, struct ethtool_ringparam *user_param,
	struct kernel_ethtool_ringparam __always_unused *kernel_ring,
	struct netlink_ext_ack __always_unused *extack)
#else
			  ...
			  u8 *hfunc)
{
	...
	if (indir) {
		err = sxevf_redir_tbl_get(&adapter->hw,
					  adapter->rx_ring_ctxt.num, indir);
	}
