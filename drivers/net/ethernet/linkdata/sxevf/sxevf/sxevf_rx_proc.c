...
static s32 sxevf_rx_ring_alloc(struct sxevf_ring *ring)
{
	...
	if (xdp_rxq_info_reg(&ring->xdp_rxq, adapter->netdev, ring->idx, 0) <
	    0) {
				     ...
				     struct sxevf_ring *rx_ring)
{
	...
	if (max_frame > SXEVF_MAX_FRAME_BUILD_SKB) {
		...
		LOG_INFO_BDF(
			"in rx max frame build skb mode, buffer size=3K\n");
	}
			    ...
			    struct sxevf_ring *rx_ring, const u32 budget)
{
	...
	LOG_DEBUG(
		"entry rx IRQ:irq=%u, ring_idx=%u, ring_reg_idx=%u, ring_tc_idx=%u,"
