			   ...
			   struct ethtool_coalesce *user)
{
	...
	if (is_mixed) {
		if (user->tx_coalesce_usecs) {
			ret = -EINVAL;
			LOG_ERROR_BDF(
				"irq both has rx and rx ring, rx_coalesce_usecs:%u"
	...
	if (adapter->irq_ctxt.tx_irq_interval ==
	    SXEVF_IRQ_ITR_CONSTANT_MODE_VALUE) {
...
static s32 sxevf_irq_ring_bind(struct sxevf_adapter *adapter)
{
	...
	if (irq_num >= total_ring) {
		for (; rxr_remain > 0; irq_idx++, irq_num--) {
			...
			sxevf_irq_interval_init(&adapter->irq_ctxt, irq_idx, 0,
						1);

			sxevf_rx_ring_init(adapter, 0, 1, rxr_idx, irq_idx,
					   rxr_idx);
		...
		LOG_INFO_BDF(
			"alloc rx irq resource priority done.irq_idx:%u "
	...
	for (; irq_num; irq_idx++, irq_num--) {
		...
		sxevf_tx_ring_init(adapter, 0, txr_cnt, txr_idx, irq_idx,
				   tx_reg_idx);

		sxevf_xdp_ring_init(adapter, txr_cnt, xdp_cnt, xdp_idx, irq_idx,
				    xdp_reg_idx);

		sxevf_rx_ring_init(adapter, txr_cnt + xdp_cnt, rxr_cnt, rxr_idx,
				   irq_idx, rxr_idx);
...
static s32 sxevf_msix_request_irqs(struct sxevf_adapter *adapter)
{
	...
	for (irq_idx = 0; irq_idx < irq_ctxt->ring_irq_num; irq_idx++) {
		...
		if (!(sxevf_set_irq_name(irq_data, netdev->name, &rx_idx,
					 &tx_idx))) {
			continue;
		}
...
s32 sxevf_poll(struct napi_struct *napi, int weight)
{
	...
	if (likely(napi_complete_done(napi, total_cleaned))) {
		LOG_INFO_BDF(
			"weight:%d cleand:%u total_cleaned:%d per_ring_budget:%d "
			" complete done\n",
			weight, cleaned, total_cleaned, per_ring_budget);
		if (adapter->irq_ctxt.rx_irq_interval ==
		    SXEVF_IRQ_ITR_CONSTANT_MODE_VALUE) {
		...
		if (!test_bit(SXEVF_DOWN, &adapter->state)) {
			hw->irq.ops->specific_irq_enable(
