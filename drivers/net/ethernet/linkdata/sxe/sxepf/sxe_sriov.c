...
static void sxe_pf_promisc_vlvf_update(struct sxe_adapter *adapter, u32 vid)
{
	...
	if (!bits &&
		...
		if (!(adapter->cap & SXE_VLAN_PROMISC)) {
			hw->filter.vlan.ops->pool_filter_bitmap_write(hw, word,
								      0);
		}
				 ...
				 u32 vf)
{
	...
	if (add && test_bit(vid, adapter->vlan_ctxt.active_vlans)) {
		err = hw->filter.vlan.ops->filter_configure(
			hw, vid, PF_POOL_INDEX(0), true, false);
		if (err) {
...
void sxe_vf_rate_update(struct sxe_adapter *adapter)
{
	...
	if (sxe_mbps_link_speed_get(adapter->link.speed) !=
		...
		LOG_DEV_INFO(
			"link speed has been changed. disable vf tx rate.\n");
	}
		    ...
		    __be16 vlan_proto)
{
	...
	if (vlan || qos) {
		if (adapter->vt_ctxt.vf_info[vf].pf_vlan) {
			LOG_INFO_BDF(
				"vf[%u] pf_vlan[%u] exist, disable first\n", vf,
...
static void sxe_clear_vf_vlan(struct sxe_adapter *adapter, u32 vf)
{
	...
	for (i = SXE_VLVF_ENTRIES; i--;) {
		...
		bits[word % 2] = vlvfb;
		bits[~word % 2] = hw->filter.vlan.ops->pool_filter_bitmap_read(
			hw, (word ^ 1));
...
s32 sxe_set_vf_mac(struct net_device *dev, s32 vf_idx, u8 *mac_addr)
{
	...
	if (is_valid_ether_addr(mac_addr)) {
		...
		if (ret < 0) {
			...
			if (test_bit(SXE_DOWN, &adapter->state)) {
				LOG_DEV_WARN(
					"vf:%u mac address has been set by pf, "
					"but pf device is not up.\n",
					vf_idx);
				LOG_DEV_WARN(
					"bring pf device up before attempting "
		...
		if (is_zero_ether_addr(vf_info->mac_addr)) {
			LOG_INFO_BDF(
				"vf:%u mac addr is zero, skip dup set to zero.\n",
				vf_idx);
			goto l_out;
		}
...
static void sxe_vf_rxtx_rst(struct sxe_adapter *adapter, u8 vf_idx)
{
	...
	hw->dma.ops->pool_rx_ring_drop_enable(hw, vf_idx, vf_info->pf_vlan,
					      ring_per_pool);
...
static void sxe_vf_rst_msg_reply(struct sxe_adapter *adapter, u8 vf_idx)
{
	...
	LOG_INFO_BDF(
		"vf_idx:%d reset msg:0x%x handle done.mac addr:%pM mc type:%d "
...
static s32 sxe_vf_mc_addr_sync(struct sxe_adapter *adapter, u32 *msg, u8 vf_idx)
{
	...
	for (i = 0; i < SXE_MTA_ENTRY_NUM_MAX; i++) {
		hw->filter.mac.ops->mta_hash_table_set(
			hw, i, mac_filter->mc_hash_table[i]);
	}
...
static void sxe_vf_uc_addr_del(struct sxe_adapter *adapter, u8 vf_idx)
{
	...
	list_for_each_entry (entry, &vf->head.list, list) {
		if (entry->vf_idx == vf_idx) {
			...
			LOG_INFO_BDF(
				"del vf:%d mac addr:%pM in mac list done(ret:%d).\n",
				       ...
				       u8 vf_idx)
{
	...
	if (index) {
		if (!is_valid_ether_addr(mac_addr)) {
			ret = -SXE_ERR_PARAM;
			LOG_ERROR_BDF(
				"index:%u vf:%d set invalid addr:%pM.(err:%d)\n",
				...
				u8 vf_idx)
{
	...
	if (num_tc > 1) {
		default_tc = netdev_get_prio_tc_map(
			adapter->netdev, adapter->dcb_ctxt.default_up);
				    ...
				    u8 vf_idx)
{
	...
	if (vf_info->cast_mode == mode) {
		LOG_INFO_BDF("vf:%d msg.cast_mode:0x%x mode:0x%x trust:%d\n",
			     vf_idx, cast_msg.cast_mode, mode,
			     vf_info->trusted);
		goto l_out;
	}
	...
	if (mode == SXE_CAST_MODE_PROMISC) {
		...
		struct sxe_uc_addr_table *uc_table =
			&adapter->mac_filter_ctxt
				 .uc_addr_table[SXE_DEFAULT_UC_ADDR_IDX];
		...
		for (i = 0; i < SXE_UC_ENTRY_NUM_MAX; i++) {
			...
			if ((entry->pool == vf_idx) &&
				...
				sxe_uc_addr_reuse_add(hw, i, vf_info->mac_addr,
						      vf_idx);
			}
		}
...
static s32 sxe_vf_link_enable_get(struct sxe_adapter *adapter, u32 *msgbuf,
				  u8 vf_idx)
{
	s32 ret = 0;
...
STATIC s32 sxe_req_msg_handle(struct sxe_adapter *adapter, u32 *msg, u8 vf_idx)
{
	...
	if (!adapter->vt_ctxt.vf_info[vf_idx].is_ready) {
		...
		ret = hw->mbx.ops->msg_send(
			hw, msg, SXE_MSG_NUM(sizeof(msg[0])), vf_idx);
	...
	if (!ret) {
		...
		msg[0] |= SXE_MSGTYPE_NACK;
		LOG_INFO_BDF(
			"vf:%d msg:0x%x no handler or handle fail.(ret:%d)\n",
			vf_idx, msg[0], ret);
	}
		      ...
		      struct ifla_vf_info *info)
{
	...
	LOG_INFO_BDF(
		"vf_idx:%d get config info pf_vlan:%d pf_qos:%d spoof_chk:%d "
...
void sxe_vt1_configure(struct sxe_adapter *adapter)
{
	...
	while (pf_pool_num--) {
		hw->filter.mac.ops->pool_rx_mode_set(
			hw, SXE_VMOLR_AUPE, PF_POOL_INDEX(pf_pool_num));
	}
...
bool sxe_vf_tx_pending(struct sxe_adapter *adapter)
{
	...
	for (vf_idx = 0; vf_idx < adapter->vt_ctxt.num_vfs; vf_idx++) {
		for (ring_idx = 0; ring_idx < ring_per_pool; ring_idx++) {
			hw->dma.ops->tx_ring_info_get(
				hw, vf_idx * ring_per_pool + ring_idx, &head,
				&tail);
...
void sxe_sriov_init(struct sxe_adapter *adapter)
{
	...
	if (max_vfs > 0) {
		...
		if (max_vfs > SXE_VF_DRV_MAX) {
			LOG_DEV_WARN(
				"max_vfs parameter invalid:%u, no assigne vfs\n",
				max_vfs);
			max_vfs = 0;
		}
	}
...
static s32 sxe_sriov_disable(struct pci_dev *pdev)
{
	...
	if (assigned) {
		ret = -EPERM;
		LOG_DEV_ERR(
			"%d vf assigned to guest, can't disable sriov.(err:%d)\n",
			assigned, ret);
		goto l_out;
	}
	...
	if ((cap != adapter->cap) || (vf != pci_num_vf(adapter->pdev))) {
		LOG_INFO_BDF(
			"previous cap:0x%x vf:%d, now cap:0x%x vf:%d changed.\n",
...
static s32 sxe_sriov_enable(struct pci_dev *pdev, u8 num_vfs)
{
	...
	if (adapter->cap & SXE_SRIOV_ENABLE) {
		...
		if (ret) {
			LOG_MSG_ERR(
				probe,
				"num_vfs:%d enable pci sriov fail.(err:%d)\n",
				num_vfs, ret);
			goto l_vf_free;
		}
		...
		LOG_INFO_BDF(
			"cap:0x%x cap2:0x%x num_vfs:%d enable sriov success.\n",
