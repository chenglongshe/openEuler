...
void sxe_uc_addr_reuse_del(struct sxe_hw *hw, const u8 *addr, u16 pool_idx)
{
	...
	if (pool_idx == PF_POOL_INDEX(0)) {
		hw->filter.mac.ops->uc_addr_pool_disable(
			hw, SXE_DEFAULT_UC_ADDR_IDX);
	}
	...
	for (i = 0; i < SXE_UC_ENTRY_NUM_MAX; i++) {
		...
		if ((entry->pool == pool) &&
			...
			if ((test_and_clear_bit(SXE_UC_ADDR_ENTRY_USED,
				...
				spin_unlock(
					&adapter->mac_filter_ctxt.uc_table_lock);

				LOG_INFO(
					"pool:%u mac addr:%pM uc_filter_addr[%u] "
					"entry del success.\n",
					pool, addr, i);
				goto l_out;
			}
...
static s32 sxe_uc_filter_init(struct sxe_adapter *adapter)
{
	...
	if (!mac_filter->uc_addr_table) {
		ret = -ENOMEM;
		LOG_ERROR_BDF(
			"rar entry:%d size:%lu mac table kcalloc fail.(err:%d)",
...
void sxe_fnav_rules_restore(struct sxe_adapter *adapter)
{
	...
	if (!hlist_empty(&adapter->fnav_ctxt.rules_list)) {
		hw->dbu.ops->fnav_specific_rule_mask_set(
			hw, &adapter->fnav_ctxt.rules_mask);

		hlist_for_each_entry_safe (
			rule, node, &adapter->fnav_ctxt.rules_list, node) {
			ring_cookie = rule->ring_cookie;

			sxe_fnav_dest_queue_parse(adapter, ring_cookie, &queue);

			hw->dbu.ops->fnav_specific_rule_add(
				hw, &rule->rule_info, rule->sw_idx, queue);
		}
	}
					       ...
					       unsigned int *hlen)
{
	...
	if (unlikely(skb_tail_pointer(skb) <
		...
		ret = -SXE_ERR_PARAM;
		LOG_ERROR(
			"error on length skb_tail_pointer=0x%p < "
...
static void sxe_fnav_sample_rule_add(struct sxe_adapter *adapter, u64 hash_cmd)
{
	...
	if (!input) {
		...
		hw->dbu.ops->fnav_single_sample_rule_del(
			hw, (u32)(hash_cmd & SXE_FNAV_HASH_REG_MASK));
		goto l_end;
	}
					  ...
					  struct sxe_fnav_rule_node *add_rule)
{
	...
	if (rule && (rule->sw_idx == sw_idx)) {
		...
		if (rule->rule_info.ntuple.bkt_hash !=
		    add_rule->rule_info.ntuple.bkt_hash) {
			ret = hw->dbu.ops->fnav_specific_rule_del(
				hw, &rule->rule_info, sw_idx);
			if (ret) {
					   ...
					   union sxe_fnav_rule_info *input_mask)
{
	...
	for (i = 1; i <= 15; i++) {
		sxe_fnav_specific_hash_iter_compute(
			i, &bucket_hash, lo_hash_dword, hi_hash_dword);
	}
				       ...
				       union sxe_fnav_rule_info *mask, u8 queue)
{
	...
	LOG_DEBUG_BDF(
		"add specific fnav mask---rule_info:vm_pool[%u], flow_type[0x%x]"
	...
	LOG_DEBUG_BDF(
		"add specific fnav rule---filter:vm_pool[%u], flow_type[0x%x]"
		"vlan_id[%u], dst_ip[%x:%x:%x:%x], src_ip[%x:%x:%x:%x]"
		"dst_port[%u], src_port[%u], flex_bytes[0x%x], bkt_hash[0x%x]"
