...
void sxe_tx_ring_buffer_clean(struct sxe_ring *ring)
{
	...
	if (!ring_is_xdp(ring))
		...
		netdev_tx_reset_queue(
			netdev_get_tx_queue(ring->netdev, ring->idx));
#ifdef HAVE_AF_XDP_ZERO_COPY
l_out:
#endif
	ring->next_to_use = 0;
...
static s32 sxe_tx_resources_alloc(struct sxe_adapter *adapter)
{
	...
	for (i = 0; i < adapter->tx_ring_ctxt.num; i++) {
		...
		if (ret < 0) {
			LOG_MSG_ERR(probe,
				    "allocation for Tx Queue %d failed\n", i);
			goto l_tx_free;
		}
	}
	...
	for (j = 0; j < adapter->xdp_ring_ctxt.num; j++) {
		...
		if (ret < 0) {
			LOG_MSG_ERR(probe,
				    "allocation for xdp Queue %d failed\n", j);
			goto l_xdp_free;
		}
	}
...
s32 sxe_tx_ring_depth_reset(struct sxe_adapter *adapter, u32 tx_cnt)
{
	...
	for (j = 0; j < adapter->xdp_ring_ctxt.num; j++, i++) {
		...
		if (ret < 0) {
			LOG_ERROR_BDF(
				"xdp ring alloc failed, xdp ring idx=%d\n", j);
			goto l_tx_free;
		}
	}
				 ...
				 struct sxe_tx_context_desc *ctxt_desc)
{
	...
	LOG_DEBUG_BDF(
		"contxt desc, ring=%u, ntu=%u, ntc=%u, vlan_macip_lens=%#x, "
			     ...
			     union sxe_tx_data_desc **desc, u16 *next_to_use)
{
	...
	for (frag = &skb_shinfo(skb)->frags[0];; frag++) {
		...
		while (unlikely(map_size > SXE_DATA_PER_DESC_SIZE_MAX)) {
			(*desc)->read.cmd_type_len = cpu_to_le32(
				cmd_type ^ SXE_DATA_PER_DESC_SIZE_MAX);
		...
		if (likely(!remaining_size)) {
			...
			LOG_DEBUG_BDF(
				"skb dma map, current_map_size=%u, remaining_size=%u, "
			      ...
			      u16 *budget, struct sxe_ring_stats *ring_stats)
{
	...
	LOG_DEBUG_BDF(
		"tx ring clean start: ring idx=%u, reg_idx=%u, next_to_use=%d, "
...
#endif
		LOG_DEBUG_BDF(
			"tx ring clean: budget=%d, bytes=%llu, packet=%llu\n",
	...
	LOG_DEBUG_BDF(
		"tx ring clean end: ring idx=%u, reg_idx=%u, next_to_use=%d, "
