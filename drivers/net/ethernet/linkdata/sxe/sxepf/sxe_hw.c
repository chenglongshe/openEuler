		...
		if (unlikely(SXE_REG_READ_FAIL != value)) {
			...
			for (i = 0; i < SXE_REG_READ_RETRY; i++) {
				value = rte_le_to_cpu_32(
					rte_read32(base_addr + SXE_STATUS));
				if (unlikely(SXE_REG_READ_FAIL != value)) {
					value = rte_le_to_cpu_32(
						rte_read32(base_addr + reg));
					LOG_INFO(
						"reg[0x%x] read ok, value=%#x\n",
						reg, value);
					break;
				} else {
					LOG_ERROR(
...
STATIC s32 sxe_hw_regs_test(struct sxe_hw *hw)
{
	...
	while (test->reg) {
		for (i = 0; i < test->array_len; i++) {
			...
			case PATTERN_TEST:
				ret = sxe_hw_reg_pattern_test(
					hw, test->reg + (i * 0x40), test->mask,
					test->write);
				break;
			case TABLE32_TEST:
				ret = sxe_hw_reg_pattern_test(
					hw, test->reg + (i * 4), test->mask,
					test->write);
				break;
			case TABLE64_TEST_LO:
				ret = sxe_hw_reg_pattern_test(
					hw, test->reg + (i * 8), test->mask,
					test->write);
				break;
		...
		*icr = 0;
		...
		if (!shared) {
			...
			*icr = 0;
			...
			if (*icr) {
				LOG_ERROR_BDF(
					"test irq: other irq failed, eicr = %x\n",
					*icr);
...
s32 sxe_hw_fc_enable(struct sxe_hw *hw)
{
	...
	for (i = 0; i < MAX_TRAFFIC_CLASS; i++) {
		if ((hw->fc.current_mode & SXE_FC_TX_PAUSE) &&
			...
			fcrth = (SXE_REG_READ(hw, SXE_RXPBSIZE(i)) - 24576) >>
				1;
		}
...
bool sxe_device_supports_autoneg_fc(struct sxe_hw *hw)
{
	...
	if (link_up) {
		supported =
			(link_speed == SXE_LINK_SPEED_1GB_FULL) ? true : false;
	}
...
static void sxe_hw_vlan_pool_filter_write(struct sxe_hw *hw, u16 reg_index,
					  u32 value)
{
...
static void sxe_hw_rss_hash_pkt_type_update(struct sxe_hw *hw, u32 version)
{
	u32 mrqc;
				 ...
				 u32 sxe_fnav_mode)
{
	...
	if ((sxe_fnav_mode != SXE_FNAV_SAMPLE_MODE) &&
		...
		LOG_ERROR_BDF(
			"mode[%u] a error fnav mode, fnav do not work. please use"
...
u64 sxe_hw_ptp_systime_get(struct sxe_hw *hw)
{
	...
	LOG_DEBUG_BDF(
		"get ptp hw systime systiml=%u, systimm=%u, ns=%" SXE_PRIU64
		"\n",
		systiml, systimm, ns);
	return ns;
}
...
u64 sxe_hw_ptp_rx_timestamp_get(struct sxe_hw *hw)
{
	...
	LOG_DEBUG_BDF(
		"ptp get rx ptp timestamp low=%u, high=%u, ns=%" SXE_PRIU64
		"\n",
		rxtsl, rxtsh, ns);
	return ns;
}
				   ...
				   u32 tses)
{
	...
	if (is_l2) {
		SXE_REG_WRITE(hw, SXE_ETQF(SXE_ETQF_FILTER_1588),
			      (SXE_ETQF_FILTER_EN | SXE_ETQF_1588 |
			       ETH_P_1588));
	} else {
...
s32 sxe_hw_rcv_msg_from_vf(struct sxe_hw *hw, u32 *msg, u16 msg_len, u16 index)
{
	...
	if (!sxe_hw_mbx_lock(hw, index)) {
		...
		LOG_ERROR_BDF(
			"vf idx:%d msg_len:%d rcv lock mailbox fail.(err:%d)\n",
			index, msg_len, ret);
		goto l_out;
	}
...
s32 sxe_hw_send_msg_to_vf(struct sxe_hw *hw, u32 *msg, u16 msg_len, u16 index)
{
	...
	if (msg_len > mbx->msg_len) {
		ret = -EINVAL;
		LOG_ERROR_BDF(
			"pf reply msg num:%d exceed limit:%d reply fail.(err:%d)\n",
				  ...
				  const struct sxe_reg_info *regs, u32 *reg_buf)
{
	...
	while (regs[i].count) {
		for (j = 0; j < regs[i].count; j++) {
			reg_buf[count + j] = SXE_REG_READ(
				hw, regs[i].addr + j * regs[i].stride);
...
void sxe_hw_mirror_rule_clear(struct sxe_hw *hw, u8 rule_id)
{
	...
	SXE_REG_WRITE(
		hw, SXE_VMRVM(rule_id + SXE_MR_VIRTUAL_POOL_MSB_REG_OFFSET), 0);
