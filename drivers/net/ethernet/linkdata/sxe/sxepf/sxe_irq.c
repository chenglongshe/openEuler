...
s32 sxe_config_space_irq_num_get(struct sxe_adapter *adapter)
{
	...
	if (msix_num == SXE_READ_CFG_WORD_FAILED) {
		ret = -EIO;
		LOG_ERROR_BDF(
			"msi-x caps read fail due to adapter removed.(err:%d)\n",
			ret);
		goto l_out;
	}
...
void sxe_disable_dcb(struct sxe_adapter *adapter)
{
	if (sxe_dcb_tc_get(adapter) > 1) {
		LOG_DEV_WARN(
			"number of DCB TCs exceeds number of available queues."
...
static s32 sxe_msix_irq_init(struct sxe_adapter *adapter)
{
	...
	if (adapter->irq_ctxt.msix_entries == NULL) {
		ret = -ENOMEM;
		LOG_ERROR_BDF(
			"msi-x irq entry num:%u per size:%lu kcalloc fail."
			"(err:%d)\n",
			      ...
			      u16 irq_idx, bool is_bind)
{
	...
	if (irq_data == NULL) {
		LOG_ERROR_BDF(
			"alloc interrupt data and ring resource in node:%d "
	...
	LOG_INFO_BDF(
		"irq_idx:%u ring_cnt:%u is_bind:%d bind to cpu:%d node:%d. "
		"tx work_limit:%d \n",
...
static s32 sxe_irq_ring_bind(struct sxe_adapter *adapter)
{
	...
	if (irq_num >= total_ring) {
		for (; rxr_remain > 0; irq_idx++) {
			...
			sxe_irq_interval_init(&adapter->irq_ctxt, irq_idx, 0,
					      1);
	...
	for (; irq_idx < irq_num; irq_idx++) {
		...
		if (ret) {
			LOG_ERROR_BDF(
				"irq_num:%u rxr_remain:%u txr_remain:%u "
	...
	if (eicr & SXE_EICR_GPI_SPP2) {
		...
		if (!test_bit(SXE_DOWN, &adapter->state)) {
			...
			if (!(state & SXE_SPP2_STATE)) {
				...
				hw->irq.ops->spp_configure(
					&adapter->hw, SXE_SPP_PROC_DELAY_US);
				adapter->phy_ctxt.sfp_info.inserted = false;
				LOG_INFO_BDF(
					"sfp is extracted, disable rxlos, spp_configure is default 7us\n");
			} else {
	...
	if (eicr & SXE_EICR_GPI_SPP1) {
		...
		if (!test_bit(SXE_DOWN, &adapter->state) &&
			...
			if (time_after(jiffies,
				       adapter->link.last_lkcfg_time +
#ifdef SXE_SFP_DEBUG
					       (HZ * sw_sfp_los_delay_ms) /
						       SXE_HZ_TRANSTO_MS)
#else
...
static int sxe_msix_request_irqs(struct sxe_adapter *adapter)
{
	...
	for (irq_idx = 0; irq_idx < irq_ctxt->ring_irq_num; irq_idx++) {
		...
		if (!(sxe_set_irq_name(irq_data, netdev->name, &rx_idx,
				       &tx_idx))) {
			continue;
		}
...
l_free_irq:
	while (irq_idx) {
		...
		irq_set_affinity_hint(
			adapter->irq_ctxt.msix_entries[irq_idx].vector, NULL);
...
static void sxe_configure_msix_hw(struct sxe_adapter *adapter)
{
	...
	for (irq_idx = 0; irq_idx < irq_ctxt->ring_irq_num; irq_idx++) {
		...
		if (irq_data->tx.xdp_ring) {
			hw->irq.ops->ring_irq_map(
				hw, true, irq_data->tx.xdp_ring->reg_idx,
				irq_idx);
		}
			 ...
			 struct ethtool_coalesce *user)
{
	...
	if (is_mixed) {
		if (user->tx_coalesce_usecs) {
			ret = -EINVAL;
			LOG_ERROR_BDF(
				"irq_idx:0 bind tx ring cnt:%u rx ring cnt:%u"
...
int sxe_poll(struct napi_struct *napi, int weight)
{
	...
	if (ring) {
		if (ring->xsk_pool) {
			LOG_DEBUG_BDF(
				"ring[%u] has xsk_umem, clean xdp tx irq\n",
				ring->idx);
			clean_complete = sxe_xdp_tx_ring_irq_clean(
				irq_data, ring, weight);
		}
	}
#endif
	if (weight <= 0) {
	...
	if (likely(napi_complete_done(napi, total_cleaned))) {
		...
		if (!test_bit(SXE_DOWN, &adapter->state)) {
			hw->irq.ops->ring_irq_enable(
				hw, BIT_ULL(irq_data->irq_idx));
		}
	}
