...
#define SXE_STAT(m)                                                            \
	SXE_STATS, sizeof(((struct sxe_adapter *)0)->m),                       \
		offsetof(struct sxe_adapter, m)
#define SXE_NETDEV_STAT(m)                                                     \
			      ...
			      u8 *queue)
{
	...
	if (ring_cookie == RX_CLS_FLOW_DISC) {
		*queue = SXE_FNAV_DROP_QUEUE;
		...
		if (!vf && (ring >= adapter->rx_ring_ctxt.num)) {
			LOG_ERROR_BDF(
				"input ring[%u] exceed max rx ring num[%u]\n",
			...
			LOG_ERROR_BDF(
				"input vf[%u] exceed max vf num[%u] or "
	...
	LOG_DEBUG_BDF(
		"parse fnav dest queue end, ring_cookie = 0x%llx, queue = %u\n",
		ring_cookie, *queue);
l_ret:
	return ret;
}
				       ...
				       struct ethtool_rx_flow_spec *flow)
{
	...
	LOG_DEBUG_BDF(
		"parse rule to user src_port[%u], dst_port[%u], src_ip[0x%x], "
	...
	LOG_DEBUG_BDF(
		"parse rule mask to user src_port[%u], dst_port[%u], src_ip[0x%x],"
		" dst_ip[0x%x] vlan_id[%u], flex_bytes[0x%x], vm_pool[%u]\n",
...
static int sxe_rss_hash_srcs_update(struct ethtool_rxnfc *nfc, u32 *cap)
{
	...
	case UDP_V4_FLOW:
		if (!(nfc->data & RXH_IP_SRC) || !(nfc->data & RXH_IP_DST)) {
			LOG_ERROR(
				"data[0x%llx] in flow_type[UDP_V4_FLOW] does not"
				"exist support hash srcs\n",
				nfc->data);
			goto l_err_ret;
		}
	...
	case UDP_V6_FLOW:
		if (!(nfc->data & RXH_IP_SRC) || !(nfc->data & RXH_IP_DST)) {
			LOG_ERROR(
				"data[0x%llx] in flow_type[UDP_V6_FLOW] does not"
				"exist support hash srcs\n",
				nfc->data);
			goto l_err_ret;
		}
...
#endif
{
	...
	if ((user_param->rx_mini_pending) || (user_param->rx_jumbo_pending)) {
		LOG_ERROR_BDF(
			"dont support set rx_mini_pending=%u or rx_jumbo_pending=%u\n",
	...
	if (!sxe_ringparam_changed(adapter, user_param, &new_tx_count,
				   &new_rx_count)) {
		LOG_DEBUG_BDF(
			"ring depth dont change, tx_depth=%u, rx_depth=%u\n",
			     ...
			     struct ethtool_channels *ch)
{
	...
	if ((1 == ch->combined_count) || (adapter->cap & SXE_SRIOV_ENABLE) ||
			    ...
			    struct ethtool_channels *ch)
{
	...
	if (!count || ch->rx_count || ch->tx_count ||
		...
		LOG_ERROR_BDF(
			"ethtool set channel failed, combined count=%u, "
	...
	if (ret) {
		LOG_ERROR_BDF(
			"sxe_ring_reassign failed, err=%d, combined count=%u, "
			"rx_count=%u, tx_count=%u, other_count=%u, max_ch=%u"
			"tc=%u\n",
			     ...
			     const struct ethtool_link_ksettings *cmd)
{
	...
	if (phy_ctxt->sfp_info.multispeed_fiber) {
		...
		if (ret) {
			LOG_MSG_INFO(
				probe,
				"setup link failed, ret = %d, advertised=%d\n",
				ret, advertised);
			       ...
			       struct sxe_ring *tx_ring, u32 size)
{
	...
	while (tx_ntc != tx_ring->next_to_use) {
		...
		if (!(tx_desc->wb.status & cpu_to_le32(SXE_TX_DESC_STAT_DD))) {
			LOG_ERROR_BDF(
				"xmit dont completed, next_to_use=%u, count=%u\n",
	...
	netdev_tx_reset_queue(
		netdev_get_tx_queue(tx_ring->netdev, tx_ring->idx));
...
STATIC int sxe_loopback_test_run(struct sxe_adapter *adapter)
{
	...
	for (j = 0; j <= lc; j++) {
		...
		LOG_DEBUG_BDF(
			"max_loop_num=%u, cnt=%u original gptc:%llu gprc:%llu "
