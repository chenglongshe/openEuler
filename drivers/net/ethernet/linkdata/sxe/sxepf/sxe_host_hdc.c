...
static s32 sxe_wait_fw_ack(struct sxe_hw *hw, u64 trace_id)
{
	...
	while (1) {
		...
		if (!ret) {
			...
			LOG_DEBUG_BDF(
				"cmd trace=0x%llx got signal, default quit\n",
				trace_id);
			ret = -EINTR;
			break;
		}
		...
		if (ret) {
			LOG_DEBUG_BDF(
				"cmd trace=0x%llx interrupted, need cancel\n",
				trace_id);
			ret = -EINTR;
			break;
		} else {
			LOG_DEBUG_BDF(
				"cmd trace=0x%llx got other signal, ignore\n",
				trace_id);
		}
#endif
	}
...
static s32 hdc_packet_ack_get(struct sxe_hw *hw, u64 trace_id,
			      HdcHeader_u *pkt_header, bool use_msi,
			      bool is_interruptible)
{
	s32 ret = 0;
...
static void hdc_packet_header_fill(HdcHeader_u *pkt_header, u8 pkt_index,
...
static void hdc_packet_send(struct sxe_hw *hw, u64 trace_id,
			    HdcHeader_u *pkt_header, u8 *data, u16 data_len)
{
	u16 dw_idx = 0;
	u16 pkt_len = 0;
	u16 offset = 0;
	...
	for (dw_idx = 0; dw_idx < pkt_len; dw_idx++) {
		...
		LOG_DEBUG_BDF(
			"trace_id=0x%llx send data to reg[%u] dword[0x%x]\n",
			      ...
			      u16 out_len)
{
	...
	for (dw_idx = 0; dw_idx < dw_num; dw_idx++) {
		...
		offset = dw_idx * BYTE_PER_DWORD;
		LOG_DEBUG_BDF(
			"trace_id=0x%llx get data from reg[%u] dword=0x%x\n",
			   ...
			   u16 in_len, bool use_msi, bool is_interruptible)
{
	...
	for (index = 0; index < pkt_num; index++) {
		LOG_DEBUG_BDF(
			"trace_id=0x%llx fill pkg header[%p], pkg_index[%u], "
				...
				bool use_msi, bool is_interruptible)
{
	...
	if (ret) {
		LOG_ERROR_BDF(
			"hw[%p] cmd trace_id=0x%llx get hdc lock fail, ret=%d\n",
			hw, trace_id, ret);
		ret = -SXE_HDC_RETRY_ERR;
		goto l_ret;
	}
			       ...
			       bool use_msi, bool is_interruptible)
{
	...
	LOG_DEBUG_BDF(
		"hw[%p] %s cmd trace=0x%llx get use sema = %p, count=%u\n", hw,
...
{
	LOG_DEBUG(
		"cmd[opcode=0x%x], trace=0x%llx, req_data_len=%u start init\n",
...
s32 sxe_driver_cmd_trans(struct sxe_hw *hw, struct sxe_driver_cmd *cmd)
{
	...
	if (ack->errcode) {
		LOG_DEV_DEBUG(
			"driver get hdc ack failed trace_id=0x%llx, err=%d\n",
