...
STATIC int sxe_rx_ring_alloc(struct sxe_adapter *adapter, struct sxe_ring *ring)
{
	...
	if (NULL == ring->rx_buffer_info) {
		LOG_ERROR_BDF(
			"ring[%u] can not alloc mem from local numa mode[%d]，"
			"try remote\n",
			ring->idx, ring_node);
		ring->rx_buffer_info = vmalloc(size);
		if (NULL == ring->rx_buffer_info) {
			LOG_ERROR_BDF(
				"ring[%u] unable to allocate memory for the"
				"rx_buffer_info\n",
				ring->idx);
			ret = -ENOMEM;
	...
	if (NULL == ring->desc.base_addr) {
		LOG_ERROR_BDF(
			"ring[%u] unable to alloc base_addr memory from the "
			"same numa mode[%d] with ring, try dev node\n",
			ring->idx, ring_node);
		ring->desc.base_addr = dma_alloc_coherent(
			dev, ring->size, &ring->desc.dma, GFP_KERNEL);
		if (NULL == ring->desc.base_addr) {
...
void sxe_rx_ring_buffer_clean(struct sxe_ring *ring)
{
	...
	LOG_DEBUG(
		"clean ring[%u] from ntc=%u, next_to_alloc=%u, rx_buffer[%p]\n",
				      ...
				      struct sxe_ring *ring)
{
	...
	if (!ring_is_lro_enabled(ring)) {
		...
		hw->dma.ops->rx_lro_ctl_configure(hw, reg_idx,
						  SXE_LROCTL_MAXDESC_16);
	}
						 ...
						 struct sxe_ring *ring)
{
	...
	if (ring->xsk_pool) {
		...
		WARN_ON(xdp_rxq_info_reg_mem_model(
			&ring->xdp_rxq, MEM_TYPE_XSK_BUFF_POOL, NULL));
		xsk_pool_set_rxq_info(ring->xsk_pool, &ring->xdp_rxq);
#else
		ring->zca.free = sxe_zca_free;
		WARN_ON(xdp_rxq_info_reg_mem_model(
			&ring->xdp_rxq, MEM_TYPE_ZERO_COPY, &ring->zca));
#endif
	} else {
...
s32 sxe_rx_configure(struct sxe_adapter *adapter)
{
	...
	if (ret) {
		LOG_ERROR_BDF(
			"allocation for Rx resources failed:Out of memory\n");
		goto l_ret;
	}
...
#define SXE_LAST_OFFSET (SKB_WITH_OVERHEAD(PAGE_SIZE) - SXE_RXBUFFER_3K)
	if (rx_buffer->page_offset > SXE_LAST_OFFSET) {
		LOG_INFO(
			"page[%p] can not reuse rx_buffer->page_offset:%u > %u\n",
			  ...
			  struct sxe_ring *rx_ring, const u32 budget)
{
	...
	LOG_DEBUG_BDF(
		"entry rx irq clean:irq=%u, ring_idx=%u, ring_reg_idx=%u, "
	...
	while (likely(total_rx_packets < budget)) {
		...
		if (IS_ERR(skb)) {
			...
			if (xdp_res & (SXE_XDP_TX | SXE_XDP_REDIR)) {
				xdp_xmit |= xdp_res;
				sxe_rx_buffer_page_offset_update(
