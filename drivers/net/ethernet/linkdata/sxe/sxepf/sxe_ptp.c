...
static long sxe_ptp_clock_create(struct sxe_adapter *adapter)
{
	...
	adapter->ptp_ctxt.ptp_clock = ptp_clock_register(
		&adapter->ptp_ctxt.ptp_clock_info, &adapter->pdev->dev);
...
void sxe_ptp_overflow_check(struct sxe_adapter *adapter)
{
	...
	bool timeout = time_is_before_jiffies(
		adapter->ptp_ctxt.last_overflow_check + SXE_OVERFLOW_PERIOD);

	if (timeout) {
		spin_lock_irqsave(&adapter->ptp_ctxt.ptp_timer_lock, flags);
		timecounter_read(&adapter->ptp_ctxt.hw_tc);
		spin_unlock_irqrestore(&adapter->ptp_ctxt.ptp_timer_lock,
				       flags);
...
{
	struct sxe_adapter *adapter =
		container_of(work, struct sxe_adapter, ptp_ctxt.ptp_tx_work);
	struct sxe_hw *hw = &adapter->hw;
	bool timeout = time_is_before_jiffies(adapter->ptp_ctxt.ptp_tx_start +
					      SXE_PTP_TX_TIMEOUT);
	u32 ts_sec;
	...
	if (ts_ns != adapter->ptp_ctxt.tx_hwtstamp_nsec ||
		...
		for (i = 0; i < SXE_TXTS_POLL_CHECK; i++) {
			hw->dbu.ops->ptp_tx_timestamp_get(hw, &last_sec,
							  &last_ns);
		}
				      ...
				      struct hwtstamp_config *config)
{
	...
	LOG_DEBUG_BDF(
		"ptp set timestamp mode: tx_type[0x%x], rx_filter[0x%x]\n",
	...
	LOG_DEBUG_BDF(
		"hw[%p] set hw timestamp: is_l2=%s, tsctl=0x%x, tses=0x%x\n",
