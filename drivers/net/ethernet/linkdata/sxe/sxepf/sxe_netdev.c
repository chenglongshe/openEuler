...
int sxe_open(struct net_device *netdev)
{
	...
	if (ret) {
		LOG_ERROR_BDF(
			"rx config failed, reset and wait for next insmode\n");
		goto l_free_tx;
	}
...
static void sxe_refill_vfta(struct sxe_adapter *adapter, u32 vfta_offset)
{
	...
	for (i = VFTA_BLOCK_SIZE; i--;) {
		...
		hw->filter.vlan.ops->filter_array_write(hw, vfta_offset + i,
							vfta[i]);
	}
...
static void sxe_vf_vlan_promisc_disable(struct sxe_adapter *adapter)
{
	...
	for (i = 0; i < adapter->vlan_ctxt.vlan_table_size;
	     i += VFTA_BLOCK_SIZE) {
...
static void sxe_vf_vlan_promisc_enable(struct sxe_adapter *adapter)
{
	...
	for (i = SXE_VLVF_ENTRIES; --i;) {
		...
		u32 vlvfb = hw->filter.vlan.ops->pool_filter_bitmap_read(
			hw, reg_offset);
...
void __sxe_set_rx_mode(struct net_device *netdev, bool lock)
{
	...
	if (netdev->flags & IFF_PROMISC) {
		...
		LOG_INFO_BDF(
			"both unicast promisc and multicast promisc enabled."
	...
	if (features & NETIF_F_RXALL) {
		flt_ctrl |= (SXE_FCTRL_SBP | SXE_FCTRL_BAM);
	}

	if (__dev_uc_sync(netdev, sxe_uc_sync, sxe_uc_unsync)) {
		flt_ctrl |= SXE_FCTRL_UPE;
		LOG_ERROR_BDF(
			"uc addr sync fail, enable unicast promisc."
			"flags:0x%x flt_ctrl:0x%x vmolr:0x%x features:0x%llx\n",
			netdev->flags, flt_ctrl, vmolr, features);
	}
	...
	if (count < 0) {
		...
		vmolr |= SXE_VMOLR_MPE;
		LOG_ERROR_BDF(
			"mc addr add fail count:%d, enable multicast promisc."
			"flags:0x%x flt_ctrl:0x%x vmolr:0x%x features:0x%llx\n",
			    ...
			    netdev_features_t features)
{
	...
	if ((features & NETIF_F_NTUPLE) || (features & NETIF_F_HW_TC)) {
		...
		if ((adapter->cap & SXE_SRIOV_ENABLE) ||
			...
			LOG_DEBUG_BDF(
				"can not switch to sample mode. vt_mode=%s,"
				" tcs=%u, rss_limit=%u\n",
				(adapter->cap & SXE_SRIOV_ENABLE) ? "on" :
									  "off",
			...
			adapter->cap |= SXE_FNAV_SAMPLE_ENABLE;
			LOG_DEBUG_BDF(
				"switch to sample mode and need_reset is %s\n",
				need_reset ? "true" : "false");
		}
	}
	...
	if (pool == adapter->pool_f.pf_num_used) {
		...
		if (((adapter->cap & SXE_DCB_ENABLE) &&
		    ...
		    *pf_pools > SXE_MAX_MACVLANS) {
			LOG_ERROR_BDF(
				"macvlan pool exceed the limit, cap=%x, pf_pool_num=%u\n",
		...
		if (total_pools >= SXE_POOLS_NUM_MAX) {
			LOG_ERROR_BDF(
				"pool num exceed the limit, total_pool_num=%u\n",
				total_pools);
			ret = -EBUSY;
			goto l_end;
		}
...
void sxe_macvlan_configure(struct sxe_adapter *adapter)
{
	...
	netdev_for_each_all_upper_dev_rcu(adapter->netdev, upper, iter)
	{
		if (netif_is_macvlan(upper)) {
			...
			if (macvlan->fwd_priv) {
				err = sxe_macvlan_ring_configure(adapter,
								 accel);
				if (err)
					continue;
			}
...
s32 sxe_ring_reassign(struct sxe_adapter *adapter, u8 tc)
{
	...
	if (ret) {
		LOG_ERROR_BDF(
			"interrupt ring assign scheme init failed, err=%d\n",
			ret);
		goto l_end;
	}
...
#endif
{
	...
	if (is_unicast_ether_addr(addr) || is_link_local_ether_addr(addr)) {
		...
		if (netdev_uc_count(dev) >= available_num) {
			LOG_ERROR_BDF(
				"netdev_uc_count=%u >= available_num=%u\n",
...
static s32 sxe_bridge_mode_configure(struct sxe_adapter *adapter, __u16 mode)
{
	...
	default:
		ret = -EINVAL;
		LOG_ERROR_BDF(
			"config hw[%p] bridge mode[%u], num_vfs[%u] failed\n",
...
STATIC void sxe_netdev_feature_init(struct net_device *netdev)
{
	...
	netdev->features = NETIF_F_SG | NETIF_F_RXCSUM | NETIF_F_HW_CSUM |
			   NETIF_F_SCTP_CRC | NETIF_F_RXHASH;
...
#endif
			       NETIF_F_NTUPLE | NETIF_F_LRO | NETIF_F_RXALL;
	...
	netdev->mpls_features |= NETIF_F_SG | NETIF_F_TSO | NETIF_F_TSO6 |
				 NETIF_F_HW_CSUM | SXE_GSO_PARTIAL_FEATURES;
