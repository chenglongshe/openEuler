...
static u8 sxe_dcbnl_state_set(struct net_device *netdev, u8 state)
{
	...
	ret = !!sxe_ring_reassign(
		adapter, state ? adapter->dcb_ctxt.cee_cfg.num_tcs.pg_tcs : 0);
				       ...
				       u8 up_map)
{
	...
	LOG_DEBUG_BDF(
		"tx pg tc config, tc=%d, prio=%u, bwg_id=%u, bwg_pct=%u, up_map=%u\n",
				       ...
				       u8 up_map)
{
	...
	LOG_DEBUG_BDF(
		"rx pg tc config, tc=%d, prio=%u, bwg_id=%u, bwg_pct=%u, up_map=%u\n",
		tc, prio_type, bwg_id, bwg_pct, up_map);
				       ...
				       u8 *up_map)
{
	...
	*up_map = dcb_ctxt->cee_cfg.tc_config[tc]
	...
	LOG_DEBUG_BDF(
		"get tx pg cfg: tc=%d, prio=%u, bwg_id=%u, bwg_pct=%u, up_map=%u\n",
				       ...
				       u8 *up_map)
{
	...
	LOG_DEBUG_BDF(
		"get rx pg cfg: tc=%d, prio=%u, bwg_id=%u, bwg_pct=%u, up_map=%u\n",
		tc, *prio, *bwg_id, *bwg_pct, *up_map);
...
static int sxe_dcbnl_ieee_setets(struct net_device *dev, struct ieee_ets *ets)
{
	...
	if (!adapter->dcb_ctxt.ieee_ets) {
		...
		hw->dma.ops->dcb_rx_up_tc_map_get(
			hw, adapter->dcb_ctxt.ieee_ets->prio_tc);
	}
...
static int sxe_dcbnl_ieee_setpfc(struct net_device *dev, struct ieee_pfc *pfc)
{
	...
	if (!adapter->dcb_ctxt.ieee_pfc) {
		adapter->dcb_ctxt.ieee_pfc =
			kmalloc(sizeof(struct ieee_pfc), GFP_KERNEL);
		if (!adapter->dcb_ctxt.ieee_pfc) {
			LOG_ERROR_BDF("kmalloc failed\n");
			ret = -ENOMEM;
			goto l_end;
		...
		for (vf = 0; vf < adapter->vt_ctxt.num_vfs; vf++) {
			...
			if (!vfinfo->pf_qos) {
				hw->dma.ops->tx_vlan_tag_set(
					hw, vfinfo->pf_vlan, app->priority, vf);
			}
		}
	}
...
void sxe_rx_drop_mode_set(struct sxe_adapter *adapter)
{
	...
	if (adapter->vt_ctxt.num_vfs ||
		...
		for (i = 0; i < adapter->rx_ring_ctxt.num; i++) {
			hw->dma.ops->rx_drop_switch(
				hw, adapter->rx_ring_ctxt.ring[i]->reg_idx,
				true);
		}
	} else {
		for (i = 0; i < adapter->rx_ring_ctxt.num; i++) {
