...
bpf_warn_invalid_xdp_action_compat(__maybe_unused struct net_device *dev,
				   __maybe_unused struct bpf_prog *prog,
				   u32 act)
{
...
static __always_inline bool __sxe_zc_rx_buffers_alloc(
	...
	bool alloc(struct sxe_ring *rx_ring, struct sxe_rx_buffer *bi))
#endif
{
...
#endif
	LOG_DEBUG_BDF(
		"entry xdp zc rx irq clean:irq=%u, ring_idx=%u, ring_reg_idx=%u, "
...
#endif
		if (unlikely(
			    !sxe_status_err_check(rx_desc, SXE_RXD_STAT_EOP))) {
...
#endif
{
	...
	if (!reuseq) {
		LOG_ERROR_BDF(
			"xdp xsk umem fill queue reuse in queue 0 is NULL\n");
		ret = -ENOMEM;
		goto l_ret;
	}
...
#endif
			LOG_ERROR_BDF(
				"async xmit in queue id[%u] error,ret=%d\n",
				qid, ret);
		}
	}
