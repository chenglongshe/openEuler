...
{
	hw->dma.ops->dcb_rx_bw_alloc_configure(
		hw, refill, max, bwg_id, prio_type, prio_tc, MAX_USER_PRIORITY);
	hw->dma.ops->dcb_tx_desc_bw_alloc_configure(hw, refill, max, bwg_id,
						    prio_type);
	hw->dma.ops->dcb_tx_data_bw_alloc_configure(
		hw, refill, max, bwg_id, prio_type, prio_tc, MAX_USER_PRIORITY);
...
void sxe_dcb_configure(struct sxe_adapter *adapter)
{
	...
	if (adapter->dcb_ctxt.dcbx_cap & DCB_CAP_DCBX_VER_CEE) {
		...
		sxe_dcb_hw_ieee_ets_configure(
			&adapter->hw, adapter->dcb_ctxt.ieee_ets, max_frame);
	}
