...
void sxe_log_string(debug_level_e level, const char *dev_name, const char *file,
...
#define DATA_DUMP(ptr, size, str)                                              \
	sxe_log_binary(__FILE__, __FUNCTION__, __LINE__, (u8 *)ptr, 0, size,   \
		       str)
...
#define WRITE_LOG(level, bdf, fmt, ...)                                        \
	sxe_log_string(level, bdf, __FILE__, __FUNCTION__, __LINE__, fmt,      \
		       ##__VA_ARGS__)
...
#define LOG_DEBUG_BDF(fmt, ...)                                                \
	WRITE_LOG(LEVEL_DEBUG, adapter->dev_name, fmt, ##__VA_ARGS__)
#define LOG_INFO_BDF(fmt, ...)                                                 \
	WRITE_LOG(LEVEL_INFO, adapter->dev_name, fmt, ##__VA_ARGS__)
#define LOG_WARN_BDF(fmt, ...)                                                 \
	WRITE_LOG(LEVEL_WARN, adapter->dev_name, fmt, ##__VA_ARGS__)
#define LOG_ERROR_BDF(fmt, ...)                                                \
	WRITE_LOG(LEVEL_ERROR, adapter->dev_name, fmt, ##__VA_ARGS__)
...
#define LOG_PR_INFO(fmt, ...) sxe_print(PRINT_INFO, "sxe", fmt, ##__VA_ARGS__);

#define LOG_PR_WARN(fmt, ...) sxe_print(PRINT_WARN, "sxe", fmt, ##__VA_ARGS__);

#define LOG_PR_ERR(fmt, ...) sxe_print(PRINT_ERR, "sxe", fmt, ##__VA_ARGS__);
...
#define SXE_BUG()
#define SXE_BUG_ON_NO_SYNC(cond)                                               \
	do {                                                                   \
		if ((cond)) {                                                  \
			printk(KERN_ERR                                        \
			       "BUG_ON's condition(%s) has been triggered\n",  \
