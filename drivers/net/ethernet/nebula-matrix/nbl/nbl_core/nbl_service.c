				       ...
				       struct net_device *netdev)
{
	...
	for (; skb; skb = nbl_serv_rep_dequeue(rep_queue_mgt)) {
		...
		if (ret == NETDEV_TX_BUSY) {
			...
			/* nerver hang in sirq too long, so if a tx_busy is returned, drop it */
...
static void nbl_serv_set_netdev_ops(void *priv, struct net_device_ops *net_device_ops, bool is_pf)
{
	...
	if (is_pf)
		net_resource_mgt->netdev_ops.pf_netdev_ops = net_device_ops;
	else
		net_resource_mgt->netdev_ops.rep_netdev_ops = net_device_ops;
}
...
static struct file_operations st_ops = {
	.owner = THIS_MODULE,
				  ...
				  u16 len, u32 version)
{
	...
	case 4:
		...
		break;
	default:
		nbl_err(common, NBL_DEBUG_CUSTOMIZED_P4, "Unkown P4 type %d", type);
		return;
	}
...
{
	struct nbl_dispatch_ops *disp_ops = NBL_SERV_MGT_TO_DISP_OPS(serv_mgt);
	const Elf32_Ehdr *elf_hdr = (Elf32_Ehdr *)fw->data;
	struct nbl_common_info *common = NBL_SERV_MGT_TO_COMMON(serv_mgt);
	Elf32_Shdr *shdr;
	...
	if (nbl_serv_calculate_md5sum(common, fw->data, fw->size, md5_result)) {
		nbl_warn(common, NBL_DEBUG_CUSTOMIZED_P4,
			 "elf md5sum caculate failed, load defalut p4 configuration");
		return 0;
	}
	...
	shdr = (Elf32_Shdr *)((u8 *)elf_hdr + elf_hdr->e_shoff);
...
static struct kobj_type nbl_kobj_vf_type = {
