				    ...
				    const struct ethtool_link_ksettings *cmd)
{
	...
	if (hw->phy.media_type == txgbe_media_type_copper || hw->phy.multispeed_fiber) {
		...
		if (hw->mac.type == txgbe_mac_aml || hw->mac.type == txgbe_mac_aml40) {
			return -EINVAL;
		} else if ((ethtool_link_ksettings_test_link_mode(cmd,
											advertising, 1000baseT_Full) ||
			    ethtool_link_ksettings_test_link_mode(cmd,
			...
			if (ethtool_link_ksettings_test_link_mode(cmd,
								  advertising, 1000baseT_Full)) {
				ethtool_link_ksettings_add_link_mode(&temp_ks,
								     supported, 1000baseT_Full);
				ethtool_link_ksettings_del_link_mode(&temp_ks,
		...
		if (adapter->flags & TXGBE_FLAG_SRIOV_ENABLED) {
			...
			for (i = 0; i < adapter->num_vfs; i++) {
				if (adapter->vfinfo[i].clear_to_send) {
					e_warn(drv, "Please take active VFS offline and restart diagnostics\n");
					data[0] = 1;
					data[1] = 1;
					data[2] = 1;
					data[3] = 1;
			     ...
			     enum ethtool_phys_id_state state)
{
	...
	case ETHTOOL_ID_ON:
		if ((hw->oem_ssid != 0x0085 && hw->oem_svid == 0x1bd4) ||
			...
			if (adapter->link_up) {
				...
				case TXGBE_LINK_SPEED_10GB_FULL:
					hw->mac.ops.led_on(hw, TXGBE_LED_LINK_10G);
					break;
				case TXGBE_LINK_SPEED_1GB_FULL:
					hw->mac.ops.led_on(hw, TXGBE_LED_LINK_1G);
					break;
				case TXGBE_LINK_SPEED_100_FULL:
					hw->mac.ops.led_on(hw, TXGBE_LED_LINK_100M);
					break;
				default:
					break;
				}
			...
			hw->mac.ops.led_on(hw, TXGBE_LED_LINK_UP);
		}
		break;
	...
	case ETHTOOL_ID_OFF:
		if ((hw->oem_ssid != 0x0085 && hw->oem_svid == 0x1bd4) ||
			...
			if (adapter->link_up) {
				switch (adapter->link_speed) {
				case TXGBE_LINK_SPEED_10GB_FULL:
					hw->mac.ops.led_off(hw, TXGBE_LED_LINK_10G);
					break;
				case TXGBE_LINK_SPEED_1GB_FULL:
					hw->mac.ops.led_off(hw, TXGBE_LED_LINK_1G);
					break;
				case TXGBE_LINK_SPEED_100_FULL:
					hw->mac.ops.led_off(hw, TXGBE_LED_LINK_100M);
					break;
				default:
					break;
				}
	...
	case ETHTOOL_ID_INACTIVE:
		...
		wr32(&adapter->hw, TXGBE_CFG_LED_CTL,
		     adapter->led_reg);
		if ((hw->subsystem_device_id & 0xF0) == TXGBE_ID_XAUI) {
			txgbe_read_mdio(&hw->phy_dev, hw->phy.addr,
					31, 0xF021, &value);
			txgbe_write_mdio(&hw->phy_dev, hw->phy.addr,
					 31, 0xF021, (value & 0xFFFC) | 0x0);
		}
		break;
	}
				  ...
				  struct ethtool_rxnfc *nfc)
{
	...
	if (flags2 != adapter->flags2) {
		...
		if ((flags2 & UDP_RSS_FLAGS) &&
			...
			e_warn(drv, "enabling UDP RSS: fragmented packets may out of order\n");
			      ...
			      struct ethtool_channels *ch)
{
	...
	if (count < adapter->active_vlan_limited + 1) {
		e_dev_info("cannot set less than active limited vlan + 1:%d",
			   (adapter->active_vlan_limited + 1));
		return -EINVAL;
	}
	...
	if (hw->mac.type != txgbe_mac_sp) {
		...
		case TXGBE_SFF_IDENTIFIER_SFP:
			...
			if (addr_mode & TXGBE_SFF_ADDRESSING_MODE) {
				e_err(drv, "Address change required to access page 0xA2, but not supported.\n");
				page_swap = true;
			}
		...
		for (i = 0; i < ee->len; i++) {
			...
			if (is_sfp) {
				if (offset < ETH_MODULE_SFF_8079_LEN)
					status = hw->phy.ops.read_i2c_eeprom(hw, offset,
								&databyte);
				else
					status = hw->phy.ops.read_i2c_sff8472(hw, offset,
								&databyte);
				...
				if (page == 0 || !(data[0x2] & 0x4)) {
					status = hw->phy.ops.read_i2c_sff8636(hw, page, offset,
								&databyte);
