...
{
	unsigned int port_reg_offset;
	u32 data;

	/* Set the LAN port indicator to port_reg_offset[1] */
	/* 1st, write the regOffset to IDA_ADDR register */
	port_reg_offset = TXGBE_ETHPHY_IDA_ADDR;
	wr32(hw, port_reg_offset, addr);

	/* 2nd, read the data from IDA_DATA register */
	port_reg_offset = TXGBE_ETHPHY_IDA_DATA;
	data = rd32(hw, port_reg_offset);
	return data;
}
...
u32 txgbe_rd32_epcs(struct txgbe_hw *hw, u32 addr)
{
	...
	/* Set the LAN port indicator to port_reg_offset[1] */
	/* 1st, write the regOffset to IDA_ADDR register */
	port_reg_offset = TXGBE_XPCS_IDA_ADDR;
	wr32(hw, port_reg_offset, addr);

	/* 2nd, read the data from IDA_DATA register */
	port_reg_offset = TXGBE_XPCS_IDA_DATA;
	data = rd32(hw, port_reg_offset);
...
static s32 txgbe_get_eeprom_semaphore(struct txgbe_hw *hw)
{
	...
	if (status == 0) {
		...
		ERROR_REPORT1(TXGBE_ERROR_POLLING,
			      "Software semaphore SMBI is not granted, MNG_SW_SM_SM is 0x%08x.\n",
				swsm);
	}
				      ...
				      bool clear)
{
	...
	/* Set the new number of MC addresses that we
	 * are being requested to use.
	 */
...
{
	hw->eeprom.ops.read(hw, hw->eeprom.sw_region_offset + TXGBE_DEVICE_CAPS, device_caps);
				 ...
				 u32 length, u32 timeout, bool return_data)
{
	...
	if (hw->mac.type == txgbe_mac_sp)
		if (hw->mac.ops.acquire_swfw_sync(hw, TXGBE_MNG_SWFW_SYNC_SW_MB) != 0)
			return TXGBE_ERR_SWFW_SYNC;
			...
			const u8 *data, u32 size)
{
	...
	if (sn_is_str) {
		...
		sn_str_len = 0x12;
		sprintf(sn_str, "%02x%08x%08x",
			(serial_num_dword2_t & 0xff),
			serial_num_dword1_t, serial_num_dword0_t);
	}
	...
	for (i = 0; i < size / 4; i++) {
		read_data = data[4 * i + 3] << 24 |
					data[4 * i + 2] << 16 |
					data[4 * i + 1] << 8 | data[4 * i];
		read_data = __le32_to_cpu(read_data);
		skip = ((i * 4 == mac_addr0_dword0_addr) ||
				(i * 4 == mac_addr0_dword1_addr) ||
				(i * 4 == mac_addr1_dword0_addr) ||
				(i * 4 == mac_addr1_dword1_addr) ||
	...
	for (i = 0; i < 256 / 4; i++) {
		read_data = vpd_tend[4 * i + 3] << 24 |
					vpd_tend[4 * i + 2] << 16 |
					vpd_tend[4 * i + 1] << 8 |
					vpd_tend[4 * i];
		read_data = __le32_to_cpu(read_data);
		if (read_data != U32_MAX) {
			status = txgbe_flash_write_dword(hw,
							 TXGBE_VPD_OFFSET + i * 4, read_data);
			if (status) {
				e_err(drv, "ERROR: Program 0x%08x @addr: 0x%08x is failed !!\n",
				      read_data, i * 4);
				txgbe_flash_read_dword(hw, i * 4, &read_data);
	...
	txgbe_flash_write_dword(hw, mac_addr0_dword0_addr,
				mac_addr0_dword0_t);
	txgbe_flash_write_dword(hw, mac_addr0_dword1_addr,
				(mac_addr0_dword1_t | 0x80000000));//lan0
	txgbe_flash_write_dword(hw, mac_addr1_dword0_addr,
				mac_addr1_dword0_t);
	txgbe_flash_write_dword(hw, mac_addr1_dword1_addr,
				(mac_addr1_dword1_t | 0x80000000));//lan1
	if (sn_is_str) {
		for (i = 0; i < 24; i++)
			txgbe_flash_write_dword(hw,
						PRODUCT_SERIAL_NUM_OFFSET_1G + 4 * i, sn[i]);
	} else {
		txgbe_flash_write_dword(hw, PRODUCT_SERIAL_NUM_OFFSET_1G,
					serial_num_dword0_t);
		txgbe_flash_write_dword(hw, PRODUCT_SERIAL_NUM_OFFSET_1G + 4,
					serial_num_dword1_t);
		txgbe_flash_write_dword(hw, PRODUCT_SERIAL_NUM_OFFSET_1G + 8,
					serial_num_dword2_t);
	}
	...
	if (hw->mac.type == txgbe_mac_aml || hw->mac.type == txgbe_mac_aml40) {
		...
		while (1) {
			...
			if (i++ > PHYINIT_TIMEOUT) {
				e_info(drv, "ERROR: Wait 0x1033c Timeout!!!\n");
				return -1;
			}
		}
...
struct txgbe_dec_ptype txgbe_ptype_lookup[256] = {
	TXGBE_UKN(0x00),
	TXGBE_UKN(0x01),
	TXGBE_UKN(0x02),
	TXGBE_UKN(0x03),
...
s32 txgbe_init_phy_ops_sp(struct txgbe_hw *hw)
{
	...
	if (hw->mac.ops.get_media_type(hw) == txgbe_media_type_copper) {
				      ...
				      bool *autoneg)
{
	...
	/* SFP */
	else if (hw->mac.ops.get_media_type(hw) == txgbe_media_type_fiber) {
		*speed = TXGBE_LINK_SPEED_10GB_FULL;
		*autoneg = false;
	}
	/* XAUI */
	else if ((hw->mac.ops.get_media_type(hw) == txgbe_media_type_copper) &&
...
void txgbe_disable_tx_laser_multispeed_fiber(struct txgbe_hw *hw)
{
	...
	if (!((hw->mac.ops.get_media_type(hw) == txgbe_media_type_fiber) ||
	      (hw->mac.ops.get_media_type(hw) == txgbe_media_type_fiber_qsfp)))
		return;
	...
	if (txgbe_close_notify(hw)) {
		...
		if (hw->mac.type == txgbe_mac_aml40)
			hw->mac.ops.led_off(hw, TXGBE_LED_LINK_UP | TXGBE_AMLITE_LED_LINK_40G |
...
s32 txgbe_set_sgmii_an37_ability(struct txgbe_hw *hw)
{
	...
	if ((hw->subsystem_device_id & 0xF0) == TXGBE_ID_MAC_SGMII ||
	    hw->mac.ops.get_media_type(hw) == txgbe_media_type_fiber)
...
s32 txgbe_set_link_to_kx4(struct txgbe_hw *hw, bool autoneg)
{
	...
	if (adapter->ffe_set == TXGBE_BP_M_KX4) {
			       ...
			       bool autoneg)
{
	...
	if (adapter->ffe_set == TXGBE_BP_M_KX) {
				 ...
				 u32 speed)
{
	...
	if (adapter->ffe_set == TXGBE_BP_M_SFI) {
			       ...
			       bool autoneg_wait_to_complete)
{
	...
	if ((hw->subsystem_device_id & TXGBE_DEV_MASK) == TXGBE_ID_KR_KX_KX4 ||
		...
		((hw->subsystem_device_id & TXGBE_DEV_MASK) == TXGBE_ID_MAC_SGMII) ||
		(hw->mac.ops.get_media_type(hw) == txgbe_media_type_copper &&
					 ...
					 bool autoneg_wait_to_complete)
{
	...
	link_speed = hw->phy.ops.setup_link_speed(hw, speed,
						autoneg_wait_to_complete);
...
static int txgbe_reset_misc(struct txgbe_hw *hw)
{
	...
	if (hw->mac.type == txgbe_mac_aml40) {
		if (!(rd32(hw, TXGBE_EPHY_STAT) & TXGBE_EPHY_STAT_PPL_LOCK)) {
			err = hw->mac.ops.setup_link(hw, TXGBE_LINK_SPEED_40GB_FULL, false);
			if (err) {
				e_dev_info("%s setup phy failed\n", __func__);
				return err;
			}
		}
	} else if (hw->mac.type == txgbe_mac_aml) {
		if ((rd32(hw, TXGBE_EPHY_STAT) & TXGBE_EPHY_STAT_PPL_LOCK)
			...
			err = hw->mac.ops.setup_link(hw, TXGBE_LINK_SPEED_AMLITE_AUTONEG, false);
			if (err) {
				e_dev_info("%s setup phy failed\n", __func__);
				return err;
			}
				       ...
				       bool cloud_mode)
{
	...
	/* store source and destination IP masks (little-endian) */
...
s32 txgbe_identify_phy(struct txgbe_hw *hw)
{
	...
	media_type = hw->mac.ops.get_media_type(hw);
...
s32 txgbe_init_eeprom_params(struct txgbe_hw *hw)
{
	...
	status = hw->eeprom.ops.read(hw, TXGBE_SW_REGION_PTR,
			&data);
	if (status)
		return status;
	...
	if (hw->phy.sfp_type == txgbe_sfp_type_1g_cu_core0 ||
		...
		*link_up = hw->f2c_mod_status;
		...
		if (*link_up)
			...
			hw->mac.ops.led_off(hw, TXGBE_LED_LINK_UP | TXGBE_LED_LINK_10G |
