...
u32 txgbe_mtd_hw_set_phy_reg_feild(struct mtd_dev *dev_ptr,
				   u16 port,
									u16 dev,
									u16 reg_addr,
									u8 field_offset,
									...
									u16 data)
{
	...
	txgbe_mtd_set_reg_field_word(tmp_data, data, field_offset, field_length, &new_data);
									...
									bool *is_e20x0_dev)
{
	...
	*is_e20x0_dev = false;
	...
	if (major == 0 && minor == 0 && inc == 0 && test == 0) {
		...
		txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port, 31, MTD_REG_SCR, &reg2));
		txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port, 31, MTD_REG_ECSR, &reg3));

		/* clear these bit indications */
		for (index = 0; index < 8; index++)
			bit16thru23[index] = 0;

		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_CCCR9, 0x0300));
		txgbe_mtd_wait(1);
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_SCR, 0x0102));
		txgbe_mtd_wait(1);

		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x06D3));
		txgbe_mtd_wait(1);
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0593));
		txgbe_mtd_wait(1);
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0513));
		txgbe_mtd_wait(1);

		index = 0;
		index2 = 0;
		while (index < 24) {
			txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0413));
			txgbe_mtd_wait(1);
			txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0513));
			txgbe_mtd_wait(1);
		...
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0413));
		txgbe_mtd_wait(1);
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0493));
		txgbe_mtd_wait(1);
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0413));
		txgbe_mtd_wait(1);
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, 0x0513));
		...
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_CCCR9, 0x5440));
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_SCR, reg2));
		txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 31, MTD_REG_ECSR, reg3));

	} else {
		/* should just read it from the firmware status register */
		txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port,
						    MTD_T_UNIT_PMA_PMD, MTD_TUNIT_XG_EXT_STATUS, &abilities));
		if (abilities & (1 << 12))
			*phy_has_macsec = false;
...
static u32 txgbe_mtd_phy_ready_after_rst(struct mtd_dev *dev_ptr,
					 u16 port,
											bool *phy_ready)
{
	u16 val;
						...
						u16 timeout)
{
	...
	txgbe_attempt(txgbe_mtd_hw_set_phy_reg_feild(dev_ptr, port,
						     MTD_T_UNIT_PMA_PMD, MTD_TUNIT_IEEE_PMA_CTRL1,
		15, 1, 1));

	if (timeout) {
		counter = 0;
		txgbe_attempt(txgbe_mtd_phy_ready_after_rst(dev_ptr, port, &phy_ready));
...
static u32 txgbe_mtd_phy_ready_after_hw_rst(struct mtd_dev *dev_ptr,
					    u16 port,
											bool *phy_ready)
{
	u16 val;
						...
						u16 timeout)
{
	...
	if (timeout) {
		counter = 0;
		txgbe_attempt(txgbe_mtd_phy_ready_after_hw_rst(dev_ptr, port, &phy_ready));
		while (!phy_ready && counter <= timeout) {
			txgbe_attempt(txgbe_mtd_wait(1));
			txgbe_attempt(txgbe_mtd_phy_ready_after_hw_rst(dev_ptr, port, &phy_ready));
			counter++;
					...
					bool an_restart)
{
	...
	if (speed_bits == MTD_ADV_NONE) {
		...
		txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port, 7, 0x0020, &temp_reg_value));
...
u32 txgbe_mtd_autoneg_restart(struct mtd_dev *dev_ptr, u16 port)
{
	...
	return (txgbe_mtd_hw_set_phy_reg_feild(dev_ptr, port, 7, 0,
					       9, 1, 1));
}
...
u32 txgbe_mtd_autoneg_speed_dulpex_res_done(struct mtd_dev *dev_ptr,
					    u16 port,
											 bool *an_speed_res_done)
{
	u16 val;
...
u32 txgbe_mtd_get_autoneg_speed_duplex_res(struct mtd_dev *dev_ptr,
					   u16 port,
											u16 *speed_resolution)
{
	...
	*speed_resolution = MTD_ADV_NONE;
	...
	if (val) {
		...
		txgbe_attempt(txgbe_mtd_autoneg_speed_dulpex_res_done(dev_ptr, port, &res_done));
							...
							bool *link_up)
{
	...
	*speed = MTD_ADV_NONE;
	...
	if (speed_is_forced) {
		...
		txgbe_attempt(txgbe_mtd_get_autoneg_speed_duplex_res(dev_ptr, port, speed));
...
static u32 txgbe_mtd_autoneg_is_completed(struct mtd_dev *dev_ptr,
					  u16 port,
									...
									u8 *pause_bits)
{
	...
	txgbe_attempt(txgbe_mtd_autoneg_is_completed(dev_ptr, port, &an_status_ready));
	...
	*this_port = 0;
	...
	if (major == 0 && minor == 0 && inc == 0 && test == 0) {
		...
		if (MTD_IS_X32X0_BASE(base_type)) {
			...
			if (reported_hw_rev == 1) {
				...
				while (!reg_ready && try_counter++ < 10) {
					txgbe_attempt(txgbe_mtd_wait(1)); /* timeout is set to 10 ms */
					txgbe_attempt(txgbe_mtd_get_phy_reg_filed(dev_ptr, port,
										  3, 0xD007, 6,
						1, &ready_bit));
					if (ready_bit == 1)
				...
				txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port, 3, 0x8EC6, &temp));
				txgbe_attempt(txgbe_mtd_xmdio_wr(dev_ptr, port, 3, 0x8EC6, 0xA5A5));
				txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port, 3, 0x8EC6, &temp2));
		...
		*this_port = (u8)thisport;
		...
		txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port,
						    MTD_T_UNIT_PMA_PMD, MTD_TUNIT_PHY_REV_INFO_REG, &temp));
...
{
	return txgbe_mtd_hw_set_phy_reg_feild(dev_ptr, port,
		MTD_C_UNIT_GENERAL, MTD_CUNIT_PORT_CTRL,
		15, 1, 1);
}
...
static u32 txgbe_mtd_rerun_serdes_autoneg_init_automode(struct mtd_dev *dev_ptr, u16 port)
{
	...
	txgbe_attempt(txgbe_mtd_set_reg_field_word(temp,
						   3, 14, 2, &temp2));  /* execute bits and disable bits set */
									...
									bool rerun_serdes_init)
{
	...
	txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port,
					    MTD_C_UNIT_GENERAL, MTD_CUNIT_PORT_CTRL, &cunit_port_ctrl));
	txgbe_attempt(txgbe_mtd_hw_xmdio_rd(dev_ptr, port,
					    MTD_C_UNIT_GENERAL, MTD_CUNIT_MODE_CONFIG, &cunit_mode_cfg));
	...
	if (mac_type < MTD_MAC_LEAVE_UNCHANGED)
		txgbe_attempt(txgbe_mtd_set_reg_field_word(cunit_port_ctrl,
							   mac_type, 0, 3, &cunit_port_ctrl));

	txgbe_attempt(txgbe_mtd_set_reg_field_word(cunit_mode_cfg,
						   (u16)mac_powerdown, 3, 1, &cunit_mode_cfg));

	if (mac_snoop_sel < MTD_MAC_SNOOP_LEAVE_UNCHANGED)
		txgbe_attempt(txgbe_mtd_set_reg_field_word(cunit_mode_cfg,
							   mac_snoop_sel, 8, 2, &cunit_mode_cfg));
