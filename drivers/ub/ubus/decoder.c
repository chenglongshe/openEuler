...
static u32 set_mmio_base_reg(struct ub_decoder *decoder)
{
	...
	if (ret)
		ub_err(decoder->uent, "set decoder mmio base fail\n");
...
static u32 set_page_table_reg(struct ub_decoder *decoder)
{
	...
	if (ret)
		ub_err(decoder->uent, "set decoder page table reg fail\n");
...
static u32 set_queue_reg(struct ub_decoder *decoder)
{
	...
	if (ret)
		ub_err(decoder->uent, "set decoder queue fail\n");
...
static u32 set_decoder_enable(struct ub_decoder *decoder)
{
	...
	if (ret)
		ub_err(decoder->uent, "set decoder enable fail\n");
...
static int ub_create_decoder(struct ub_bus_controller *ubc)
{
	...
	if (ret) {
		ub_err(uent, "decoder create page table fail\n");
		goto release_queue;
	}
...
static void unset_mmio_base_reg(struct ub_decoder *decoder)
{
	...
	if (ret)
		ub_err(uent, "unset mmio base reg fail\n");
}
...
static void unset_page_table_reg(struct ub_decoder *decoder)
{
	...
	if (ret)
		ub_err(uent, "unset page table reg fail\n");
}
...
void ub_init_decoder_usi(struct ub_entity *uent)
{
	...
	if (ret) {
		ub_err(uent, "get decoder usi idx fail\n");
		return;
	}

	irq_num = ub_irq_vector(uent, usi_idx);
	if (irq_num < 0) {
		ub_err(uent, "ub get irq vector fail, ret=%d\n", irq_num);
		return;
	}
	...
	if (ret)
		ub_err(uent, "decoder request_irq fail, ret=%d\n", ret);
	else
