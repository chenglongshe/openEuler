					       ...
					       ub_ers_result_t orig)
{
	...
	udrv = uent->driver;
	if (!udrv || !udrv->err_handler || !udrv->err_handler->resource_enabled) {
		result = orig;
		goto out;
	}

	new_one = udrv->err_handler->resource_enabled(uent);
...
static ub_ers_result_t ub_error_report(struct ub_entity *uent, ub_channel_state_t state)
{
	...
	device_lock(&uent->dev);
	udrv = uent->driver;
	if (!udrv || !udrv->err_handler || !udrv->err_handler->error_detected) {
		ub_warn(uent, "No driver bounded or no handler provided.\n");
		result = UB_ERS_RESULT_NO_ERR_DRIVER;
		goto exit;
	}

	result = udrv->err_handler->error_detected(uent, state);
