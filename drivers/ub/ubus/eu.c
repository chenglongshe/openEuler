...
static int ub_eu_table_init_common(struct ub_entity *uent)
{
	...
	if (ret) {
		ub_err(uent, "EU TABLE get fail, ret=%d\n", ret);
		return ret;
	}
	if (entries == 0) {
	...
	if (cfg_ret) {
		ret = (int)cfg_ret;
		ub_err(uent, "EU TABLE set fail, ret=%d\n", ret);
		goto dma_free;
	}
...
static void ub_eu_table_uninit_common(struct ub_entity *uent)
{
	...
	if (ret)
		ub_err(uent, "EU TABLE unset fail, ret=%d\n", (int)ret);
...
void ub_eu_table_init(struct ub_entity *uent)
{
	...
	if (ubc->ops && ubc->ops->eu_table_init) {
		...
		if (ret) {
			ub_err(uent, "EU TABLE private init fail, ret=%d\n",
			       ret);
			goto uninit_common;
		}
	}
