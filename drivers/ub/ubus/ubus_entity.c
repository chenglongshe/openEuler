...
static void ub_config_upi(struct ub_entity *uent)
{
	...
	if (is_ibus_controller(uent) && uent->ubc->cluster) {
		...
		if (ret) {
			dev_err(dev, "update cluster upi fail, ret=%d\n", ret);
			return;
		}
...
static void ub_res_space_prepare(struct ub_entity *pue)
{
	...
	if (ub_cfg_read_dword(pue, UB_CFG1_SUPPORT_FEATURE_L, &support_feature)) {
		ub_err(pue, "read cfg1 support feature fail\n");
		return;
	}
...
static int ub_setup_ent_primary(struct ub_entity *uent)
{
	...
	if (ret) {
		ub_err(uent, "get UB_TOTAL_NUMBER_ENTITIES fail, ret=%d\n", ret);
		return ret;
	}
...
static void ub_get_module_id(struct ub_entity *uent)
{
	...
	if (ret) {
		ub_err(uent, "Get dev module fail %d\n", ret);
		return;
	}
...
int ub_setup_ent(struct ub_entity *uent)
{
	...
	if (ret) {
		ub_err(uent, "probe message fail, ret=%d\n", ret);
		return ret;
	}

	ret = ub_get_guid(uent);
	if (ret) {
		ub_err(uent, "get guid fail, ret=%d\n", ret);
		goto err_alloc;
	}
	...
	if (ret) {
		ub_err(uent, "alloc eid fail, ret=%d\n", ret);
		goto err_alloc;
	}

	uent_num = ub_entity_num_alloc();
	if (uent_num < 0) {
		ub_err(uent, "alloc dev uent_num fail, ret=%d\n", uent_num);
		goto free_eid;
	}
...
void ub_entity_enable(struct ub_entity *uent, u8 enable)
{
	...
	ub_info(uent, "modify active status to %u\n", enable);
