...
static int uburma_get_jfs_tjfr_objs(struct uburma_file *file,
				    uint64_t jetty_handle,
				    uint64_t tjetty_handle,
				    struct uburma_uobj **jetty_uobj,
				    struct uburma_uobj **tjetty_uobj)
{
	*jetty_uobj = uobj_get_read(UOBJ_CLASS_JFS, jetty_handle, file);
	if (IS_ERR_OR_NULL(*jetty_uobj)) {
		uburma_log_err("failed to find jfs with handle %llu",
			       jetty_handle);
		return -EINVAL;
	}

	*tjetty_uobj =
		uobj_get_read(UOBJ_CLASS_TARGET_JFR, tjetty_handle, file);
	if (IS_ERR_OR_NULL(*tjetty_uobj)) {
		uobj_put_read(*jetty_uobj);
		uburma_log_err("failed to find target jfr with handle %llu",
			       tjetty_handle);
		return -EINVAL;
	}
	return 0;
...
static int uburma_cmd_advise_jfr(struct ubcore_device *ubc_dev,
				 struct uburma_file *file,
				 struct uburma_cmd_hdr *hdr)
{
	...
	if (uburma_get_jfs_tjfr_objs(file, arg.in.jetty_handle,
				     arg.in.tjetty_handle, &jfs_uobj,
				     &tjfr_uobj))
		return -EINVAL;
...
static int uburma_cmd_unadvise_jfr(struct ubcore_device *ubc_dev,
				   struct uburma_file *file,
				   struct uburma_cmd_hdr *hdr)
{
...
static int uburma_cmd_advise_jetty(struct ubcore_device *ubc_dev,
				   struct uburma_file *file,
				   struct uburma_cmd_hdr *hdr)
{
	struct uburma_cmd_advise_jetty arg;
	...
	ret = ubcore_advise_jetty(jetty_uobj->object, tjetty_uobj->object,
				  &udata);
	if (ret != 0)
					...
					struct uburma_cmd_hdr *hdr)
{
	...
	if (ubc_dev->sip_table.entry == NULL) {
				 ...
				 struct uburma_cmd_modify_tp *arg)
{
	...
	if (ubc_dev == NULL || ubc_dev->ops == NULL ||
	    ubc_dev->ops->modify_user_tp == NULL) {
