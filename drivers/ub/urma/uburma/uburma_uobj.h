...
struct uobj_idr_type {
	...
	int __must_check (*destroy_func)(struct uburma_uobj *uobj,
					 enum uburma_remove_reason why);
};

struct uobj_idr_ex_type {
	struct uobj_type type;
	int __must_check (*destroy_func)(struct uburma_uobj *uobj,
					 enum uburma_remove_reason why);
	int __must_check (*destroy_batch_func)(struct uburma_uobj **uobj,
					       int arr_num, int *bad_index,
					       enum uburma_remove_reason why);
};
...
struct uobj_fd_type {
	...
	int flags;
	int (*context_closed)(struct uburma_uobj *uobj,
			      enum uburma_remove_reason why);
};
...
struct uburma_jfc_uobj_array {
	struct uburma_jfc_uobj *uobj_arr;
	int size;
};
...
#define _uobj_class_set(_id, _type_attrs)            \
	((const struct uobj_class_def){ .id = (_id), \
...
#define uobj_type_alloc_idr_ex(_size, _order, _destroy_func, \
			       _destroy_batch_func)          \
	((&((const struct uobj_idr_ex_type) {                          \
			.type = {                                      \
				.type_class = &uobj_idr_type_class,        \
				.obj_size = (_size),                       \
				.destroy_order = (_order),                 \
			...
			.destroy_func = (_destroy_func),               \
			.destroy_batch_func = (_destroy_batch_func),  \
		}))->type)

#define uobj_type_alloc_fd(_order, _obj_size, _context_closed, _fops, _name, \
			   _flags)                                           \
...
#define uobj_alloc(class_id, ufile) \
	uobj_alloc_begin(uobj_get_type(class_id), ufile, class_id)
...
#define uobj_put_del_batch(uobj_arr, arr_num) uobj_put_batch(uobj_arr, arr_num)
...
extern const struct file_operations uburma_notifier_fops;
