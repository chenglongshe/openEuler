...
struct ubcore_ctp *ubcore_create_ctp(struct ubcore_device *dev,
				     struct ubcore_ctp_cfg *cfg)
{
	...
	ret = ubcore_hash_table_find_add(&dev->ht[UBCORE_HT_CTP], &ctp->hnode,
					 ctp->ctpn);
	if (ret != 0) {
...
{
	struct ubcore_ctp *ctp =
		container_of(ref_cnt, struct ubcore_ctp, ref_cnt);
...
int ubcore_destroy_ctp(struct ubcore_ctp *ctp)
{
	...
	if (ret != 0) {
		(void)ubcore_hash_table_find_add(&dev->ht[UBCORE_HT_CTP],
						 &ctp->hnode, ctp->ctpn);
		/* inc ctp use cnt? */
...
struct ubcore_ctp *ubcore_find_remove_ctp(struct ubcore_device *dev,
					  uint32_t idx)
{
	struct ubcore_ctp *ctp;
	...
	ctp = ubcore_hash_table_lookup_nolock(&dev->ht[UBCORE_HT_CTP], idx,
					      &idx);
	if (ctp == NULL) {
		spin_unlock(&dev->ht[UBCORE_HT_CTP].lock);
		return NULL;
