...
static void ubcore_utp_kref_release(struct kref *ref_cnt)
{
	struct ubcore_utp *utp =
		container_of(ref_cnt, struct ubcore_utp, ref_cnt);
...
static int ubcore_find_add_utp(struct ubcore_hash_table *ht,
			       struct ubcore_utp *utp)
{
...
struct ubcore_utp *ubcore_create_utp(struct ubcore_device *dev,
...
void ubcore_find_remove_utp(struct ubcore_device *dev, uint32_t idx)
{
	...
	utp = ubcore_hash_table_lookup_nolock(&dev->ht[UBCORE_HT_UTP], idx,
					      &idx);
	if (utp == NULL) {
		spin_unlock(&dev->ht[UBCORE_HT_UTP].lock);
		return;
