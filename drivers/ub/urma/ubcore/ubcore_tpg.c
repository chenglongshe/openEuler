...
struct ubcore_tpg *ubcore_create_tpg(struct ubcore_device *dev,
...
void ubcore_tpg_get(void *obj)
{
	...
	if (!tpg)
		return;
	ubcore_log_debug("get tpg, tpgn:%u, in dev:%s,  refcnt to: %u",
			 tpg->tpgn, tpg->ub_dev->dev_name,
			 kref_read(&tpg->ref_cnt) + 1);
...
{
	struct ubcore_tpg *tpg =
		container_of(ref_cnt, struct ubcore_tpg, ref_cnt);
...
void ubcore_tpg_kref_put(struct ubcore_tpg *tpg)
{
	...
	ubcore_log_debug("put tpg, tpgn:%u, in dev:%s,  refcnt to: %u",
			 tpg->tpgn, tpg->ub_dev->dev_name,
			 kref_read(&tpg->ref_cnt) - 1);
...
{
	return ubcore_hash_table_lookup_get(&dev->ht[UBCORE_HT_TPG], tpgn,
					    &tpgn);
}
...
int ubcore_find_remove_tpg(struct ubcore_device *dev, uint32_t tpgn)
{
	...
	tpg = ubcore_hash_table_lookup_nolock(&dev->ht[UBCORE_HT_TPG], tpgn,
					      &tpgn);
	if (tpg == NULL) {
		spin_unlock(&dev->ht[UBCORE_HT_TPG].lock);
		return -1;
...
{
	return ubcore_hash_table_find_add(&dev->ht[UBCORE_HT_TP], &tp->hnode,
					  tp->tpn);
}
...
struct ubcore_tp *ubcore_find_remove_tp_node(struct ubcore_device *dev,
					     uint32_t tpn)
{
	struct ubcore_tp *tp;
...
static void ubcore_set_tp_init_flag(union ubcore_tp_flag *flag,
				    union ubcore_tp_cfg_flag in)
{
...
static void ubcore_store_tp_init_cfg(struct ubcore_tpg *tpg,
				     struct ubcore_tp *tp,
			   ...
			   struct ubcore_tp_cfg *cfg)
{
	...
	if (dev->ops == NULL || dev->ops->create_multi_tp == NULL ||
	    ...
	    tp_cnt > UBCORE_MAX_TP_CNT_IN_GRP)
		return -EINVAL;
	...
	for (i = 0; i < tp_cnt; i++) {
		...
		/* tpg pointer is filled in each tp,
		 * so kref of tpg should be increased for each tp
		 */
		ubcore_tpg_kref_get(tpg);
		ubcore_store_tp_init_cfg(tpg, single_tp, dev, &cfg[i]);
		mutex_init(&single_tp->lock);
		tpg->tp_list[i] = single_tp;

		ret = ubcore_add_tp(dev, single_tp);
		if (ret != 0) {
			ubcore_log_err(
				"Failed to add tp:%u to the tp table and tpg:%u, dev:%s",
				single_tp->tpn, tpg->tpgn, dev->dev_name);
			for (j = 0; j < i; j++) {
				(void)ubcore_find_remove_tp_node(dev,
								 tp[j]->tpn);
				/* tp->tpg should not be NULL before udma destroy tp */
				ubcore_tpg_kref_put(tpg);
			}
		...
		ubcore_log_debug(
			"success to add tp:%u to the tp table and tpg:%u, dev:%s",
			single_tp->tpn, tpg->tpgn, dev->dev_name);
	}
...
int ubcore_destroy_multi_tp_from_index(struct ubcore_device *dev,
				       struct ubcore_tpg *tpg,
				       uint32_t from_index)
{
	...
	if (from_index >= tpg->tpg_cfg.tp_cnt) {
		mutex_unlock(&tpg->mutex);
		return 0;
	}

	tp_cnt = tpg->tpg_cfg.tp_cnt - from_index;
	for (i = 0; i < (int)tp_cnt; i++) {
		tp[i] = tpg->tp_list[i + (int)from_index];
		tpg->tp_list[i + (int)from_index] = NULL;
		ubcore_log_debug("tp set null %d", i + (int)from_index);
	}
	tpg->tpg_cfg.tp_cnt = from_index;
	mutex_unlock(&tpg->mutex);

	for (i = 0; i < (int)tp_cnt; i++) {
		ubcore_tp_kref_put(tp[i]);
		wait_for_completion(&tp[i]->comp);
		mutex_destroy(&tp[i]->lock);

		single_tp = ubcore_find_remove_tp_node(dev, tp[i]->tpn);
		if (single_tp == NULL)
			ubcore_log_err(
				"failed to find tp with tpn %u and tpgn %u, dev:%s",
	...
	if (ret != (int)tp_cnt) {
		ubcore_log_err("Failed to destroy multi tp_cnt:%u, ret: %d",
			       tp_cnt, ret);
		return -EINVAL;
	}
...
int ubcore_destroy_multi_tp(struct ubcore_device *dev, struct ubcore_tpg *tpg)
{
	return ubcore_destroy_multi_tp_from_index(dev, tpg, 0);
}

uint32_t ubcore_modify_tp_in_tpg(struct ubcore_device *dev,
				 struct ubcore_tpg *tpg,
				 struct ubcore_tp_attr *attr,
				 union ubcore_tp_attr_mask *mask,
				 struct ubcore_tp **failed_tp)
{
	struct ubcore_tp *tp_list[UBCORE_MAX_TP_CNT_IN_GRP] = {};
	uint32_t tp_cnt = 0;
	int ret;
	int i;
	...
	mutex_lock(&tpg->mutex);
	tp_cnt = tpg->tpg_cfg.tp_cnt;
	if (tp_cnt > UBCORE_MAX_TP_CNT_IN_GRP)
		tp_cnt = UBCORE_MAX_TP_CNT_IN_GRP;
	for (i = 0; i < (int)tp_cnt; i++) {
		tp_list[i] = tpg->tp_list[i];
		ubcore_tp_get(tp_list[i]);
	}
	mutex_unlock(&tpg->mutex);

	ret = dev->ops->modify_multi_tp(tp_cnt, tp_list, attr, mask, failed_tp);
	if (ret != (int)tp_cnt)
		ubcore_log_err("Failed to modify multi tp %d and tpgn %u ", ret,
			       tpg->tpgn);

	for (i = 0; i < ret && i < UBCORE_MAX_TP_CNT_IN_GRP; i++)
		ubcore_modify_tp_attr(tp_list[i], &attr[i], mask[i]);

	for (i = 0; i < (int)tp_cnt; i++)
		ubcore_tp_kref_put(tp_list[i]);

	return (ret > 0 ? (uint32_t)ret : 0);
}

struct ubcore_tp *ubcore_find_get_tp_in_tpg(struct ubcore_tpg *tpg,
					    uint32_t tpn)
{
...
int ubcore_find_add_tpg(struct ubcore_device *dev, struct ubcore_tpg *tpg)
{
	...
	if (ubcore_hash_table_lookup_nolock(ht, tpg->tpgn, &tpg->tpgn) !=
	    NULL) {
		spin_unlock(&ht->lock);
		return -EEXIST;
	}
