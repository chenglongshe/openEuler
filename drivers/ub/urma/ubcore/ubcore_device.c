...
{
	struct sip_idx_node *cur, *nex;

	list_for_each_entry_safe(cur, nex, &uvs->sip_list, node) {
		ubcore_delete_sip(cur->sip_info);
	}
	ubcore_log_info("sip list is empty\n");
}
...
static void ubcore_global_release_file(struct kref *ref)
{
	...
	if (file->uvs != NULL) {
		ubcore_uvs_release_sip_list(file->uvs);
		ubcore_uvs_release_nl_buffer(file->uvs);
		ubcore_uvs_kref_put(file->uvs);
		file->uvs = NULL;
	}
...
static int ubcore_global_close(struct inode *i_node, struct file *filp)
{
	...
	if (file->uvs != NULL) {
		...
		ubcore_log_info(
			"set uvs %s with id %u to dead in ubcore global close\n",
			ins->name, ins->id);
	}
...
int ubcore_find_upi_with_dev_name(const char *dev_name, uint32_t *upi)
{
	...
	list_for_each_entry(entry, &g_upi_list, node) {
		if (entry != NULL &&
		    strcmp(entry->dev->dev_name, dev_name) == 0) {
			*upi = entry->upi;
			ret = 0;
			break;
		}
...
int ubcore_add_upi_list(struct ubcore_device *dev, uint32_t upi)
{
	...
	new_entry = kzalloc(sizeof(struct ubcore_upi_entry), GFP_KERNEL);
	if (new_entry == NULL)
		return -ENOMEM;
...
static bool ubcore_netdev_in_ubdev(struct ubcore_device *dev,
		       ...
		       enum ubcore_transport_type type)
{
	...
	return ubcore_lookup_mue_by_sip_addr(netaddr, type);
}
...
static struct ubcore_ht_param g_ht_params[] = {
	...
	[UBCORE_HT_TP] = { UBCORE_HASH_TABLE_SIZE,
			   offsetof(struct ubcore_tp, hnode),
			   offsetof(struct ubcore_tp, tpn), sizeof(uint32_t),
			   NULL, ubcore_free_driver_obj, ubcore_tp_get },

	[UBCORE_HT_TPG] = { UBCORE_HASH_TABLE_SIZE,
			    offsetof(struct ubcore_tpg, hnode),
			    offsetof(struct ubcore_tpg, tpgn), sizeof(uint32_t),
			    NULL, ubcore_free_driver_obj, ubcore_tpg_get },

	/* key: seid + deid */
	[UBCORE_HT_RM_VTP] = { UBCORE_HASH_TABLE_SIZE,
			       offsetof(struct ubcore_vtp, hnode),
			       offsetof(struct ubcore_vtp, cfg) +
				       offsetof(struct ubcore_vtp_cfg,
						local_eid),
			       sizeof(union ubcore_eid) * 2, NULL,
			       ubcore_free_driver_obj, ubcore_vtp_get },

	/* key: deid + djetty */
	[UBCORE_HT_RC_VTP] = { UBCORE_HASH_TABLE_SIZE,
			       offsetof(struct ubcore_vtp, hnode),
			       offsetof(struct ubcore_vtp, cfg) +
				       offsetof(struct ubcore_vtp_cfg,
						peer_eid),
			       sizeof(union ubcore_eid) + sizeof(uint32_t),
			       NULL, ubcore_free_driver_obj, ubcore_vtp_get },

	/* key: seid + deid */
	[UBCORE_HT_UM_VTP] = { UBCORE_HASH_TABLE_SIZE,
			       offsetof(struct ubcore_vtp, hnode),
			       offsetof(struct ubcore_vtp, cfg) +
				       offsetof(struct ubcore_vtp_cfg,
						local_eid),
	...
	/* key: src_eid + des_eid */
	[UBCORE_HT_RM_VTPN] = { UBCORE_HASH_TABLE_SIZE,
				offsetof(struct ubcore_vtpn, hnode),
				offsetof(struct ubcore_vtpn, local_eid),
				2 * sizeof(union ubcore_eid), NULL,
				ubcore_free_driver_obj, ubcore_vtpn_get },

	/* key: src_eid + des_eid + src_jetty + des_jetty */
	[UBCORE_HT_RC_VTPN] = { UBCORE_HASH_TABLE_SIZE,
				offsetof(struct ubcore_vtpn, hnode),
				offsetof(struct ubcore_vtpn, local_eid),
				2 * sizeof(union ubcore_eid) +
					2 * sizeof(uint32_t),
				NULL, ubcore_free_driver_obj, ubcore_vtpn_get },

	/* key: src_eid + des_eid */
	[UBCORE_HT_UM_VTPN] = { UBCORE_HASH_TABLE_SIZE,
				offsetof(struct ubcore_vtpn, hnode),
				offsetof(struct ubcore_vtpn, local_eid),
				2 * sizeof(union ubcore_eid), NULL,
				ubcore_free_driver_obj, ubcore_vtpn_get },
	...
	/* key: vtpn */
	[UBCORE_HT_VTPN] = { UBCORE_HASH_TABLE_SIZE,
			     offsetof(struct ubcore_vtpn, vtpn_hnode),
			     offsetof(struct ubcore_vtpn, vtpn),
			     sizeof(uint32_t), NULL, ubcore_free_driver_obj,
			     ubcore_vtpn_get },

	/* key: utp idx */
	[UBCORE_HT_UTP] = { UBCORE_HASH_TABLE_SIZE,
			    offsetof(struct ubcore_utp, hnode),
			    offsetof(struct ubcore_utp, utpn), sizeof(uint32_t),
			    NULL, ubcore_free_driver_obj, ubcore_utp_get },

	/* key: ctp idx */
	[UBCORE_HT_CTP] = { UBCORE_HASH_TABLE_SIZE,
			    offsetof(struct ubcore_ctp, hnode),
			    offsetof(struct ubcore_ctp, ctpn), sizeof(uint32_t),
			    NULL, ubcore_free_driver_obj, ubcore_ctp_get },
...
static int ubcore_send_mue_info_to_all_uvs(const struct ubcore_device *dev,
					   struct ubcore_nlmsg *req_msg,
					   const char *mue_action)
{
	...
	for (i = 0; i < count; i++) {
		ret = ubcore_nl_send_nowait_without_cb(req_msg, uvs_list[i]);
		if (ret != 0)
			ubcore_log_err(
				"Failed to send %s mue dev %s info request to uvs %s(id %u), ret:%d\n",
				mue_action, dev->dev_name, uvs_list[i]->name,
				uvs_list[i]->id, ret);
		else {
			success_count++;
			ubcore_log_info(
				"Success to send %s mue dev %s info request to uvs %s(id %u)\n",
				mue_action, dev->dev_name, uvs_list[i]->name,
				uvs_list[i]->id);
		}
	}
	...
	if (count != 0 && success_count == count)
		return 0;
	else
		return -1;
}

static int ubcore_send_remove_mue_dev_info(struct ubcore_device *dev)
{
	...
	req_msg = kcalloc(1,
			  sizeof(struct ubcore_nlmsg) +
				  sizeof(struct ubcore_update_mue_dev_info_req),
			  GFP_KERNEL);
	if (req_msg == NULL)
		return -ENOMEM;
...
static int ubcore_query_send_mue_dev_info(struct ubcore_device *dev)
{
	struct ubcore_nlmsg *req_msg;
	int ret;

	req_msg = ubcore_new_mue_dev_msg(dev);
	if (req_msg == NULL)
		return -1;

	ret = ubcore_send_mue_info_to_all_uvs(dev, req_msg, "add");
	kfree(req_msg);
	return ret;
}
...
static void ubcore_init_net_addr(struct ubcore_device *dev)
{
	if (dev->transport_type == UBCORE_TRANSPORT_UB &&
	    dev->attr.tp_maintainer && ubcore_get_netlink_valid()) {
		if (ubcore_query_send_mue_dev_info(dev) != 0)
			ubcore_log_warn(
	...
	ubcore_update_all_vlan_netaddr(dev, UBCORE_ADD_NET_ADDR);
}
...
static int init_ubcore_device(struct ubcore_device *dev)
{
	...
	if (!dev->attr.virtualization)
		(void)ubcore_add_upi_list(dev, UCBORE_DEFAULT_UPI);
...
static void ubcore_remove_usv_sip_entry_without_lock(struct ubcore_device *dev,
						     uint32_t idx)
{
	if (ubcore_del_net_addr(dev, idx) != 0)
		ubcore_log_err("Failed to delete net addr for dev %s idx %u",
			       dev->dev_name, idx);
	(void)ubcore_del_sip_entry_without_lock(&dev->sip_table, idx);
	(void)ubcore_sip_idx_free_without_lock(&dev->sip_table, idx);
}
...
static void ubcore_remove_uvs_sip_info(struct ubcore_device *dev)
{
	...
	for (i = 0; i < dev->sip_table.max_sip_cnt; i++) {
		sip_info = &dev->sip_table.entry[i].sip_info;
		if (!sip_info->is_active)
			continue;
		if (ubcore_get_netlink_valid() == true)
			(void)ubcore_notify_uvs_del_sip(dev, sip_info, i);

		// remove sip in case that uvs does not respond
		ubcore_remove_usv_sip_entry_without_lock(dev, i);
	}
	mutex_unlock(&dev->sip_table.lock);
}

static void ubcore_uninit_net_addr(struct ubcore_device *dev)
{
	ubcore_free_netdev_port_list(dev);

	ubcore_update_all_vlan_netaddr(dev, UBCORE_DEL_NET_ADDR);
	ubcore_update_netdev_addr(dev, dev->netdev, UBCORE_DEL_NET_ADDR, false);

	if (dev->transport_type == UBCORE_TRANSPORT_UB &&
	    dev->attr.tp_maintainer) {
		ubcore_remove_uvs_sip_info(dev);
		if (ubcore_get_netlink_valid() &&
		    ubcore_send_remove_mue_dev_info(dev) != 0)
			ubcore_log_warn("failed to remove mue dev info %s",
					dev->dev_name);
	}
}
...
static int ubcore_initiate_negotiation(struct ubcore_device *dev)
{
	...
	req_msg =
		kcalloc(1, sizeof(struct ubcore_req) + data_length, GFP_KERNEL);
	if (req_msg == NULL)
		return -ENOMEM;
...
static int ubcore_config_device_in_register(struct ubcore_device *dev)
{
	...
	if (ubcore_get_netlink_valid() == false && !dev->attr.virtualization) {
		ubcore_log_info(
			"UVS is not connected, and use default config. dev: %s.\n",
			dev->dev_name);
	...
	req_msg = kcalloc(1,
			  sizeof(struct ubcore_req) +
				  sizeof(struct ubcore_msg_config_device_req),
			  GFP_KERNEL);
	if (req_msg == NULL)
		return -ENOMEM;
	...
	/* New MUE devices need to be query suspend info. */
	data->is_mue_dev = dev->attr.tp_maintainer;
...
int ubcore_register_device(struct ubcore_device *dev)
{
	...
	ubcore_init_net_addr(dev);

	ret = ubcore_create_main_device(dev);
	if (ret) {
		ubcore_uninit_net_addr(dev);
...
static bool ubcore_preprocess_event(struct ubcore_event *event)
{
	if (event->event_type == UBCORE_EVENT_TP_ERR &&
		...
		if (event->ub_dev->transport_type == UBCORE_TRANSPORT_UB) {
			...
			if (ubcore_change_tp_to_err(event->ub_dev,
						    event->element.tp) != 0)
				ubcore_log_info(
					"ubcore change tp %u to error failed",
					event->element.tp->tpn);
		}
		return true;
	} else if (event->event_type == UBCORE_EVENT_TP_SUSPEND &&
		   event->element.tp != NULL) {
		ubcore_log_info("ubcore detect tp %u suspend event",
				event->element.tp->tpn);
		...
		if (event->ub_dev->transport_type == UBCORE_TRANSPORT_UB)
			ubcore_report_tp_flush_done(event->ub_dev,
						    event->element.tp);
		return true;
	}
	return false;
...
void ubcore_clear_pattern3_eid(struct ubcore_device *dev, union ubcore_eid *eid)
{
	...
	if (dev->attr.virtualization ||
	    ubcore_find_upi_with_dev_name(dev->dev_name, &pattern3_upi) != 0)
		return;
...
static int ubcore_handle_add_sip_idx(struct ubcore_uvs_instance *uvs,
				     struct ubcore_sip_table *sip_table,
				     uint32_t index)
{
	struct sip_idx_node *cur, *tmp, *new_node;

	if (uvs == NULL || uvs->policy != UBCORE_RESTORE_POLICY_CLEANUP) {
		ubcore_log_info("add sip find no uvs\n");
		return 0;
	}

	spin_lock(&uvs->sip_list_lock);
	list_for_each_entry_safe(cur, tmp, &uvs->sip_list, node) {
		if (cur->sip_idx == index) {
			ubcore_log_info("find sip_idx %u in uvs %s\n", index,
					uvs->name);
			spin_unlock(&uvs->sip_list_lock);
			return 0;
		}
	}

	new_node = kzalloc(sizeof(struct sip_idx_node), GFP_KERNEL);
	if (new_node == NULL) {
		spin_unlock(&uvs->sip_list_lock);
		ubcore_log_err("failed to alloc sip_idx_node\n");
		return -ENOMEM;
	}

	new_node->sip_idx = index;
	new_node->sip_info = &sip_table->entry[index].sip_info;
	list_add_tail(&new_node->node, &uvs->sip_list);
	atomic_inc(&sip_table->entry[index].uvs_cnt);
	spin_unlock(&uvs->sip_list_lock);
	ubcore_log_info("add sip_idx %u to uvs %s, uvs_cnt %u\n", index,
			uvs->name,
			atomic_read(&sip_table->entry[index].uvs_cnt));
	return 0;
}
...
static int ubcore_add_device_sip(struct ubcore_sip_info *sip, uint32_t *sip_idx)
{
	...
	uvs = ubcore_find_get_uvs_by_pid(pid);

	dev = ubcore_find_mue_device_by_name(sip->dev_name);
	if (!dev || dev->transport_type != UBCORE_TRANSPORT_UB) {
		ubcore_log_err("update sip, dev:%s no mue, or not UB\n",
			       sip->dev_name);
		return -1;
	}
	...
	if (ubcore_lookup_sip_idx(&dev->sip_table, sip, &index) == 0) {
		...
		if (ubcore_handle_add_sip_idx(uvs, &dev->sip_table, index) !=
		    0) {
			ubcore_log_err("failed to handle sip_idx %u\n", index);
			ret = -1;
			goto put_uvs;
		}
		ret = -EEXIST;
		...
		if (dev->ops != NULL && dev->ops->add_net_addr != NULL) {
			ret = dev->ops->add_net_addr(dev, &sip->addr, index);
			if (ret != 0) {
				ubcore_log_err(
					"Failed to set net addr, ret: %d", ret);
				goto free_sip_index;
			}
		}
		/* add net_addr entry, record idx -> netaddr mapping */
		ret = ubcore_add_sip_entry(&dev->sip_table, sip, index);
		if (ret != 0) {
			ubcore_log_err("Failed to add sip entry, sip_idx %u\n",
				       index);
			goto del_net_addr;
		}
	...
	*sip_idx = index;
	...
	return ret;
del_ubcore_sip_idx:
	ubcore_del_sip_entry(&dev->sip_table, index);
del_net_addr:
	(void)ubcore_del_net_addr(dev, index);
free_sip_index:
...
static int ubcore_handle_del_sip_idx(struct ubcore_uvs_instance *uvs,
				     struct ubcore_sip_table *sip_table,
				     uint32_t index)
{
	struct sip_idx_node *cur, *tmp;
	int ret;

	if (uvs == NULL || uvs->policy != UBCORE_RESTORE_POLICY_CLEANUP) {
		ret = (int)atomic_read(&sip_table->entry[index].uvs_cnt);
		ubcore_log_info("del sip find no uvs, ret %d\n", ret);
		return ret;
	}
	...
	list_for_each_entry_safe(cur, tmp, &uvs->sip_list, node) {
		if (cur->sip_idx == index) {
			list_del(&cur->node);
			kfree(cur);
			ret = (int)atomic_dec_return(
				&sip_table->entry[index].uvs_cnt);
			spin_unlock(&uvs->sip_list_lock);
			ubcore_log_info("del uvs sip_idx %u, uvs %s, ret %d\n",
					index, uvs->name, ret);
			return ret;
		}
	}
	...
	ret = (int)atomic_read(&sip_table->entry[index].uvs_cnt);
	spin_unlock(&uvs->sip_list_lock);
	ubcore_log_info("sip_idx %u not found in uvs %s, ret %d\n", index,
			uvs->name, ret);
	return ret;
}
...
static int ubcore_del_device_sip(struct ubcore_sip_info *sip)
{
	...
	dev = ubcore_find_mue_device_by_name(sip->dev_name);
	if (!dev || dev->transport_type != UBCORE_TRANSPORT_UB) {
		ubcore_log_err("del sip, dev:%s no mue, or dev not UB\n",
			       sip->dev_name);
		return -1;
	}
	...
	if (ubcore_handle_del_sip_idx(uvs, &dev->sip_table, index) == 0) {
		ret = ubcore_del_sip_entry(&dev->sip_table, index);
		/* Only delete_net_addr when sip changed from valid into invalid to avoid UAF */
		if (ret == 0 && ubcore_del_net_addr(dev, index) != 0) {
			ubcore_log_err("Failed to delete net addr");
			goto add_sip_entry;
		}
		/* nodify uvs add sip info */
		if (ubcore_get_netlink_valid() == true &&
		    ubcore_notify_uvs_del_sip(dev, sip, index) != 0)
			goto add_net_addr;
...
int ubcore_add_sip(struct ubcore_sip_info *sip, uint32_t *sip_idx)
{
	if (sip == NULL || sip_idx == NULL ||
	    strnlen(sip->dev_name, UBCORE_MAX_DEV_NAME) >=
		    UBCORE_MAX_DEV_NAME ||
	    strnlen(sip->netdev_name, UBCORE_MAX_DEV_NAME) >=
		    UBCORE_MAX_DEV_NAME) {
		ubcore_log_err("invalid parameter.\n");
		return -EINVAL;
	}
...
{
	if (sip == NULL || strnlen(sip->dev_name, UBCORE_MAX_DEV_NAME) >=
				   UBCORE_MAX_DEV_NAME) {
		ubcore_log_err("invalid parameter.\n");
		return -EINVAL;
	}
				     ...
				     const union ubcore_net_addr_union *addr)
{
	...
	for (i = 0; i < sip_table->max_sip_cnt; i++) {
		if (sip_table->entry[i].sip_info.is_active &&
		    memcmp(addr, &sip_table->entry[i].sip_info.addr.net_addr,
...
struct ubcore_device *
ubcore_lookup_mue_by_sip_addr(union ubcore_net_addr_union *addr,
			      enum ubcore_transport_type type)
{
	...
	down_read(&g_device_rwsem);
	list_for_each_entry(dev, &g_device_list, list_node) {
		if (dev != NULL && dev->attr.tp_maintainer &&
		    dev->transport_type == type) {
