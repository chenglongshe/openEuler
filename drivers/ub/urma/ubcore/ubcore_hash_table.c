...
int ubcore_hash_table_alloc(struct ubcore_hash_table *ht,
			    const struct ubcore_ht_param *p)
{
	uint32_t i;
...
void ubcore_hash_table_free_with_cb(struct ubcore_hash_table *ht,
				    void (*free_cb)(void *))
{
...
void ubcore_hash_table_add_nolock(struct ubcore_hash_table *ht,
...
void ubcore_hash_table_add(struct ubcore_hash_table *ht,
			   struct hlist_node *hnode, uint32_t hash)
{
	spin_lock(&ht->lock);
	if (ht->head == NULL) {
...
void ubcore_hash_table_remove(struct ubcore_hash_table *ht,
			      struct hlist_node *hnode)
{
	spin_lock(&ht->lock);
...
int ubcore_hash_table_check_remove(struct ubcore_hash_table *ht,
				   struct hlist_node *hnode)
{
	spin_lock(&ht->lock);
...
void *ubcore_hash_table_lookup_nolock_get(struct ubcore_hash_table *ht,
...
void *ubcore_hash_table_lookup_get(struct ubcore_hash_table *ht, uint32_t hash,
				   const void *key)
{
	void *obj = NULL;
...
void *ubcore_hash_table_lookup_nolock(struct ubcore_hash_table *ht,
				      uint32_t hash, const void *key)
{
	struct hlist_node *pos = NULL;
	void *obj = NULL;
...
void *ubcore_hash_table_lookup(struct ubcore_hash_table *ht, uint32_t hash,
			       const void *key)
{
	void *obj = NULL;
...
int ubcore_hash_table_find_add(struct ubcore_hash_table *ht,
			       struct hlist_node *hnode, uint32_t hash)
{
	spin_lock(&ht->lock);
	if (ht->head == NULL) {
...
void *ubcore_hash_table_find_remove(struct ubcore_hash_table *ht, uint32_t hash,
				    const void *key)
{
	struct hlist_node *pos = NULL, *next = NULL;
	void *obj = NULL;
