...
static void ubcore_nl_msg_list_pop(void)
{
	struct ubcore_nl_message *message = NULL, *next;
	unsigned long flags;
	int ret = 0;
	int msg_size;
	int now_buffer;
	...
	list_for_each_entry_safe(message, next, &g_nl_msg_list, node) {
		uvs = ubcore_uvs_find_get_by_genl_port(message->uvs_genl_port);
		if (uvs == NULL) {
			ubcore_log_err("Failed to find uvs: %u\n",
				       message->uvs_genl_port);
			list_del(&message->node);
			kfree(message);
			continue;
		}

		genl_sock = uvs->genl_sock;
		genl_port = uvs->genl_port;
		if (genl_sock == NULL || genl_port == UBCORE_NL_INVALID_PORT) {
			ubcore_log_err(
				"genl_port or genl_sock is invalid for uvs %s",
				uvs->name);
		...
		now_buffer = atomic_sub_return(msg_size, &g_nl_buffer_size);
		atomic_add(msg_size, &uvs->nl_wait_buffer);
		if (now_buffer < 0) {
			atomic_add(msg_size, &g_nl_buffer_size);
			atomic_sub(msg_size, &uvs->nl_wait_buffer);
			ubcore_uvs_kref_put(uvs);
			break;
		}

		ret = nlmsg_unicast(genl_sock, message->nl_skb, genl_port);
		if (ret != 0) {
			ubcore_log_err("Failed to send nl msg %d\n", ret);
			atomic_add(msg_size, &g_nl_buffer_size);
			atomic_sub(msg_size, &uvs->nl_wait_buffer);
			/*
			 * Failure to send a message can lead to a deadlock;
			 * a mechanism for continuing to send messages
			 * upon failure needs to be implemented subsequently
			 */
			ubcore_uvs_kref_put(uvs);
			break;
		}
...
int ubcore_nl_msg_ack_ops(struct sk_buff *skb, struct genl_info *info)
{
	...
	uvs = ubcore_uvs_find_get_by_genl_port(info->snd_portid);
	if (uvs == NULL) {
		ubcore_log_err("Failed to find uvs, port = %d\n",
			       info->snd_portid);
		return 0;
	}

	atomic_sub(ack_size, &uvs->nl_wait_buffer);
...
int ubcore_tp_req_ops(struct sk_buff *skb, struct genl_info *info)
{
	struct ubcore_uvs_instance *uvs = NULL;
	struct ubcore_nlmsg *resp =
		NULL; // resp might not alloc memory, must init resp to null
	struct ubcore_nlmsg *req;
	...
	if (uvs == NULL) {
		kfree(req);
		kfree(resp); // since init resp to null, we can use kfree here
		return 0;
	}

	if (ubcore_genl_unicast(resp, ubcore_nlmsg_len(resp), uvs) != 0)
			       ...
			       enum ubcore_event_type event_type)
{
	...
	uvs = ubcore_find_get_uvs_by_ue(dev, vtp->cfg.ue_idx);
	if (uvs == NULL) {
		ubcore_log_err("Failed to find uvs for ue %u", vtp->cfg.ue_idx);
		kfree(req_msg);
		return;
	}
...
{
	bool any_uvs_is_connected;

	any_uvs_is_connected = ubcore_uvs_list_get_alive_count() > 0;
	if (!any_uvs_is_connected)
		ubcore_log_warn("The nelink service is not established well");

	return any_uvs_is_connected;
}
			       ...
			       struct ubcore_device *dev)
{
	...
	for (; i < max_cnt; i++) {
		sip = &sip_table->entry[i].sip_info;
		if (!sip->is_active)
			continue;
		req_msg = ubcore_new_sip_req_msg(dev, sip,
						 (uint32_t)(unsigned long)i);
		if (req_msg == NULL) {
			ubcore_log_warn("failed to get sip info %s",
					dev->dev_name);
			continue;
		}
...
static int ubcore_get_sip_info_dump(struct sk_buff *skb,
				    struct netlink_callback *cb)
{
...
{
	char uvs_name[UBCORE_MAX_UVS_NAME_LEN];
	uint32_t payload_len;
	int ret;

	payload_len = (uint32_t)nla_len(info->attrs[UBCORE_PAYLOAD_DATA]);
	if (payload_len == 0 || payload_len > UBCORE_MAX_UVS_NAME_LEN) {
		ubcore_log_err("invalid payload len: %d", payload_len);
		return 0;
	}

	if (info->attrs[UBCORE_PAYLOAD_DATA] == NULL) {
		ubcore_log_err("invalid payload data\n");
		return 0;
	}

	(void)memcpy(uvs_name, nla_data(info->attrs[UBCORE_PAYLOAD_DATA]),
		     UBCORE_MAX_UVS_NAME_LEN - 1);
	uvs_name[UBCORE_MAX_UVS_NAME_LEN - 1] = '\0';

	ret = ubcore_uvs_set_genl_info(uvs_name, info->snd_portid,
				       genl_info_net(info)->genl_sock);
	if (ret != 0)
		ubcore_log_err("uvs instance %s doesn't exist\n", uvs_name);
...
void ubcore_unset_genl_pid_ops(uint32_t genl_port)
{
	...
	uvs = ubcore_uvs_find_get_by_genl_port(genl_port);
	if (uvs != NULL) {
		ubcore_log_info(
			"successfully unset port %u for uvs %s, uvs_id %u.\n",
...
void ubcore_uvs_release_nl_buffer(struct ubcore_uvs_instance *uvs)
{
	...
	wait_buffer = atomic_read(&uvs->nl_wait_buffer);
	atomic_add(wait_buffer, &g_nl_buffer_size);
	ubcore_log_warn(
		"successfully release wait buffer for uvs: %s, size: %d/%d\n",
		uvs->name, wait_buffer, atomic_read(&g_nl_buffer_size));
	ubcore_uvs_kref_put(uvs);
}
