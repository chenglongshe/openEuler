...
int ubcore_config_function_migrate_state(struct ubcore_device *dev,
					 ...
					 enum ubcore_mig_state state)
{
	...
	if (dev == NULL || dev->ops == NULL ||
	    dev->ops->config_function_migrate_state == NULL) {
		ret = -EINVAL;
		ubcore_log_err("invalid param");
		return ret;
	}

	ret = dev->ops->config_function_migrate_state(dev, ue_idx, cnt, cfg,
						      state);
	if (ret < 0)
...
int ubcore_modify_vtp(struct ubcore_device *dev,
		      struct ubcore_vtp_param *vtp_param,
		      struct ubcore_vtp_attr *vattr,
		      union ubcore_vtp_attr_mask *vattr_mask)
{
	struct ubcore_tpg *tmp;
	struct ubcore_vtp *vtp;
	...
	if (vtp->role != UBCORE_VTP_TARGET) { // switch to mig dest
		...
		if (ret != 0) {
			ubcore_log_err(
				"Fail to modify vtp when call ubcore ops, ret:%d.\n",
				ret);
			goto put_vtp;
		}
	}
	...
	if (vtp_param->trans_mode == UBCORE_TP_UM) {
		// no vice utp for now, need to add it;
		ubcore_utp_kref_put(vtp->cfg.utp);
		vtp->cfg.utp = vattr->tp.utp;
	} else {
		ubcore_tpg_kref_put(vtp->cfg.tpg);
		tmp = vtp->cfg.tpg;
		vtp->cfg.tpg = vattr->tp.tpg;
		vtp->vice_tpg_info.vice_tpg = tmp;
	}

	if (vtp->vice_tpg_info.node_state == STATE_READY)
	...
	else // STATE_NORMAL SHOULD NOT HAPPEN!!!
		ubcore_log_warn(
			"STATE is normal when modify vtp, vtpn:%u, tpgn:%u, vice tpgn:%u.\n",
			vtp->cfg.vtpn, vtp->cfg.tpg->tpgn,
			vtp->vice_tpg_info.vice_tpg->tpgn);

	ubcore_log_info("Now STATE is:%u, vtpn:%u, tpgn:%u, vice tpgn:%u.\n",
			vtp->vice_tpg_info.node_state, vtp->cfg.vtpn,
			vtp->cfg.tpg->tpgn, vtp->vice_tpg_info.vice_tpg->tpgn);
...
struct ubcore_vtp *ubcore_check_and_map_vtp(struct ubcore_device *dev,
					    struct ubcore_vtp_cfg *cfg,
					    uint32_t role)
{
	struct ubcore_vice_tpg_info vice_tpg_info = { 0 };
	...
	vice_tpg_info.vice_role = UBCORE_VTP_NO_LOCATION;
	vtp = ubcore_find_get_vtp(dev, cfg->trans_mode, &cfg->local_eid,
				  &cfg->peer_eid);
	if (vtp != NULL) {
		ubcore_log_info("vtp already exists");
		if (vtp->cfg.vtpn == UINT_MAX) { // only this may happen
			vtp_role = (role == vtp->role) ? role :
							       UBCORE_VTP_DUPLEX;
			vice_tpg_info = vtp->vice_tpg_info;
			// delete original vtp
			ubcore_remove_vtp(dev, cfg->trans_mode, vtp);
			ubcore_vtp_kref_put(vtp);
			ubcore_log_debug("vtpn is UINT_MAX, delete old one");
		} else { // this may happen during lm
			ubcore_log_warn("now should add vice tpg, vtpn:%u.\n",
					vtp->cfg.vtpn);
			return vtp;
		}
	} else if (vtp == NULL && cfg->vtpn == UINT_MAX) {
		/* vtp is forbidden to be created by driver when vtpn is UINT_MAX */
		ubcore_log_warn("Invalid operation with invalid vtpn.\n");
		return NULL;
	}
	...
	vtp->ub_dev = dev;
	vtp->role = vtp_role;
	vtp->vice_tpg_info = vice_tpg_info;
	ubcore_vtp_map_attr(vtp, cfg);
						   ...
						   uint32_t role)
{
	...
	vtp->role = role;
	ubcore_vtp_map_attr(vtp, cfg);
	vtp->vice_tpg_info.vice_role = UBCORE_VTP_NO_LOCATION;
	vtp->cfg.vtpn = UINT_MAX;
	kref_init(&vtp->ref_cnt);
				...
				uint32_t target_uvs_id)
{
	...
	for (; i < ht->p.size; i++) {
		hlist_for_each_entry(vtp, &ht->head[i], hnode) {
			if (ubcore_check_ue2uvs_mapping(dev, vtp->cfg.ue_idx,
							target_uvs_id))
				++cnt;
		}
	}
