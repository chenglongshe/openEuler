			      ...
			      struct ubcore_jfs_wr **bad_wr)
{
	...
	if (jetty == NULL || jetty->ub_dev == NULL ||
	    ...
	    jetty->ub_dev->ops->post_jetty_send_wr == NULL || wr == NULL ||
	    bad_wr == NULL) {
			      ...
			      struct ubcore_jfr_wr **bad_wr)
{
	...
	if (jetty == NULL || jetty->ub_dev == NULL ||
	    jetty->ub_dev->ops == NULL ||
	    jetty->ub_dev->ops->post_jetty_recv_wr == NULL || wr == NULL ||
	    bad_wr == NULL) {
		ubcore_log_err("Invalid parameter");
		return -EINVAL;
	}
		       ...
		       struct ubcore_jfs_wr **bad_wr)
{
	...
	if (jfs == NULL || jfs->ub_dev == NULL || jfs->ub_dev->ops == NULL ||
	    jfs->ub_dev->ops->post_jfs_wr == NULL || wr == NULL ||
	    bad_wr == NULL) {
		ubcore_log_err("Invalid parameter");
		return -EINVAL;
	}
		       ...
		       struct ubcore_jfr_wr **bad_wr)
{
	...
	if (jfr == NULL || jfr->ub_dev == NULL || jfr->ub_dev->ops == NULL ||
	    jfr->ub_dev->ops->post_jfr_wr == NULL || wr == NULL ||
	    bad_wr == NULL) {
		ubcore_log_err("Invalid parameter");
		return -EINVAL;
	}
