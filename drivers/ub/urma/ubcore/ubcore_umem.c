...
static void umem_unpin_pages(struct ubcore_umem *umem, uint64_t nents)
{
	...
	for_each_sg(umem->sg_head.sgl, sg, nents, i) {
		...
		if (sg == NULL) {
			ubcore_log_err("Invalid sg pointer.\n");
			continue;
		}

		page = sg_page(sg);
		if (page == NULL) {
			ubcore_log_err("Invalid page pointer.\n");
			continue;
		}
		/* Prevent a large number of concurrent accesses
		 * from holding spin_lock for too long, causing system reset
		 */
		cond_resched();
...
static int umem_pin_pages(uint64_t cur_base, uint64_t npages,
...
static struct scatterlist *umem_sg_set_page(struct scatterlist *sg_start,
...
static int umem_add_new_pinned(struct ubcore_umem *umem, uint64_t npages)
{
	...
	if (new_pinned > lock_limit && !capable(CAP_IPC_LOCK)) {
		ubcore_log_err(
			"Npages to be pinned is greater than RLIMIT_MEMLOCK[%llu].\n",
			lock_limit);
		return -ENOMEM;
	}
	return 0;
				   ...
				   uint32_t gup_flags, struct page **page_list)
{
	...
	while (page_count != 0) {
		cond_resched();
		pinned = umem_pin_pages(cur_base, page_count, gup_flags,
					page_list);
		if (pinned < 0) {
			ubcore_log_err(
		...
		sg_list_start =
			umem_sg_set_page(sg_list_start, pinned, page_list);
	}
	return npages;
}

static int umem_verify_input(struct ubcore_device *ub_dev, uint64_t va,
...
static int umem_dma_map(struct ubcore_umem *umem, uint64_t npages,
			unsigned long dma_attrs)
{
	int ret;

	ret = dma_map_sg_attrs(umem->ub_dev->dma_dev, umem->sg_head.sgl,
			       (int)npages, DMA_BIDIRECTIONAL, dma_attrs);
	if (ret == 0) {
...
static int ubcore_fill_umem(struct ubcore_umem *umem, struct ubcore_device *dev,
...
static struct ubcore_umem *ubcore_get_target_umem(struct ubcore_device *dev,
						  uint64_t va, uint64_t len,
						  union ubcore_umem_flag flag,
						  struct page **page_list)
{
	/* FOLL_LONGTERM flag added when pin_user_pages_fast called */
	uint32_t gup_flags = (flag.bs.writable == 1) ? FOLL_WRITE : 0;
...
void ubcore_umem_release(struct ubcore_umem *umem)
{
	...
	if (IS_ERR_OR_NULL(umem) || umem->ub_dev == NULL ||
	    umem->owning_mm == NULL) {
	...
	npages = umem_cal_npages(umem->va, umem->length);
	dma_unmap_sg(umem->ub_dev->dma_dev, umem->sg_head.sgl, (int)umem->nmap,
		     DMA_BIDIRECTIONAL);
	umem_free_sgt(umem);
	umem_atomic_sub(npages, umem->owning_mm);
	mmdrop(umem->owning_mm);
...
uint64_t ubcore_umem_find_best_page_size(struct ubcore_umem *umem,
					 uint64_t page_size_bitmap, uint64_t va)
{
	...
	tmp_ps_bitmap = page_size_bitmap &
			GENMASK(BITS_PER_LONG - 1, PAGE_SHIFT);

	tmp_va = va;
	mask = tmp_ps_bitmap & GENMASK(BITS_PER_LONG - 1,
				       bits_per((umem->length - 1 + va) ^ va));
