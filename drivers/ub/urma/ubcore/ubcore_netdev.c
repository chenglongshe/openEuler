...
ubcore_lookup_sip_info_without_lock(struct ubcore_sip_table *sip_table,
				    uint32_t idx)
{
	struct ubcore_sip_info *sip = NULL;

	if (idx >= sip_table->max_sip_cnt ||
	    !sip_table->entry[idx].sip_info.is_active) {
		ubcore_log_err("sip node does not exist");
		return NULL;
	}
	sip = &sip_table->entry[idx].sip_info;
	return sip;
}

static struct ubcore_nlmsg *
ubcore_alloc_sip_req(enum ubcore_cmd msg_type,
		     enum ubcore_transport_type transport_type,
		     uint32_t payload_len, struct ubcore_sip_info *sip_info)
{
	struct ubcore_nlmsg *req_msg;

	req_msg =
		kzalloc(sizeof(struct ubcore_nlmsg) + payload_len, GFP_KERNEL);
	if (req_msg == NULL)
		return NULL;
			      ...
			      struct ubcore_sip_info *sip_info, uint32_t index)
{
	...
	ubcore_log_info("notify all uvs to del sip: mue %s, sip_idx %u\n",
			sip_info->dev_name, index);

	req_msg = ubcore_alloc_sip_req(UBCORE_CMD_DEL_SIP_REQ,
				       dev->transport_type,
				       sizeof(struct ubcore_del_sip_req),
				       sip_info);
	if (req_msg == NULL)
		return -ENOMEM;
	...
	sip_req->index = index;
	(void)memcpy(sip_req->dev_name, dev->dev_name, UBCORE_MAX_DEV_NAME);

	uvs_list = ubcore_uvs_list_get_all_alive(&count);
	for (i = 0; i < count; i++) {
		ubcore_log_info(
			"sending del_sip request to uvs %s, uvs_id %u\n",
			uvs_list[i]->name, uvs_list[i]->id);

		resp_msg = ubcore_nl_send_wait(dev, req_msg, uvs_list[i]);
		if (resp_msg == NULL) {
			ubcore_log_err(
				"Failed to wait query response from uvs %s, uvs_id %u\n",
				uvs_list[i]->name, uvs_list[i]->id);
			continue;
		}
		...
		if (resp_msg->msg_type != UBCORE_CMD_DEL_SIP_RESP ||
		    resp_msg->payload_len !=
			    sizeof(struct ubcore_del_sip_resp) ||
		    resp->ret != UBCORE_NL_RESP_SUCCESS) {
			ubcore_log_err(
				"del sip is rejected with type %d from uvs %s, uvs_id %u, ret %d\n",
				resp_msg->msg_type, uvs_list[i]->name,
				uvs_list[i]->id,
					    ...
					    uint32_t index)
{
	...
	req_msg = ubcore_alloc_sip_req(UBCORE_CMD_ADD_SIP_REQ,
				       dev->transport_type,
				       sizeof(struct ubcore_add_sip_req),
				       sip_info);
	if (req_msg == NULL)
		return NULL;
	...
	if (strnlen(sip_info->netdev_name, UBCORE_MAX_DEV_NAME) ==
	    UBCORE_MAX_DEV_NAME) {
		ubcore_log_err("sip_info->netdev_name len is invalid");
		kfree(req_msg);
		return NULL;
	}
	(void)memcpy(sip_req->netdev_name, sip_info->netdev_name,
		     UBCORE_MAX_DEV_NAME);
	return req_msg;
}
			      ...
			      struct ubcore_sip_info *sip_info, uint32_t index)
{
	...
	ubcore_log_info("notify all uvs to add sip: mue %s, sip_idx %u\n",
			sip_info->dev_name, index);
	...
	for (i = 0; i < count; i++) {
		ubcore_log_info(
			"sending add_sip request to uvs %s, uvs_id %u\n",
			uvs_list[i]->name, uvs_list[i]->id);
		...
		if (resp_msg->msg_type != UBCORE_CMD_ADD_SIP_RESP ||
		    resp_msg->payload_len !=
			    sizeof(struct ubcore_add_sip_resp) ||
		    resp->ret != UBCORE_NL_RESP_SUCCESS) {
			ubcore_log_err(
				"add sip is rejected with type %d from uvs %s, uvs_id %u, ret %d\n",
				resp_msg->msg_type, uvs_list[i]->name,
				uvs_list[i]->id,
				(resp == NULL ? 1 : resp->ret));
		} else
...
int ubcore_check_port_state(struct ubcore_device *dev)
{
	...
	if (ubcore_query_device_status(dev, &status) != 0) {
		ubcore_log_err(
			"query device status for state failed with dev name %s\n",
			dev->dev_name);
		return -EPERM;
	}
	...
	for (i = 0; i < UBCORE_MAX_PORT_CNT; i++) {
		if (status.port_status[i].state == UBCORE_PORT_ACTIVE) {
			ubcore_log_debug(
				"Success to query dev %s - port %u state and it's active.\n",
				dev->dev_name, i);
			return 0;
		}
	}
			     ...
			     uint8_t *port_list, uint8_t *port_cnt)
{
	...
	list_for_each_entry(port_info, &dev->port_list, node) {
		if (port_info->ndev == real_netdev) {
			(void)memcpy(port_list, port_info->port_list,
				     UBCORE_MAX_PORT_CNT);
...
static int ubcore_add_new_port(struct ubcore_ndev_port *port_info,
			       uint8_t port_id, struct ubcore_device *dev,
			       struct net_device *ndev)
{
	uint8_t i;
	...
	if (port_info->port_cnt >= UBCORE_MAX_PORT_CNT) {
		ubcore_log_err(
			"Failed to add port because it's over the max length");
		return -1;
	}
	for (i = 0; i < UBCORE_MAX_PORT_CNT; i++) {
			   ...
			   unsigned int port_id)
{
	...
	if (dev == NULL || ndev == NULL ||
	    strnlen(dev->dev_name, UBCORE_MAX_DEV_NAME) >=
		    UBCORE_MAX_DEV_NAME ||
	    strnlen(netdev_name(ndev), UBCORE_MAX_DEV_NAME) >=
		    UBCORE_MAX_DEV_NAME) {
	...
	list_for_each_entry(port_info, &dev->port_list, node) {
		if (port_info->ndev == ndev) {
			if (ubcore_port_duplicate_check(port_info,
							...
							ndev) != 0) {
				up_write(&g_port_list_lock);
				ubcore_log_err("Failed to do %s", __func__);
				return -1;
			}
			if (ubcore_add_new_port(port_info, (uint8_t)port_id,
						dev, ndev) != 0) {
				up_write(&g_port_list_lock);
			...
			up_write(&g_port_list_lock);
			/* sync to sip table */
			ubcore_update_netdev_addr(dev, dev->netdev,
						  UBCORE_UPDATE_NET_ADDR,
						  false);
			ubcore_update_all_vlan_netaddr(dev,
						       UBCORE_UPDATE_NET_ADDR);
			return 0;
		}
	}
	...
	up_write(&g_port_list_lock);
	ubcore_log_info("ndev:%s bound port[0]: %u\n", netdev_name(ndev),
			new_node->port_list[0]);
	ubcore_update_netdev_addr(dev, dev->netdev, UBCORE_UPDATE_NET_ADDR,
				  false);
	ubcore_update_all_vlan_netaddr(dev, UBCORE_UPDATE_NET_ADDR);
	return 0;
}
EXPORT_SYMBOL(ubcore_set_port_netdev);

/* del corresponding port id, if the port list cnt is 0, it will del the entry */
static int ubcore_del_port(struct ubcore_ndev_port *port_info, uint8_t port_id,
			   struct ubcore_device *dev, struct net_device *ndev)
{
	bool del = false;
	uint8_t i;
	...
	if (port_info->port_cnt == 0) {
		...
		kfree(port_info);
		ubcore_log_info(
			"ndev:%s bound port_list has been remove entirely\n",
			netdev_name(ndev));
	}
	return 0;
}
			     ...
			     unsigned int port_id)
{
	...
	list_for_each_entry(port_info, &dev->port_list, node) {
		if (port_info->ndev == ndev) {
			if (ubcore_del_port(port_info, (uint8_t)port_id, dev,
					    ndev) != 0) {
				up_write(&g_port_list_lock);
	...
	ubcore_log_err(
		"Failed to find and remove ndev:%s dev_name: %s bound port: %u\n",
		netdev_name(ndev), dev->dev_name, port_id);
	return -1;
}
EXPORT_SYMBOL(ubcore_unset_port_netdev);

void ubcore_free_netdev_port_list(struct ubcore_device *dev)
{
	...
	list_for_each_entry_safe(port_info, next, &dev->port_list, node) {
		if (port_info != NULL) {
			if (port_info->port_cnt != 0) {
				...
				(void)memset(port_info->port_list, 0,
					     sizeof(uint8_t) *
						     UBCORE_MAX_PORT_CNT);
			}
			list_del(&port_info->node);
...
int ubcore_sip_idx_alloc(struct ubcore_sip_table *sip_table)
{
	...
	mutex_lock(&sip_table->lock);
	ret_idx = (uint32_t)find_first_zero_bit(sip_table->index_bitmap,
						UBCORE_MAX_SIP);
...
int ubcore_sip_idx_free_without_lock(struct ubcore_sip_table *sip_table,
				     uint32_t idx)
{
...
int ubcore_sip_idx_free(struct ubcore_sip_table *sip_table, uint32_t idx)
{
	...
	mutex_lock(&sip_table->lock);
	ret = ubcore_sip_idx_free_without_lock(sip_table, idx);
	mutex_unlock(&sip_table->lock);
	return ret;
}

int ubcore_add_sip_entry(struct ubcore_sip_table *sip_table,
			 const struct ubcore_sip_info *sip, uint32_t idx)
{
	mutex_lock(&sip_table->lock);
	if (idx >= sip_table->max_sip_cnt ||
	    sip_table->entry[idx].sip_info.is_active) {
	...
	(void)memcpy(&sip_table->entry[idx].sip_info, sip,
		     sizeof(struct ubcore_sip_info));
	sip_table->entry[idx].sip_info.is_active = true;
	atomic_set(&sip_table->entry[idx].uvs_cnt, 0);
	mutex_unlock(&sip_table->lock);
	ubcore_log_info(
		"mue_dev_name: %s sip table add entry idx: %d. addr: %pI6c\n",
		sip->dev_name, idx, &sip->addr.net_addr);
	return 0;
}

int ubcore_del_net_addr(struct ubcore_device *dev, uint32_t idx)
{
	if (dev == NULL || dev->ops == NULL ||
	    dev->ops->delete_net_addr == NULL) {
		ubcore_log_err("Invalid ops");
		return -EINVAL;
	}
	return dev->ops->delete_net_addr(dev, idx);
}

int ubcore_del_sip_entry_without_lock(struct ubcore_sip_table *sip_table,
				      uint32_t idx)
{
	if (idx >= sip_table->max_sip_cnt ||
	    !sip_table->entry[idx].sip_info.is_active) {
	...
	ubcore_log_info("mue_name: %s del sip entry idx: %u, addr: %pI6c.\n",
			sip_table->entry[idx].sip_info.dev_name, idx,
			&sip_table->entry[idx].sip_info.addr.net_addr);
	sip_table->entry[idx].sip_info.is_active = false;
	return 0;
}
...
int ubcore_del_sip_entry(struct ubcore_sip_table *sip_table, uint32_t idx)
{
	...
	mutex_lock(&sip_table->lock);
	ret = ubcore_del_sip_entry_without_lock(sip_table, idx);
	mutex_unlock(&sip_table->lock);
	return ret;
}
...
int ubcore_update_sip_entry(struct ubcore_sip_table *sip_table,
			    struct ubcore_sip_info *new_sip, uint32_t *sip_idx,
			    struct ubcore_sip_info *old_sip)
{
	...
	for (i = 0; i < sip_table->max_sip_cnt; i++) {
		if (!sip_table->entry[i].sip_info.is_active ||
		    !ubcore_sip_compare(&sip_table->entry[i].sip_info, new_sip))
			continue;

		*sip_idx = i;
		*old_sip = sip_table->entry[i].sip_info;

		sip_table->entry[i].sip_info = *new_sip;
		sip_table->entry[i].sip_info.is_active = true;
		ret = 0;
		ubcore_log_info(
			"mue_name: %s update sip entry idx: %d, addr: %pI6c.",
			sip_table->entry[i].sip_info.dev_name, i,
			&sip_table->entry[i].sip_info.addr.net_addr);
		break;
	}
	mutex_unlock(&sip_table->lock);
	return ret;
...
int ubcore_lookup_sip_idx(struct ubcore_sip_table *sip_table,
			  struct ubcore_sip_info *sip, uint32_t *idx)
{
	uint32_t i;
	...
	for (i = 0; i < sip_table->max_sip_cnt; i++) {
		if (sip_table->entry[i].sip_info.is_active &&
		    ubcore_sip_compare(&sip_table->entry[i].sip_info, sip)) {
			*idx = i;
			break;
		}
	}
