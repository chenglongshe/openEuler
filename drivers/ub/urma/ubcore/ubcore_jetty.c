...
struct ubcore_jfs *ubcore_find_get_jfs(struct ubcore_device *dev,
				       uint32_t jfs_id)
{
	if (dev == NULL) {
	...
	return ubcore_hash_table_lookup_get(&dev->ht[UBCORE_HT_JFS], jfs_id,
					    &jfs_id);
}

struct ubcore_jfr *ubcore_find_get_jfr(struct ubcore_device *dev,
				       uint32_t jfr_id)
{
	if (dev == NULL) {
		ubcore_log_err("dev is NULL\n");
	...
	return ubcore_hash_table_lookup_get(&dev->ht[UBCORE_HT_JFR], jfr_id,
					    &jfr_id);
}
				     ...
				     struct ubcore_udata *udata)
{
	...
	if (dev == NULL || cfg == NULL || dev->ops == NULL ||
	    dev->ops->create_jfc == NULL || dev->ops->destroy_jfc == NULL)
		      ...
		      struct ubcore_udata *udata)
{
	...
	if (jfc == NULL || attr == NULL || jfc->ub_dev == NULL ||
	    jfc->ub_dev->ops == NULL || jfc->ub_dev->ops->modify_jfc == NULL)
		return -EINVAL;
			    ...
			    int *bad_jfc_index)
{
	...
	if (jfc_arr == NULL || jfc_num <= 0 || bad_jfc_index == NULL) {
		ubcore_log_err("Invalid parameter.");
		return -EINVAL;
	}
	...
	for (i = 0; i < jfc_num; ++i) {
		...
		if (jfc == NULL || jfc->ub_dev == NULL ||
		    jfc->ub_dev->ops == NULL ||
		    jfc->ub_dev->ops->destroy_jfc_batch == NULL) {
			*bad_jfc_index = 0;
		      ...
		      struct ubcore_udata *udata)
{
	...
	if (jfs == NULL || attr == NULL || jfs->ub_dev == NULL ||
	    jfs->ub_dev->ops == NULL || jfs->ub_dev->ops->modify_jfs == NULL)
		return -EINVAL;
			    ...
			    int *bad_jfs_index)
{
	...
	if (jfs_arr == NULL || jfs_num <= 0 || bad_jfs_index == NULL) {
		ubcore_log_err("Invalid parameter.");
		return -EINVAL;
	}
	...
	for (i = 0; i < jfs_num; ++i) {
		...
		if (jfs == NULL || jfs->ub_dev == NULL ||
		    jfs->ub_dev->ops == NULL ||
		    jfs->ub_dev->ops->destroy_jfs_batch == NULL) {
			*bad_jfs_index = 0;
			ubcore_log_err("Invalid parameter, index is %d", i);
			return -EINVAL;
		}
		      ...
		      struct ubcore_udata *udata)
{
	...
	if (jfr == NULL || attr == NULL || jfr->ub_dev == NULL ||
	    jfr->ub_dev->ops == NULL || jfr->ub_dev->ops->modify_jfr == NULL)
		return -EINVAL;
			    ...
			    int *bad_jfr_index)
{
	...
	if (jfr_arr == NULL || jfr_num <= 0 || bad_jfr_index == NULL) {
		ubcore_log_err("Invalid parameter.");
		return -EINVAL;
	}
	...
	for (i = 0; i < jfr_num; ++i) {
		...
		if (jfr == NULL || jfr->ub_dev == NULL ||
		    jfr->ub_dev->ops == NULL ||
		    jfr->ub_dev->ops->destroy_jfr_batch == NULL) {
			*bad_jfr_index = 0;
			ubcore_log_err("Invalid parameter, index is %d", i);
			return -EINVAL;
		}
					...
					struct ubcore_udata *udata)
{
	...
	if (!ubcore_have_ops(dev) || dev->ops->unimport_jfr == NULL ||
					 ...
					 struct ubcore_udata *udata)
{
	...
	if (dev == NULL || cfg == NULL || dev->ops == NULL ||
	    dev->ops->create_jetty == NULL || dev->ops->destroy_jetty == NULL ||
	    !ubcore_eid_valid(dev, cfg->eid_index, udata))
		return ERR_PTR(-EINVAL);
			...
			struct ubcore_udata *udata)
{
	...
	if (jetty == NULL || attr == NULL || jetty->ub_dev == NULL ||
	    jetty->ub_dev->ops == NULL ||
		       ...
		       struct ubcore_jetty_attr *attr)
{
	...
	if (jetty == NULL || cfg == NULL || attr == NULL ||
	    jetty->ub_dev == NULL || jetty->ub_dev->ops == NULL ||
			      ...
			      int *bad_jetty_index)
{
	...
	if (jetty_arr == NULL || jetty_num <= 0 || bad_jetty_index == NULL) {
		ubcore_log_err("Invalid parameter.");
		return -EINVAL;
	}
					  ...
					  struct ubcore_udata *udata)
{
	...
	if (!ubcore_have_ops(dev) || dev->ops->unimport_jetty == NULL ||
	    cfg == NULL || dev->attr.dev_cap.max_eid_cnt <= cfg->eid_index)
		return ERR_PTR(-EINVAL);
...
int ubcore_unimport_jetty(struct ubcore_tjetty *tjetty)
{
	...
	if (tjetty == NULL || tjetty->ub_dev == NULL ||
	    tjetty->ub_dev->ops == NULL ||
	    tjetty->ub_dev->ops->unimport_jetty == NULL ||
...
static int ubcore_advice_jfs_tjfr(struct ubcore_tp_advice *advice,
				  struct ubcore_jfs *jfs,
				  struct ubcore_tjetty *tjfr)
{
...
static int ubcore_advice_jetty_tjetty(struct ubcore_tp_advice *advice,
				      struct ubcore_jetty *jetty,
				      struct ubcore_tjetty *tjetty)
{
	(void)memset(advice, 0, sizeof(struct ubcore_tp_advice));
			...
			struct ubcore_udata *udata)
{
	...
	if (jetty == NULL || tjetty == NULL ||
	    !ubcore_have_tp_ops(jetty->ub_dev)) {
		ubcore_log_err("invalid parameter.\n");
		return -1;
	}
	...
	ret = ubcore_advise_tp(jetty->ub_dev, &tjetty->cfg.id.eid, &advice,
			       udata);
	ubcore_put_advice(&advice);
	return ret;
}
				      ...
				      struct ubcore_udata *udata)
{
	...
	if (dev->ops == NULL || dev->ops->unbind_jetty == NULL) {
...
int ubcore_unbind_jetty(struct ubcore_jetty *jetty)
{
	...
	if ((jetty->jetty_cfg.trans_mode != UBCORE_TP_RC) || tjetty == NULL ||
	    (tjetty->cfg.trans_mode != UBCORE_TP_RC)) {
		ubcore_log_err("trans mode is not rc type.\n");
		return -EINVAL;
	}
	...
	ubcore_event_callback_t jfae_handler, struct ubcore_udata *udata)
{
	...
	if (dev == NULL || cfg == NULL || dev->ops == NULL ||
	    dev->ops->create_jetty_grp == NULL ||
	    dev->ops->delete_jetty_grp == NULL ||
	    !ubcore_eid_valid(dev, cfg->eid_index, udata))
		return ERR_PTR(-EINVAL);
						...
						struct ubcore_udata *udata)
{
	...
	if (!ubcore_have_ops(dev) || dev->ops->import_jetty == NULL ||
	    dev->ops->unimport_jetty == NULL || cfg == NULL ||
	    dev->attr.dev_cap.max_eid_cnt <= cfg->eid_index || cb == NULL)
		return ERR_PTR(-EINVAL);
...
int ubcore_bind_jetty_async(struct ubcore_jetty *jetty,
			    ...
			    struct ubcore_udata *udata)
{
	if (jetty == NULL || tjetty == NULL || cb == NULL ||
	    !ubcore_have_ops(jetty->ub_dev)) {
		ubcore_log_err("invalid parameter.\n");
		return -1;
	}
