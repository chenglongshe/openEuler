...
static DEFINE_MUTEX(g_ubcore_net_event_lock);
...
static uint32_t g_ubcore_support_versions[UBCORE_SUPPORT_VERION_NUM] = {
	UBCORE_VERSION0
};
...
static void ubcore_sip_init(struct ubcore_sip_info *sip,
			    struct ubcore_device *dev,
			    const struct ubcore_net_addr *netaddr,
			    struct net_device *netdev)
{
	memcpy(sip->dev_name, dev->dev_name, UBCORE_MAX_DEV_NAME);
	memcpy(&sip->addr, netaddr, sizeof(struct ubcore_net_addr));

	if (netdev) {
		ubcore_fill_port_netdev(dev, netdev, sip->port_id,
					&sip->port_cnt);
		sip->mtu = dev->transport_type == UBCORE_TRANSPORT_UB ?
					 (uint32_t)sip_get_mtu_with_ub(netdev->mtu) :
					 (uint32_t)sip_get_mtu(netdev->mtu);
		memcpy(sip->netdev_name, netdev_name(netdev),
		       UBCORE_MAX_DEV_NAME);
	} else {
		/* mtu is 4k for UBC scenario */
		sip->mtu = UBCORE_MTU_4096;
	}
}

static void ubcore_notify_uvs_update_sip(struct ubcore_device *mue_dev,
					 struct ubcore_sip_info *new_sip,
					 struct ubcore_sip_info *old_sip,
					 uint32_t index)
{
	(void)ubcore_notify_uvs_del_sip(mue_dev, old_sip, index);
	(void)ubcore_notify_uvs_add_sip(mue_dev, new_sip, index);
}

static bool ubcore_notify_uvs_update_sip_sync(struct ubcore_device *mue_dev,
					      struct ubcore_sip_info *new_sip,
					      struct ubcore_sip_info *old_sip,
					      enum ubcore_sip_op_type sip_op,
					      uint32_t index)
{
	...
	case UBCORE_SIP_UPDATE:
		ubcore_notify_uvs_update_sip(mue_dev, new_sip, old_sip, index);
		return true;
	default:
...
static void ubcore_notify_uvs_update_sip_task(struct work_struct *work)
{
	...
	(void)ubcore_notify_uvs_update_sip_sync(l_work->mue_dev,
						&l_work->new_sip,
						&l_work->old_sip,
						l_work->sip_op, l_work->index);
	kfree(l_work);
}

static int ubcore_notify_uvs_update_sip_async(struct ubcore_device *mue_dev,
					      struct ubcore_sip_info *new_sip,
					      struct ubcore_sip_info *old_sip,
					      enum ubcore_sip_op_type sip_op,
					      uint32_t index)
...
static int ubcore_notify_uvs_update_sip_manage(struct ubcore_device *mue_dev,
					       struct ubcore_sip_info *new_sip,
					       struct ubcore_sip_info *old_sip,
					       enum ubcore_sip_op_type sip_op,
					       uint32_t index, bool async)
{
	int ret = 0;

	if (!async) {
		(void)ubcore_notify_uvs_update_sip_sync(mue_dev, new_sip,
							old_sip, sip_op, index);
		return 0;
	}

	ret = ubcore_notify_uvs_update_sip_async(mue_dev, new_sip, old_sip,
						 sip_op, index);
	if (ret != 0)
...
static int ubcore_get_upi(char *dev_name, uint32_t pattern, uint32_t *upi)
{
	...
	if (pattern == UBCORE_PATTERN_3) {
		if (ubcore_find_upi_with_dev_name(dev_name, upi) != 0) {
...
static int ubcore_handle_update_eid(struct ubcore_device *mue_dev,
				    uint32_t ue_idx, enum ubcore_net_addr_op op,
				    struct ubcore_eid_update_info *eid_info)
{
	...
	if (eid_info->upi_present) {
		...
		if (ubcore_get_upi(eid_info->dev_name, eid_info->pattern,
				   &cfg.upi) != 0) {
	...
	if (op == UBCORE_ADD_NET_ADDR)
		return ubcore_add_ueid(mue_dev, ue_idx, &cfg);

	if (op == UBCORE_DEL_NET_ADDR)
		return ubcore_delete_ueid(mue_dev, ue_idx, &cfg);

	return -EINVAL;
}

int ubcore_send_eid_update_req(struct ubcore_device *dev,
			       ...
			       uint32_t *upi)
{
	...
	if (dev->attr.tp_maintainer)
		ret = ubcore_handle_update_eid(dev, dev->attr.ue_idx,
					       msg_data->op,
					       &msg_data->eid_info);
	else
		ret = ubcore_send_req(
...
static void ubcore_handle_add_sip(struct ubcore_device *mue_dev,
				  struct ubcore_sip_info *sip, bool async)
{
	uint32_t index;
	int ret;

	ret = ubcore_lookup_sip_idx(&mue_dev->sip_table, sip, &index);
	if (ret == 0) {
		ubcore_log_err("sip already exists\n");
		return;
	}

	index = (uint32_t)ubcore_sip_idx_alloc(&mue_dev->sip_table);
	if (mue_dev->ops != NULL && mue_dev->ops->add_net_addr != NULL &&
	    mue_dev->ops->add_net_addr(mue_dev, &sip->addr, index) != 0)
		ubcore_log_err("Failed to set net addr");

	/* add net_addr entry, record idx -> netaddr mapping */
	(void)ubcore_add_sip_entry(&mue_dev->sip_table, sip, index);

	/* nodify uvs add sip info */
	if (ubcore_notify_uvs_update_sip_manage(
		    mue_dev, sip, NULL, UBCORE_SIP_ADD, index, async) != 0)
		ubcore_log_err("notify uvs sip failed");
}

static void ubcore_handle_delete_sip(struct ubcore_device *mue_dev,
				     struct ubcore_sip_info *sip, bool async)
{
	uint32_t index;

	if (ubcore_lookup_sip_idx(&mue_dev->sip_table, sip, &index) != 0)
		return;

	if (ubcore_del_net_addr(mue_dev, index) != 0)
		ubcore_log_err("Failed to delete net addr");

	(void)ubcore_del_sip_entry(&mue_dev->sip_table, index);
	(void)ubcore_sip_idx_free(&mue_dev->sip_table, index);
	/* nodify uvs delete sip info */
	if (ubcore_notify_uvs_update_sip_manage(
		    mue_dev, NULL, sip, UBCORE_SIP_DEL, index, async) != 0)
		ubcore_log_err("notify uvs sip failed");
}

static void ubcore_hanlde_update_sip(struct ubcore_device *mue_dev,
				     struct ubcore_sip_info *sip, bool async)
{
	...
	ret = ubcore_update_sip_entry(&mue_dev->sip_table, sip, &sip_idx,
				      &old_sip);
	if (ret != 0) {
	...
	if (ubcore_notify_uvs_update_sip_manage(mue_dev, sip, &old_sip,
						UBCORE_SIP_UPDATE, sip_idx,
						async) != 0)
		ubcore_log_err("notify uvs sip failed");
}

static int ubcore_hanlde_update_net_addr(struct ubcore_device *mue_dev,
					 uint32_t ue_idx,
					 struct ubcore_update_net_addr_req *req,
					 bool async)
{
	if (ubcore_is_ub_device(mue_dev) && req->sip_present) {
		if (req->op == UBCORE_ADD_NET_ADDR)
			ubcore_handle_add_sip(mue_dev, &req->sip_info, async);
		else if (req->op == UBCORE_DEL_NET_ADDR)
			ubcore_handle_delete_sip(mue_dev, &req->sip_info,
						 async);
		else if (req->op == UBCORE_UPDATE_NET_ADDR)
			ubcore_hanlde_update_sip(mue_dev, &req->sip_info,
						 async);
	}

	if (req->eid_present)
		return ubcore_handle_update_eid(mue_dev, ue_idx, req->op,
						&req->eid_info);
...
static int
ubcore_send_net_addr_update_req(struct ubcore_device *dev,
				 ...
				 struct net *net, uint32_t *eid_idx)
{
	...
	if (ubcore_check_ctrlplane(dev)) {
		ubcore_log_warn(
			"No need to update eid table for control plane.\n");
		return 0;
	}
...
static int ubcore_handle_inetaddr_event(struct net_device *netdev,
					unsigned long event,
					struct ubcore_net_addr *netaddr)
{
	...
	if (netdev == NULL ||
	    (netdev->reg_state >= NETREG_UNREGISTERING && event != NETDEV_DOWN))
		return NOTIFY_DONE;
	...
	for (i = 0; i < num_devices; i++) {
		...
		switch (event) {
		case NETDEV_UP:
			ubcore_update_net_addr(dev, netdev, netaddr,
					       UBCORE_ADD_NET_ADDR, true);
			break;
		case NETDEV_DOWN:
			ubcore_update_net_addr(dev, netdev, netaddr,
					       UBCORE_DEL_NET_ADDR, true);
			break;
		default:
			break;
		}
				     ...
				     unsigned long event, void *arg)
{
	...
	netdev = ifa->idev->dev;
	ubcore_log_info(
		"Get a ipv6 event %s from netdev %s%s ip %pI6c prefixlen %u\n",
		netdev_cmd_to_name(event), netdev_name(netdev),
		netdev_reg_state(netdev), &ifa->addr, ifa->prefix_len);

	memset(&netaddr, 0, sizeof(struct ubcore_net_addr));
	memcpy(&netaddr.net_addr, &ifa->addr, sizeof(struct in6_addr));
	(void)ubcore_fill_netaddr_macvlan(&netaddr, netdev,
					  UBCORE_NET_ADDR_TYPE_IPV6);
	netaddr.prefix_len = ifa->prefix_len;

	if (netaddr.net_addr.in6.subnet_prefix ==
	    cpu_to_be64(UBCORE_LOCAL_SHUNET))
...
static int ubcore_ipv4_notifier_call(struct notifier_block *nb,
				     unsigned long event, void *arg)
{
	...
	ubcore_log_info("Get a ipv4 event %s netdev %s%s ip %pI4 prefixlen %u",
			netdev_cmd_to_name(event), netdev_name(netdev),
			netdev_reg_state(netdev), &ifa->ifa_address,
			ifa->ifa_prefixlen);
				  ...
				  struct net_device *netdev)
{
	...
	(void)ubcore_fill_netaddr_macvlan(&na_entry->addr, netdev,
					  UBCORE_NET_ADDR_TYPE_IPV4);
				  ...
				  struct net_device *netdev)
{
	...
	memcpy(&na_entry->addr.net_addr, ipv6, sizeof(struct in6_addr));
	(void)ubcore_fill_netaddr_macvlan(&na_entry->addr, netdev,
					  UBCORE_NET_ADDR_TYPE_IPV6);
	na_entry->addr.prefix_len = prefix_len;
	list_add_tail(&na_entry->node, list);
...
static void ubcore_netdev_get_ipv4(struct net_device *netdev,
				   struct list_head *list)
{
	...
	in_dev_for_each_ifa_rcu(ifa, in_dev) {
		ubcore_add_ipv4_entry(list, ifa->ifa_address,
				      ifa->ifa_prefixlen, netdev);
	}
	rcu_read_unlock();
}

static void ubcore_netdev_get_ipv6(struct net_device *netdev,
				   struct list_head *list)
{
	...
	list_for_each_entry(ifa, &in_dev->addr_list, if_list) {
		ubcore_add_ipv6_entry(list, (struct in6_addr *)&ifa->addr,
				      ifa->prefix_len, netdev);
	}
			   ...
			   enum ubcore_net_addr_op op, bool async)
{
	...
	if ((op == UBCORE_ADD_NET_ADDR || op == UBCORE_DEL_NET_ADDR)) {
		ret = ubcore_update_eid_tbl(dev, netaddr,
					    op == UBCORE_ADD_NET_ADDR,
		...
		if (ret == 0 && dev->dynamic_eid) {
			...
			memcpy(req.eid_info.eid.raw, netaddr->net_addr.raw,
			       UBCORE_NET_ADDR_BYTES);
			memcpy(req.eid_info.dev_name, dev->dev_name,
			       UBCORE_MAX_DEV_NAME);
		}
	}

	// If dev is not MUE, send msg to MUE
	if (dev->attr.tp_maintainer)
		ret = ubcore_hanlde_update_net_addr(dev, dev->attr.ue_idx, &req,
						    async);
	else
		ret = ubcore_send_net_addr_update_req(
			dev, &req);

	// Delete eid entry if failed
	if (ret != 0 && op == UBCORE_ADD_NET_ADDR && req.eid_present)
		(void)ubcore_update_eid_tbl(dev, netaddr, false,
					    dev_net(netdev), &eid_idx);
			       ...
			       enum ubcore_net_addr_op op, bool async)
{
	...
	list_for_each_entry_safe(na_entry, next, &na_list, node) {
		if (na_entry->addr.net_addr.in6.subnet_prefix ==
		    cpu_to_be64(UBCORE_LOCAL_SHUNET))
			continue;
				    ...
				    enum ubcore_net_addr_op op)
{
	...
	for (i = 0; i < VLAN_N_VID; i++) {
		net_dev = __vlan_find_dev_deep_rcu(dev->netdev,
						   htons(ETH_P_8021Q), i);
		if (!net_dev)
			continue;

		ubcore_update_netdev_addr(dev, net_dev, op, false);
		ubcore_log_info("update dev:%s, in vlan id:%u", dev->dev_name,
				i);
	}
}

static void ubcore_change_mtu(struct ubcore_device *dev,
			      struct net_device *netdev)
{
...
static int
ubcore_netdev_event_change_upper(struct ubcore_device *dev,
				 ...
				 struct netdev_notifier_changeupper_info *info)
{
	...
	if (dev == NULL || dev->ops == NULL || dev->ops->bond_add == NULL ||
	    dev->ops->bond_remove == NULL || info == NULL || bond == NULL ||
	    slave == NULL) {
	...
	if (info->linking && info->upper_info == NULL) {
		ubcore_log_info("upper info is NULL\n");
		ubcore_put_device(dev);
		return -EINVAL;
	}
	...
	if (info->linking) {
		...
		if (ret != 0) {
			ubcore_log_err("Failed to bond_add and ret value is %d",
				       ret);
			return -EIO;
		}
	} else {
		...
		if (ret != 0) {
			ubcore_log_err(
				"Failed to bond_remove and ret value is %d",
				ret);
			return -EIO;
		}
	}
...
static int ubcore_netdev_event_change_lower_state(
	struct ubcore_device *dev, struct net_device *slave,
	struct netdev_notifier_changelowerstate_info *info)
{
	...
	if (info == NULL || info->lower_state_info == NULL) {
		ubcore_log_info("lower state info is NULL\n");
		return -EINVAL;
	}
	...
	if (bond) {
		ubcore_log_info(
			"Event with master netdev %s and slave netdev %s",
			netdev_name(bond), netdev_name(slave));
	} else {
		ubcore_log_info(
			"Event with master netdev NULL and slave netdev %s",
			netdev_name(slave));
	}
	lag_lower_info = info->lower_state_info;
	ret = dev->ops->slave_update(bond, slave, lag_lower_info);
	if (ret != 0) {
		ubcore_log_err("Failed to slave_update and ret value is %d",
			       ret);
		return -EIO;
	}
...
ubcore_find_master_netdev(unsigned long event,
			  struct netdev_notifier_changeupper_info *info,
			  struct net_device *slave)
{
...
static void ubcore_do_netdev_notify(unsigned long event,
				    struct ubcore_device *dev,
...
static void ubcore_do_netdev_bond_notify(unsigned long event,
					 struct ubcore_device *dev,
					 struct net_device *netdev, void *arg)
{
	switch (event) {
	case NETDEV_CHANGEUPPER:
		...
		if (dev->transport_type == UBCORE_TRANSPORT_UB)
			(void)ubcore_netdev_event_change_upper(dev, netdev,
							       arg);
		else
			ubcore_put_device(dev);
		break;
	case NETDEV_CHANGELOWERSTATE:
		if (dev->transport_type == UBCORE_TRANSPORT_UB)
			(void)ubcore_netdev_event_change_lower_state(
				dev, netdev, arg);
		break;
	default:
		break;
...
static int ubcore_net_notifier_call(struct notifier_block *nb,
				    unsigned long event, void *arg)
{
	...
	ubcore_log_info("Get a net event %s from ubcore_dev %s%s",
			netdev_cmd_to_name(event), netdev_name(netdev),
			netdev_reg_state(netdev));

	if (event == NETDEV_CHANGEUPPER || event == NETDEV_CHANGELOWERSTATE) {
		mutex_lock(&g_ubcore_net_event_lock);
		devices = ubcore_get_devices_from_netdev(netdev, &num_devices);
		if (devices == NULL) {
			master_netdev =
				ubcore_find_master_netdev(event, arg, netdev);
			if (master_netdev == NULL) {
				ubcore_log_warn(
					"Can not find master netdev by slave netdev %s",
					netdev_name(netdev));
				mutex_unlock(&g_ubcore_net_event_lock);
				return NOTIFY_DONE;
			}
			ubcore_log_info("Success to find master netdev %s",
					netdev_name(master_netdev));
			devices = ubcore_get_devices_from_netdev(master_netdev,
								 &num_devices);
			if (devices == NULL) {
				ubcore_log_warn(
					"Can not find devices from master netdev %s",
					netdev_name(master_netdev));
				mutex_unlock(&g_ubcore_net_event_lock);
				return NOTIFY_DONE;
			}
		...
		for (i = 0; i < num_devices; i++)
			ubcore_do_netdev_bond_notify(event, devices[i], netdev,
						     arg);
		mutex_unlock(&g_ubcore_net_event_lock);
	} else {
		devices = ubcore_get_devices_from_netdev(netdev, &num_devices);
...
static int __init ubcore_init(void)
{
	...
	/* netlink_register_notifier should be executed after initializing uvs list */
	ubcore_uvs_list_init();
	ubcore_ue2uvs_tables_init();
...
reg_pnet:
	ubcore_genl_exit();
genl_init:
	ubcore_ue2uvs_tables_uninit();
	ubcore_uvs_list_uninit();
