...
size_t conti_mem_allocator_expand(struct conti_mem_allocator *allocator, size_t size)
{
	...
	if (expand_size > 0)
		pr_info("%s: expand expect size %#zx, actual size %#zx\n", current->comm, size,
			expand_size);
...
size_t conti_mem_allocator_contract(struct conti_mem_allocator *allocator, size_t size)
{
	...
	if (contract_size > 0)
		pr_info_ratelimited("%s: nid: %d, contract expect size %#zx, actual size %#zx\n",
...
static size_t conti_mem_allocator_free_poisoned(struct conti_mem_allocator *allocator)
{
	 ...
	 */
	if (free_size > 0)
		pr_info("%s: nid: %d, %#zx poisoned memory freed\n", current->comm, allocator->nid,
			free_size);
...
static int conti_pool_thread(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		if (conti_has_poisoned_memseg(allocator)) {
			...
			pr_info("%s: nid=%d, free poisoned done, ret=%#zx\n", __func__,
		...
		if (conti_need_contract(allocator)) {
			...
			if (size > 0) {
				...
				if (ret_size)
					pr_info("%s: nid=%d, contract done, ret=%#zx\n", __func__,
						allocator->nid, ret_size);
			}
		}
		...
		if (conti_need_expand(allocator)) {
			...
			if (size > 0) {
				...
				if (ret_size)
					pr_info("%s: nid=%d, expand done, ret=%#zx\n", __func__,
						allocator->nid, ret_size);
			}
		}
	}
...
#ifdef OBMM_KUNIT_TESTS
#include "tests/conti_mem_allocator_test.c"
#endif
