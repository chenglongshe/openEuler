...
static unsigned long get_vma_mem_state(const vm_flags_t vm_flags, bool cacheable)
{
	...
	pr_info_ratelimited("VMA init mem_state: vma_flags=0x%lx, cacheable=%d, mem_state=0x%lx\n",
...
{
	pr_info_ratelimited("VMA opened range (0x%lx-0x%lx)\n", vma->vm_start, vma->vm_end);
}
...
static void obmm_vma_close(struct vm_area_struct *vma)
{
	...
	pr_info("obmm_shmdev munmap: mem_id=%d pid=%d vma=[%#lx, %#lx]\n", reg->regionid,
...
static int obmm_shm_fops_open(struct inode *inode, struct file *file)
{
	...
	pr_info("obmm_shmdev open: mem_id=%d pid=%d f_mode=%#x f_flags=%#x\n", reg->regionid,
	...
	pr_info("obmm_shmdev open: mem_id=%d pid=%d completed.\n", reg->regionid, current->pid);
...
static int obmm_shm_fops_release(struct inode *inode __always_unused, struct file *file)
{
	...
	pr_info("obmm_shmdev release: mem_id=%d pid=%d\n", reg->regionid, current->pid);
	put_obmm_region(reg);

	pr_info("obmm_shmdev release: mem_id=%d pid=%d completed.\n", reg->regionid, current->pid);
			   ...
			   enum obmm_mmap_granu mmap_granu)
{
	...
	pr_info_ratelimited("mmap region %d: size=%#llx\n", reg->regionid, reg->mem_size);
...
static void print_mmap_param(const struct file *file, const struct vm_area_struct *vma)
{
	...
	pr_info("obmm_shmdev mmap: mem_id=%d pid=%d vma=[%#lx, %#lx] pgoff=%#lx ", reg->regionid,
...
static int obmm_shm_fops_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	pr_info("obmm_shmdev mmap: mem_id=%d pid=%d vma=[%#lx, %#lx] mapped: mem_state=%#x.\n",
...
// Verify whether cache ops is valid.
...
{
	pr_info_ratelimited(
		"obmm_set_ownership: pid=%d va=[%#llx, %#llx) mem_state=%#x cache_ops=%#x\n",
				  ...
				  const struct obmm_cmd_update_range *update_info)
{
	...
	pr_info_ratelimited("obmm_set_ownership: completed.\n");
	return 0;
