...
static int setup_pa_prefilled(struct obmm_import_region *i_reg)
{
	int ret;
...
{
	return setup_pa_prefilled(i_reg);
}
...
static int teardown_remote_numa(struct obmm_import_region *i_reg, bool force)
{
	...
	pr_info("external called: remove_memory_remote, ret=%pe\n", ERR_PTR(ret));
			...
			unsigned long length, unsigned long cache_ops)
{
	...
	if (cache_ops == OBMM_SHM_CACHE_WB_INVAL)
