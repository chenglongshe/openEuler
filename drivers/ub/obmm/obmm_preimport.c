...
int preimport_prepare_common(struct preimport_range *pr, uint8_t base_dist)
{
	...
	pr_info("external called: add_memory_remote() returned %d\n", ret);
	if (ret < 0)
		return -EPERM;
...
err_remove_memory_remote:
	...
	pr_info("external called: remove_memory_remote() returned %d\n", ret_err);
	return ret;
}
...
int preimport_release_common(struct preimport_range *pr, bool force)
{
	...
	pr_info("external called: remove_memory_remote() returned %pe\n", ERR_PTR(ret));
	if (ret && !force)
		return ret;
...
{
	pr_info("obmm_preimport: pa=%#llx ", cmd->pa);
	pr_cont("length=%#llx scna=%#x dcna=%#x flags=%#llx nid=%d base_dist=%u deid="
		EID_FMT64 " seid=" EID_FMT64 " priv_len=%u\n",
		cmd->length, cmd->scna, cmd->dcna, cmd->flags, cmd->numa_id, cmd->base_dist,
		EID_ARGS64_H(cmd->deid), EID_ARGS64_L(cmd->deid), EID_ARGS64_H(cmd->seid),
		EID_ARGS64_L(cmd->seid), cmd->priv_len);
}
