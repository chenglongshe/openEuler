...
int ub_write_queue_flush(uint32_t scna)
{
	...
	pr_info_ratelimited("call external: ub_mem_drain(scna=%#x)\n", scna);
	...
	pr_info_ratelimited("external called: ub_mem_drain\n");
	return 0;
}
			    ...
			    uint8_t cache_ops)
{
	...
	if (cache_ops == OBMM_SHM_CACHE_NONE)
		return 0;
	pr_info_ratelimited("flush cache: region=%d, offset=0x%lx, length=0x%lx, cache_ops=%u\n",
