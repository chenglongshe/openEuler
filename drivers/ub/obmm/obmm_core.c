...
static void enable_obmm_region_get(struct obmm_region *region)
{
	refcount_set(&region->refcnt, 1);
}
void activate_obmm_region(struct obmm_region *region)
{
	enable_obmm_region_get(region);
}
...
static int obmm_addr_query(struct obmm_cmd_addr_query *cmd_addr_query)
{
	...
	if (cmd_addr_query->key_type == OBMM_QUERY_BY_PA) {
		pr_info_ratelimited("obmm_query_by_pa: pa=%#llx\n", cmd_addr_query->pa);
		...
		pr_info_ratelimited("obmm_query_by_id_offset: mem_id=%llu offset=%#llx\n",
