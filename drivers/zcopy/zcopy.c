			...
			unsigned long src_addr, int max_nr, int *rss)
{
	...
	if (unlikely(folio_test_large(src_folio) && max_nr != 1)) {
		...
		set_ptes(dst_vma->vm_mm, dst_addr, dst_ptep, pte, nr);
		return nr;
	}
	...
	set_ptes(dst_vma->vm_mm, dst_addr, dst_ptep, pte, 1);
	return 1;
}
					...
					int dst_pid, int src_pid, unsigned long size)
{
	...
	if (src_mm == dst_mm) {
		ret = -EINVAL;
		pr_err("Attach is not allowed within the same address space");
		goto put_dst_mm;
	}
...
static long zcopy_ioctl(struct file *file, unsigned int type, unsigned long ptr)
{
	...
	case IO_ATTACH:
	{
		...
		if (copy_from_user((void *)&ctx, (void *)ptr,
			...
			pr_err("copy from user for attach failed\n");
			ret = -EFAULT;
			break;
		}
...
static int register_device_zcopy(void)
{
	...
	if (ret) {
		pr_err("register_unexport_func failed\n");
		goto err_out;
	}
...
static int __init zcopy_init(void)
{
	...
	if (ret) {
		pr_err("register_device_zcopy failed\n");
		return -1;
	}
