...
int zxdh_mgr_par_get(struct zxdh_mgr *dh_mgr)
{
	...
	struct zxdh_mgr_msg *cmd =
		kzalloc(sizeof(struct zxdh_mgr_msg), GFP_KERNEL);
	struct zxdh_chan_msg *pmsg =
		kzalloc(sizeof(struct zxdh_chan_msg), GFP_KERNEL);
	...
	pr_info("qp_cnt=%d, cq_cnt=%d, srq_cnt=%d, ceq_cnt=%d, ah_cnt=%d, mr_cnt=%d, pbleq_cnt=%d, pblem_cnt=%d.\n",
	...
	pr_info("ird_hmc_base=0x%llx,ah_hmc_base=0x%llx,mr_hmc_base=0x%llx,pbleq_hmc_base=0x%llx,pblem_hmc_base=0x%llx.\n",
...
static void clean_bond_old_gid(struct ib_device *ibdev, struct net_device *primary_netdev){
	...
	for (i = 0; i < attr.gid_tbl_len; i++) {
		...
		if (ndev != NULL && ndev == primary_netdev) {
			pr_info("[rdma_bond] clean bond old gid ndev name=%s primary_netdev name=%s i=%d", ndev->name, primary_netdev->name, i);
			pr_info("[rdma_bond] gid.subnet_prefix=0x%llx, gid=%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x", gid_attr->gid.global.subnet_prefix,
			...
			pr_info("[rdma_bond] ndev and primary_netdev not equal: ndev name=%s primary_netdev name=%s i=%d", ndev->name, primary_netdev->name, i);
		}
		rdma_put_gid_attr(gid_attr);
	}
}
...
#else 
    pr_info("[rdma_bond] NETDEV_TO_IBDEV_SUPPORT is not defined\n");
    iwdev->netdev = new_netdev;
#endif
...
int32_t set_rdma_firmware_speed(struct net_device *netdev, uint32_t bps)
{
	...
	if (status_code != 200) {
		pr_info("[%s] status code not ok, status:%d", __func__,
			status_code);
		ib_device_put(ibdev);
		return -1;
	}
...
{
	pr_info("double plane: unregister_remote_ip_event_handler\n");
...
void rdma_update_remote_ip(struct zxdh_rdma_to_eth_ip_para *info)
{
	...
	if (info->ipv4 == true) {
		sprintf(s_straddr, "%pI4", &info->src_ip[3]);
		sprintf(d_straddr, "%pI4", &info->dst_ip[3]);
	} else {
		sprintf(s_straddr, "%pI6", info->src_ip);
		sprintf(d_straddr, "%pI6", info->dst_ip);
	}

	pr_info("%s[%d]: ipv4=%d, name=%s, s_straddr=%s, d_straddr=%s, src_ip=0x%x-0x%x-0x%x-0x%x, dst_ip=0x%x-0x%x-0x%x-0x%x, src_mac=0x%llx, dst_mac=0x%llx, mode=%d, linked_fid=0x%x\n",
		__func__, __LINE__, info->ipv4, info->ifname, s_straddr, d_straddr,info->src_ip[0], info->src_ip[1], info->src_ip[2], info->src_ip[3],
