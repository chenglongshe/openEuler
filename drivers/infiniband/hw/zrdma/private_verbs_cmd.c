...
extern u16 zxdh_get_rc_gqp_id(u16 qp_8k_index, u16 vhca_gqp_start,
			      u16 vhca_gqp_cnt);
...
#endif
{
	...
	case ZXDH_RESET_RETRY_TX_ITEM_FLAG:
		...
		break;
	default:
		pr_err("reset qp unknow opcode:%lld\n", reset_opcode);
		err_code = EINVAL;
		break;
	}
free_exit:
...
#endif
{
	...
	qpc_buf.va = NULL;
	qpc_buf.size = ALIGN(ZXDH_QP_CTX_SIZE, ZXDH_QPC_ALIGNMENT);
	qpc_buf.va = dma_alloc_coherent(iwdev->rf->hw.device, qpc_buf.size,
					&qpc_buf.pa, GFP_KERNEL);
	if (!qpc_buf.va) {
		pr_err("query qpc alloc dma failed:ENOMEM\n");
		return -ENOMEM;
	}
	err_code = zxdh_fill_qpc(&iwdev->rf->sc_dev, iwqp->sc_qp.qp_ctx_num, &qpc_buf);
	if (err_code) {
		pr_err("query qpc fill qpc failed:%d\n", err_code);
		dma_free_coherent(iwdev->rf->hw.device, qpc_buf.size,
				  qpc_buf.va, qpc_buf.pa);
		return -EFAULT;
	}
	copy_qpc_to_resp(qpc_buf.va, &resp);
	dma_free_coherent(iwdev->rf->hw.device, qpc_buf.size, qpc_buf.va,
			  qpc_buf.pa);
			   ...
			   struct zxdh_cap_cfg *cap_cfg)
{
	...
	u64 wqe_offset[RDMA_TX_CAP_WQE_MOD_NUM] = {
		RDMATX_CAP_NODE0_WQE_PRE_READ, RDMATX_CAP_NODE0_WQE_HANDLE,
			   ...
			   struct zxdh_cap_cfg *cap_cfg)
{
	...
	u8 node_select, node_choose, comapre_loop;
	u64 wqe_offset[RDMA_TX_CAP_WQE_MOD_NUM] = {
		RDMATX_CAP_NODE1_WQE_PRE_READ, RDMATX_CAP_NODE1_WQE_HANDLE,
...
static void clean_data_cap_buff(struct zxdh_sc_dev *dev, u64 size)
{
	...
	for (i = 0; i < numbufs; i++, j++) {
		memset(dev->data_cap_sd.entry[j].u.bp.addr.va, 0,
...
static int allocate_addr_for_data_cap(struct zxdh_device *iwdev, struct zxdh_ucontext *ucontext, struct zxdh_cap_addr_info *cap_addr_info, __u64 *cap_pa){
	...
	if (cap_addr_info->addr_info.cap_direct_dma_addr.cap_cpu_addr == NULL) {
		pr_err("allocate_addr_for_data_cap fail!\n");
		return -ENOMEM;
	}
	...
	if (cap_cfg->size > ZXDH_CAP_DATA_HOST_MEM_SIZE) {
		...
		if ((cap_cfg->cap_data_start_cap & 0x3) == 0x3) {
			if (mmap_len > (ZXDH_CAP_DATA_HMC_MEM_SIZE / 2)) {
				pr_err("prepare_addr_for_data_cap mmap_len:%llu too big!\n", mmap_len);
				return -EINVAL;
			}
			...
			*cap_iova_addr[NODE1] = dev->data_cap_sd.data_cap_base + mmap_len;
			...
			if (mmap_len > ZXDH_CAP_DATA_HMC_MEM_SIZE) {
				pr_err("prepare_addr_for_data_cap node0 mmap_len:%llu too big!\n", mmap_len);
				return -EINVAL;
			}
			*cap_iova_addr[NODE0] = dev->data_cap_sd.data_cap_base;
			...
			if (mmap_len > ZXDH_CAP_DATA_HMC_MEM_SIZE) {
				pr_err("prepare_addr_for_data_cap node1 mmap_len:%llu too big!\n", mmap_len);
				return -EINVAL;
			}
		...
		for (i=0; i<CAP_NODE_NUM; i++){
			if (*cap_iova_addr[i] != 0) {
				...
				if (!iwdev->hw_data_cap.cap_txrx_use_iova[i].entry_info.cap_mmap_entry) {
		...
		for (i = 0; i < CAP_NODE_NUM; i++) {
			if (cap_cfg->cap_data_start_cap & (1 << i)) {
				if (cap_position == CAP_TX) {
					if (allocate_addr_for_data_cap(iwdev, ucontext, &iwdev->hw_data_cap.cap_tx_use_direct_dma[i], &cap_pa[i]) != 0) {
						pr_err("zxdh_zalloc_mapped for tx node%u fail!\n", i);
						return -ENOMEM;
					}
				}else if (cap_position == CAP_RX) {
					if (allocate_addr_for_data_cap(iwdev, ucontext, &iwdev->hw_data_cap.cap_rx_use_direct_dma[i], &cap_pa[i]) != 0) {
...
#endif
{
	...
	if (cap_cfg.size > ZXDH_CAP_DATA_HOST_MEM_SIZE) {
		...
		if (iwdev->hw_data_cap.cap_txrx_use_iova[NODE0].addr_info.cap_iova_addr != 0 ||
			...
			pr_err("vhca_id:%u,cap_position:%u iova cap already start.\n", dev->vhca_id, cap_cfg.cap_position);
			return CAP_ALREADY_START;	
		}
	}else{
		if (cap_cfg.cap_position == CAP_TX) {
			READ_REGISTER_AND_CHECK(iwdev->rf, RDMATX_DATA_START_CAP, &reg_val);
			dma_addr[NODE0] = &iwdev->hw_data_cap.cap_tx_use_direct_dma[NODE0].addr_info.cap_direct_dma_addr.cap_dma_addr;
			dma_addr[NODE1] = &iwdev->hw_data_cap.cap_tx_use_direct_dma[NODE1].addr_info.cap_direct_dma_addr.cap_dma_addr;
			free_type = FREE_TYPE_TX;
		} else if (cap_cfg.cap_position == CAP_RX) {
			READ_REGISTER_AND_CHECK(iwdev->rf, RDMARX_DATA_START_CAP, &reg_val);
			dma_addr[NODE0] = &iwdev->hw_data_cap.cap_rx_use_direct_dma[NODE0].addr_info.cap_direct_dma_addr.cap_dma_addr;
			dma_addr[NODE1] = &iwdev->hw_data_cap.cap_rx_use_direct_dma[NODE1].addr_info.cap_direct_dma_addr.cap_dma_addr;
	...
	if ((cap_cfg.cap_data_start_cap & 0x1) == 0x1) {
		...
		if (cap_cfg.cap_position == CAP_TX) {
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_ADDR_LOW_NODE0,
				dma_addr_low);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_ADDR_HIGH_NODE0,
				dma_addr_high);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_LEN_LOW_NODE0,
				cap_cfg.size);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_LEN_HIGH_NODE0, 0);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMATX_CAP_VHCA_NUM_NODE0,
						 dev->vhca_id);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMATX_CAP_AXI_ID_NODE0, 0);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_CAP_ID_NODE0, cap_id);
		...
		if (cap_cfg.cap_position == CAP_RX) {
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_ADDR_LOW_NODE0,
				dma_addr_low);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_ADDR_HIGH_NODE0,
				dma_addr_high);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_LEN_LOW_NODE0,
				cap_cfg.size);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMARX_CAP_VHCA_NUM_NODE0,
						 dev->vhca_id);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMARX_CAP_AXI_ID_NODE0, 0);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_CAP_ID_NODE0, cap_id);
	...
	if ((cap_cfg.cap_data_start_cap & 0x2) == 0x2) {		
		...
		if (cap_cfg.cap_position == CAP_TX) {
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_ADDR_LOW_NODE1,
				dma_addr_low);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_ADDR_HIGH_NODE1,
				dma_addr_high);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_LEN_LOW_NODE1,
				cap_cfg.size);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_AXI_WR_LEN_HIGH_NODE1, 0);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMATX_CAP_VHCA_NUM_NODE1,
						 dev->vhca_id);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMATX_CAP_AXI_ID_NODE1, 0);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMATX_CAP_CAP_ID_NODE1, cap_id);
		...
		if (cap_cfg.cap_position == CAP_RX) {
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_ADDR_LOW_NODE1,
				dma_addr_low);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_ADDR_HIGH_NODE1,
				dma_addr_high);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_LEN_LOW_NODE1,
				cap_cfg.size);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_AXI_WR_LEN_HIGH_NODE1, 0);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMARX_CAP_VHCA_NUM_NODE1,
						 dev->vhca_id);
			WRITE_REGISTER_AND_CHECK(iwdev->rf,
						 RDMARX_CAP_AXI_ID_NODE1, 0);
			WRITE_REGISTER_AND_CHECK(
				iwdev->rf, RDMARX_CAP_CAP_ID_NODE1, cap_id);
	...
	if (cap_cfg.cap_position == CAP_TX) {
		WRITE_REGISTER_AND_CHECK(iwdev->rf, RDMATX_DATA_START_CAP,
					 cap_cfg.cap_data_start_cap);
	} else if (cap_cfg.cap_position == CAP_RX) {
		WRITE_REGISTER_AND_CHECK(iwdev->rf, RDMARX_DATA_START_CAP,
					 cap_cfg.cap_data_start_cap);
	}
...
#endif
{
	...
	if (cap_position == CAP_TX) {
		WRITE_REGISTER_AND_CHECK(iwdev->rf, RDMATX_DATA_START_CAP, 0);
	} else if (cap_position == CAP_RX) {
		WRITE_REGISTER_AND_CHECK(iwdev->rf, RDMARX_DATA_START_CAP, 0);
	} else {
...
{
	if (cap_addr_info->entry_info.cap_mmap_entry != NULL) {
		pr_info("free_mmap_addr rdma_user_mmap_entry_remove!\n");
#ifdef RDMA_MMAP_DB_SUPPORT
		rdma_user_mmap_entry_remove(
			cap_addr_info->entry_info.cap_mmap_entry);
#else
	...
	if (cap_addr_info->addr_info.cap_direct_dma_addr.cap_cpu_addr != NULL) {
		dma_free_coherent(iwdev->rf->sc_dev.hw->device,
					  ...
					  cap_addr_info->addr_info.cap_direct_dma_addr.cap_dma_addr);
		cap_addr_info->addr_info.cap_direct_dma_addr.cap_cpu_addr = NULL;
		pr_info("free_mmap_addr,length:%u!\n", length);
	}
}
...
#endif
	if (!cap_addr_info->entry_info.cap_mmap_entry) {
		pr_err("allocate_addr_for_mmap cap_mmap_entry insert err!\n");
		return -ENOMEM;
	}
	return 0;
}
...
#endif
{
	...
	if (mp_cap_cfg.cap_use_l2d) {
		...
		if(allocate_addr_for_mmap(iwdev, ucontext, length, &iwdev->hw_data_cap.mp_cap, &mp_cap_resp.cap_pa)){
	...
	for (i = 0; i < mp_cap_cfg.qpn_num; i++) {
		if (mp_cap_cfg.qpn[i] < (iwdev->rf->sc_dev.base_qpn + 1) ||
		    mp_cap_cfg.qpn[i] > (iwdev->rf->sc_dev.base_qpn +
					 iwdev->rf->max_qp - 1)) {
			pr_err("qpn:%u, base_qpn:%u overload",
			       mp_cap_cfg.qpn[i], iwdev->rf->sc_dev.base_qpn);
			return -EINVAL;
		}
...
#ifdef RDMA_MMAP_DB_SUPPORT
		rdma_user_mmap_entry_remove(
			iwdev->hw_data_cap.mp_cap.entry_info.cap_mmap_entry);
#else
		zxdh_user_mmap_entry_del_hash(
			iwdev->hw_data_cap.mp_cap.entry_info.cap_mmap_entry);
#endif
...
#endif
{
	...
	for (i = 0; i < cap_gqp.gqp_num; i++) {
		...
		if (cap_gqp.gqpid[i] <= GQP_ID_1023) {
			...
			reg_addr = (mp_idx * MP_OFFSET) + BASE_FOR_LITTLE_GQP +
				   (REG_BYTE * WRITE_RAM_REG_IDX);
			REG_OP_AND_CHECK(zxdh_rdma_reg_write, iwdev->rf, reg_addr, gqp_idx);
		...
		reg_addr = READ_RAM_REG_BASE + i * MP_OFFSET;
		/* e0b8:read ram address bak*/
		REG_OP_AND_CHECK(zxdh_rdma_reg_write, iwdev->rf, reg_addr, 0);
	}
...
#endif
{
	...
	for (gqp_id = gqp_start; gqp_id < gqp_start + gqp_cnt; gqp_id++) {
		REG_OP_AND_CHECK(zxdh_rdma_reg_write, iwdev->rf, 0x62065f81d4,
				 gqp_id);
		REG_OP_AND_CHECK(zxdh_rdma_reg_read, iwdev->rf, 0x62065f84c0,
				 &read_reg_val);
...
#endif
{
	...
	for (i = 0; i < 8; i++) {
		REG_OP_AND_CHECK(zxdh_rdma_reg_write, iwdev->rf,
				 C_RDMA_TX_SUB_RW_RSV0, i);
		REG_OP_AND_CHECK(zxdh_rdma_reg_read, iwdev->rf,
				 C_RDMA_TX_SUB_RO_RSV5, &read_reg_val);
	...
	REG_OP_AND_CHECK(zxdh_rdma_reg_read, iwdev->rf,
			 C_ACK_CPU_FIFO_OVERFLOW_CNT, &read_reg_val);
	...
	struct zxdh_dma_mem *resource_buf) 
{
	...
	if (ceqn < dev->base_ceqn || ceqn > (dev->base_ceqn + rf->max_cqe - 1)) {
		...
		pr_err("ceqn boundary :[%d,%d]\n", dev->base_ceqn, dev->base_ceqn + rf->max_cqe - 1);
		return -EOVERFLOW;
	}
...
#endif
{
	...
	resource_buf.va = dma_alloc_coherent(rf->hw.device,
					resource_buf.size, &resource_buf.pa,
					GFP_KERNEL);
	if (!resource_buf.va) {
		pr_err("get qpc alloc dma failed:ENOMEM\n");
		return -ENOMEM;
	}
...
pre_validate_hw_object_request(struct zxdh_get_object_data_req *object_req)
{
	...
	if (ret) {
		pr_err("query hw object, validate request object id err:%d, object_id: %d",
			       ...
			       struct zxdh_cqc_item *cqc_item)
{
	...
	resource_buf.va = dma_alloc_coherent(rf->hw.device, resource_buf.size,
					     &resource_buf.pa, GFP_KERNEL);
	if (!resource_buf.va) {
		pr_err("query cq shadow area, dma allocate err: ENOMEM\n");
		return -ENOMEM;
	}
	...
	cqc_item->root_pble = ZXDH_GET_QPC_ITEM(
		...
		RDMACPC_ROOT_PBLE);
	pr_info("query hw object, cqc leaf_pbl_size: 0x%x, doorbell_shadow_addr: 0x%llx, log_cqe_num: 0x%x, hw_cq_head: 0x%x,  cq_address: 0x%llx, root_pble: 0x%llx",
...
free_exit:
	if (resource_buf.va) {
		dma_free_coherent(rf->hw.device, resource_buf.size,
				  resource_buf.va, resource_buf.pa);
		resource_buf.va = NULL;
	}
	return ret;
			     ...
			     struct hw_object_wqe_context *object_context)
{
	...
	if (ret) {
		pr_err("query hw object, compute cq src address and length err:%d",
		       ret);
		return ret;
	}
				...
				struct zxdh_ceqc_item *ceqc_item)
{
	...
	resource_buf.va = dma_alloc_coherent(rf->hw.device, resource_buf.size,
					     &resource_buf.pa, GFP_KERNEL);
	if (!resource_buf.va) {
		pr_err("query ceqc, dma allocate err: ENOMEM\n");
		return -ENOMEM;
	}
				...
				struct zxdh_aeqc_item *aeqc_item)
{
	...
	pr_info("query hw object, aeqc aeq_head: 0x%x, leaf_pbl_size: 0x%x, virtually_mapped: 0x%x, aeq_size: 0x%x,  aeq_address: 0x%llx",
			     ...
			     struct hw_object_wqe_context *object_context)
{
	...
	if (qp_addr_ctx.addr_mode != 0 && object_req->object_num != 1) {
		pr_err("query qpc, When the addressing mode is virtual addressing, object_num can only be 1.object_num:%d\n",
		       object_req->object_num);
				...
				struct zxdh_srqc_item *srqc_item)
{
	...
	srqc_buf.va = dma_alloc_coherent(iwdev->rf->hw.device, srqc_buf.size,
...
free_exit:
	if (srqc_buf.va) {
		dma_free_coherent(iwdev->rf->hw.device, srqc_buf.size,
				  srqc_buf.va, srqc_buf.pa);
		srqc_buf.va = NULL;
	}
	return err_code;
}
	...
	struct zxdh_get_object_data_req *object_data_req)
{
	...
	if (!validate_allocate_buffer_size(hw_object_wqe_ctx)) {
		pr_err("query hw object, validate buffer: buffer size more than 2M 0x%u\n",
				...
				struct hw_object_wqe_context *object_wqe_ctx)
{
	...
	cqp_info->in.u.dma_writeread.src_path_index.inter_select =
		object_wqe_ctx->src_interface_select;
	pr_info("dma read hw object, src path index: vhca_id: %u, object id: %d, waypartion %d, path select %d, interface select %d\n",
		cqp_info->in.u.dma_writeread.src_path_index.vhca_id,
		cqp_info->in.u.dma_writeread.src_path_index.obj_id,
		cqp_info->in.u.dma_writeread.src_path_index.waypartion,
		cqp_info->in.u.dma_writeread.src_path_index.path_select,
	...
	pr_info("dma read hw object, dest path index: vhca_id: %u, object id: %d, waypartion %d, path select %d, interface select %d\n",
		cqp_info->in.u.dma_writeread.dest_path_index.vhca_id,
		cqp_info->in.u.dma_writeread.dest_path_index.obj_id,
		cqp_info->in.u.dma_writeread.dest_path_index.waypartion,
		cqp_info->in.u.dma_writeread.dest_path_index.path_select,
	...
	struct hw_object_wqe_context *object_wqe_ctx)
{
	...
	pr_info("query hw object response, object_mmap_offset: %llu, length %u, vhca_id: %u, route_id: %d, wqe_size: %u, srqp_aligned_offset: %llu\n",
...
#endif
{
	...
	if (iwdev->hw_data_cap.hw_object_mmap.addr_info.cap_direct_dma_addr
			    .cap_cpu_addr != NULL ||
	...
	ret = hw_object_calculate_wqe_context(dev, &object_data_req,
					      &object_wqe_ctx);
	if (ret) {
		pr_err("query hw object, caculate wqe context err: %d\n", ret);
		return ret;
	}
	...
	object_wqe_ctx.dest_address =
		iwdev->hw_data_cap.hw_object_mmap.addr_info.cap_direct_dma_addr
			.cap_dma_addr;
				...
				u64 reg_va, u16 *count_ret, u32 *reg_list, u64 *base_reg)
{
	...
	case 0x6330200:
		if (zxdh_rdma_reg_read(rf, reg_va, &value)) {
			pr_err("[check_health_err] read 0x633 reg value falied, reg_va: %llx\n", reg_va);
			(*count_ret) --;
		}
		break;
	default:
		...
		if (*base_reg == 0 || offset < 0 || offset >= MAX_READ_REG_SIZE) {
			if (zxdh_rdma_regs_read(rf, reg_va, reg_list, MAX_READ_REG_SIZE)) {
				pr_err("[check_health_err] read reg value falied, reg_va: %llx\n", reg_va);
				(*count_ret) --;
				return 0;
			}
			*base_reg = reg_va;
...
static int clean_all_gqps_mp_cap(struct zxdh_device *iwdev)
{
	...
	pr_info("clean_all_gqps_mp_cap gqp_num:%u", gqp_num);
	gqp_idx = 0xff;
	for (i = 0; i < gqp_num; i++) {
		mp_idx = 0;
		if (gqpid[i] <= GQP_ID_1023) {
			...
			mp_idx = ((gqpid[i] - GQP_OFFSET) / GQP_MOD) +
				 MP_IDX_INC;
			reg_addr = (mp_idx * MP_OFFSET) + BASE_FOR_LITTLE_GQP +
				   (REG_BYTE * WRITE_RAM_REG_IDX);
			REG_OP_AND_CHECK(zxdh_rdma_reg_write, iwdev->rf,
		     ...
		     struct zxdh_db_show_res_map_resp *show_res_map_resp)
{
	...
	for (j = 0; j < cnt_8k; j += MAX_READ_REG_SIZE) {
		...
		if (ret) {
			kfree(base_memory);
			pr_err("[get_8k_gqp_reg_value] zxdh_rdma_regs_read failed at chunk %d",
			       j / MAX_READ_REG_SIZE);
			return -EFAULT;
		}
	}
	...
	if (ret) {
		kfree(base_memory);
		pr_err("[get_8k_gqp_reg_value] zxdh_rdma_reg_read falied");
		return -EFAULT;
	}
			    ...
			    struct zxdh_db_show_res_map_resp *show_res_map_resp)
{
	...
	for (j = 0; j < gqp_cnt; j += MAX_READ_REG_SIZE) {
		chunk_size = (gqp_cnt - j) < MAX_READ_REG_SIZE ?
				     (gqp_cnt - j) :
				     MAX_READ_REG_SIZE;
		READ_REGISTERS_AND_CHECK(rf, reg_array[j], &value_array[j],
					 chunk_size);
	}

	idx[gqp_cnt] = vhca_ud_gqp;
	reg_array[gqp_cnt] = C_DB_SHOW_GQP_VHCA_MAP + vhca_ud_gqp * 0x4;
	READ_REGISTER_AND_CHECK(rf, reg_array[gqp_cnt], &value_array[gqp_cnt]);

	COPY_TO_USER_SAFE(show_res_map_req->reg_va, reg_array,
			  sizeof(u64) * (gqp_cnt + 1));
	COPY_TO_USER_SAFE(show_res_map_req->value_va, value_array,
			  sizeof(u32) * (gqp_cnt + 1));
	COPY_TO_USER_SAFE(show_res_map_req->idx_va, idx,
			  sizeof(u32) * (gqp_cnt + 1));
				 ...
				 u32 *num, u32 index, void *rf)
{
	...
	REG_OP_AND_CHECK(zxdh_rdma_reg_write, rf, C_CHECK_GQP_ACTIVE_WRITE,
			 index);
	REG_OP_AND_CHECK(zxdh_rdma_reg_read, rf, C_CHECK_GQP_ACTIVE_READ,
			 &read_reg_val);
	...
	if ((read_reg_val & qps_act_bit) != 0) {
		...
		reg_array[*num] = reg;
		idx[*num] = index;
		READ_REGISTER_AND_CHECK(rf, reg, &value_array[*num]);
		(*num)++;
		return 0;
	}
			    ...
			    struct zxdh_db_show_res_map_resp *show_res_map_resp)
{
	...
	COPY_TO_USER_SAFE(show_res_map_req->value_va, value_array,
			  sizeof(u32) * num);
	COPY_TO_USER_SAFE(show_res_map_req->idx_va, idx, sizeof(u32) * num);
		    ...
		    struct zxdh_db_show_res_map_resp *show_res_map_resp)
{
	...
	if (!iwqp) {
		pr_err("get_qp_8k_reg_value: iwqp is null!\n");
...
#endif
{
	...
	if (!reg_values) {
		pr_err("zxdh_read_ram error: allocate memory failed!\n");
		return -ENOMEM;
	}
