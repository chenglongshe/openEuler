...
int zxdh_alloc_ucontext(struct ib_ucontext *uctx, struct ib_udata *udata)
{
    ...
    rf = iwdev->rf;
    pr_debug("%s[%d]: req.userspace_ver=%d rf=0x%llx srq_l2d_base_paddr=0x%llx srq_l2d_size=0x%x\n", __func__, __LINE__,
        req.userspace_ver, (u64)rf, rf->srq_l2d_base_paddr, rf->srq_l2d_size);
	...
	if (req.userspace_ver != ZXDH_CONTEXT_VER_V1 && req.userspace_ver != ZXDH_CONTEXT_VER_V2) {
        pr_err("%s[%d]: Invalid userspace driver version detected. Detected version %d, should be %d or %d\n", __func__, __LINE__,
        ...
        pr_debug("%s[%d]: rdma kernel srq use ddr, but userspace driver not! userspace_ver=%d.\n", __func__, __LINE__, req.userspace_ver);
    } else if (req.userspace_ver == ZXDH_CONTEXT_VER_V1 && kernel_srq_use_l2d == true) {
        rf->rdma_srq_mem_type = USER_DDR_KERNEL_L2D;
        pr_debug("%s[%d]: rdma kernel srq use l2d, but userspace driver not! userspace_ver=%d.\n", __func__, __LINE__, req.userspace_ver);
    ...
    #else
        ucontext->srq_db_mmap_entry = zxdh_user_mmap_entry_add_hash(
            ucontext, srq_db_bar_off, ZXDH_MMAP_IO_NC,
            &uresp.srq_db_mmap_key);
    #endif /* RDMA_MMAP_DB_SUPPORT */
...
static void free_cap_mmap_entry(struct zxdh_cap_addr_info *cap_addr_info)
{
	if (cap_addr_info->entry_info.cap_mmap_entry != NULL) {
		pr_info("free_cap_mmap_entry rdma_user_mmap_entry_remove!\n");
#ifdef RDMA_MMAP_DB_SUPPORT
...
#endif
{
	...
	if (remote_ip_update_hook && (dual_tor_switch == ZXDH_DUAL_TOR_SWITCH_OPEN) && (iwqp->sc_qp.qp_uk.qp_type == ZXDH_QP_TYPE_ROCE_RC)) {
		if (av->sgid_addr.saddr.sa_family == AF_INET6) {
			sprintf(s_straddr, ", src_ip: %pI6", &av->sgid_addr.saddr_in6.sin6_addr);
			sprintf(d_straddr, ", dest_ip: %pI6", &av->dgid_addr.saddr_in6.sin6_addr);
		} else {
			sprintf(s_straddr, ", src_ip: %pI4", &av->sgid_addr.saddr_in.sin_addr);
			sprintf(d_straddr, ", dest_ip: %pI4", &av->dgid_addr.saddr_in.sin_addr);
		}
		ret = qp_remote_ip_info_process(ibqp, RDMA_DEL_REMOTE_IP);
		if (ret) {
			pr_err("%s[%d]: ipv4=%d, name=%s, op_type=%d, src_ip=0x%x-0x%x-0x%x-0x%x, dst_ip=0x%x-0x%x-0x%x-0x%x, src_port=0x%x, dst_port=0x%x, s_straddr=%s, d_straddr=%s\n",
				...
				udp_info->dest_ip_addr[0], udp_info->dest_ip_addr[1], udp_info->dest_ip_addr[2], udp_info->dest_ip_addr[3], udp_info->src_port, udp_info->dst_port, s_straddr, d_straddr);
		} else {
			pr_debug("%s[%d]: ipv4=%d, name=%s, op_type=%d, src_ip=0x%x-0x%x-0x%x-0x%x, dst_ip=0x%x-0x%x-0x%x-0x%x, src_port=0x%x, dst_port=0x%x, s_straddr=%s, d_straddr=%s\n",
				__func__, __LINE__, udp_info->ipv4, iwdev->netdev->name, RDMA_DEL_REMOTE_IP, udp_info->local_ipaddr[0], udp_info->local_ipaddr[1], udp_info->local_ipaddr[2], udp_info->local_ipaddr[3],
				udp_info->dest_ip_addr[0], udp_info->dest_ip_addr[1], udp_info->dest_ip_addr[2], udp_info->dest_ip_addr[3], udp_info->src_port, udp_info->dst_port, s_straddr, d_straddr);
		}
	}
...
#ifdef Z_DH_DEBUG_OPEN
#if 0
static void zxdh_print_hw_cqc(__le64 *cq_ctx)
{
	u64 temp;

	get_64bit_val(cq_ctx, 0, &temp);
	pr_info("scqe_break_moderation_en:0x%llx", FIELD_GET(BIT_ULL(6), temp));
	pr_info("cq_period:0x%llx", FIELD_GET(GENMASK_ULL(17, 7), temp));
	pr_info("cq_max:0x%llx", FIELD_GET(GENMASK_ULL(33, 18), temp));
	pr_info("vHCA:0x%llx", FIELD_GET(GENMASK_ULL(43, 34), temp));
	pr_info("cqc_debug_set:0x%llx", FIELD_GET(GENMASK_ULL(53, 44), temp));
	pr_info("CQ_OVERFLOW_CHECK_En:0x%llx", FIELD_GET(BIT_ULL(54), temp));
	pr_info("Leaf_PBL_Size:0x%llx", FIELD_GET(GENMASK_ULL(56, 55), temp));
	pr_info("cq_pble_axi_error_locked_flag:0x%llx",
		FIELD_GET(BIT_ULL(57), temp));
	pr_info("cqe_size:0x%llx", FIELD_GET(BIT_ULL(58), temp));
	pr_info("cq_overflow_locked_flag:0x%llx", FIELD_GET(BIT_ULL(59), temp));
	pr_info("CQ_State:0x%llx", FIELD_GET(GENMASK_ULL(63, 60), temp));

	get_64bit_val(cq_ctx, 8, &temp);
	pr_info("Doorbell_Shadow_Addr:0x%llx",
		FIELD_GET(GENMASK_ULL(57, 0), temp));

	get_64bit_val(cq_ctx, 16, &temp);
	pr_info("CQ_Shadow_Read_Threshold:0x%llx",
		FIELD_GET(GENMASK_ULL(15, 0), temp));
	pr_info("log_CQE_NUM:0x%llx", FIELD_GET(GENMASK_ULL(28, 24), temp));
	pr_info("is_in_list_cnt:0x%llx", FIELD_GET(GENMASK_ULL(45, 33), temp));
	pr_info("cq_st:0x%llx", FIELD_GET(GENMASK_ULL(47, 46), temp));
	pr_info("CEQ_ID:0x%llx", FIELD_GET(GENMASK_ULL(59, 48), temp));

	get_64bit_val(cq_ctx, 24, &temp);
	pr_info("arm_valid:0x%llx", FIELD_GET(BIT_ULL(4), temp));
	pr_info("hw_arm_next:0x%llx", FIELD_GET(BIT_ULL(5), temp));
	pr_info("hw_arm_seq_num:0x%llx", FIELD_GET(GENMASK_ULL(7, 6), temp));
	pr_info("hw_cq_select:0x%llx", FIELD_GET(GENMASK_ULL(17, 12), temp));
	pr_info("hw_cq_tail:0x%llx", FIELD_GET(GENMASK_ULL(40, 18), temp));
	pr_info("hw_cq_head:0x%llx", FIELD_GET(GENMASK_ULL(63, 41), temp));

	get_64bit_val(cq_ctx, 32, &temp);
	pr_info("CQ_Address:0x%llx", FIELD_GET(GENMASK_ULL(55, 0), temp));

	get_64bit_val(cq_ctx, 40, &temp);
	pr_info("CQ_Context_Value:0x%llx", FIELD_GET(GENMASK_ULL(62, 0), temp));

	get_64bit_val(cq_ctx, 48, &temp);
	pr_info("Leaf_PBLE:0x%llx", FIELD_GET(GENMASK_ULL(27, 0), temp));
	pr_info("cqe_next_cnt:0x%llx", FIELD_GET(GENMASK_ULL(43, 28), temp));
	pr_info("cqe_se_cnt:0x%llx", FIELD_GET(GENMASK_ULL(59, 44), temp));

	get_64bit_val(cq_ctx, 56, &temp);
	pr_info("Root_PBLE:0x%llx", FIELD_GET(GENMASK_ULL(51, 0), temp));
}
#endif
int zxdh_query_cqc(struct zxdh_sc_cq *cq)
{
	...
	cqp_info->in.u.query_cqc.cqn = cq->cq_uk.cq_id;
	...
	get_64bit_val(cqc_buf.va, 0, &temp);
	cq->cq_overflow_locked_flag = FIELD_GET(BIT_ULL(59), temp);
#if 0
	pr_info("***CQ %d HW CQC info print start***\n", cq->cq_uk.cq_id);
	zxdh_print_hw_cqc(cqc_buf.va);
	pr_info("****CQ %d HW CQC info print end****\n", cq->cq_uk.cq_id);
#endif
free_rsrc:
	dma_free_coherent(dev->hw->device, cqc_buf.size, cqc_buf.va,
			  cqc_buf.pa);
	cqc_buf.va = NULL;
	return err_code;
}

static void zxdh_print_hw_ceqc(__le64 *ceq_ctx)
{
	u64 temp;

	get_64bit_val(ceq_ctx, 0, &temp);
	pr_info("ceq_period[2:0]:0x%llx", FIELD_GET(GENMASK_ULL(2, 0), temp));
	pr_info("vHCA:0x%llx", FIELD_GET(GENMASK_ULL(12, 3), temp));
	pr_info("INT_num:0x%llx", FIELD_GET(GENMASK_ULL(30, 13), temp));
	pr_info("INT_type:0x%llx", FIELD_GET(BIT_ULL(31), temp));
	pr_info("CEQE_Head:0x%llx", FIELD_GET(GENMASK_ULL(52, 32), temp));
	pr_info("CEQE_Valid:0x%llx", FIELD_GET(BIT_ULL(53), temp));
	pr_info("Leaf_PBL_Size:0x%llx", FIELD_GET(GENMASK_ULL(55, 54), temp));
	pr_info("CEQ_size:0x%llx", FIELD_GET(GENMASK_ULL(57, 56), temp));
	pr_info("log_CEQ_num:0x%llx", FIELD_GET(GENMASK_ULL(62, 58), temp));
	pr_info("CEQ_State:0x%llx", FIELD_GET(BIT_ULL(63), temp));

	get_64bit_val(ceq_ctx, 8, &temp);
	pr_info("CEQ_Address:0x%llx", FIELD_GET(GENMASK_ULL(56, 0), temp));
	pr_info("ceq_period[9:3]:0x%llx", FIELD_GET(GENMASK_ULL(63, 57), temp));

	get_64bit_val(ceq_ctx, 16, &temp);
	pr_info("ceq_max_cnt:0x%llx", FIELD_GET(GENMASK_ULL(15, 0), temp));
	pr_info("ceq_axi_rsp_err_flag:0x%llx", FIELD_GET(BIT_ULL(16), temp));
}

int zxdh_query_ceqc(struct zxdh_sc_ceq *ceq)
{
	struct zxdh_sc_dev *dev = ceq->dev;
	struct zxdh_pci_f *rf = container_of(dev, struct zxdh_pci_f, sc_dev);
	struct zxdh_dma_mem ceqc_buf;
	struct zxdh_cqp_request *cqp_request;
	struct cqp_cmds_info *cqp_info;
	int err_code = 0;
	int status;
	...
	ceqc_buf.va = NULL;
	ceqc_buf.size = ALIGN(ZXDH_CEQ_CTX_SIZE, ZXDH_CEQC_ALIGNMENT);
	ceqc_buf.va = dma_alloc_coherent(dev->hw->device, ceqc_buf.size,
					 &ceqc_buf.pa, GFP_KERNEL);
	if (!ceqc_buf.va)
		return -ENOMEM;
	...
	cqp_info->in.u.query_ceqc.ceqn = ceq->ceq_id;
	...
	pr_info("***CEQ %d HW CEQC info print start***\n", ceq->ceq_id);
	zxdh_print_hw_ceqc(ceqc_buf.va);
	pr_info("****CEQ %d HW CEQC info print end****\n", ceq->ceq_id);

free_rsrc:
	dma_free_coherent(dev->hw->device, ceqc_buf.size, ceqc_buf.va,
			  ceqc_buf.pa);
	ceqc_buf.va = NULL;
	return err_code;
}

static void zxdh_print_hw_aeqc(__le64 *aeq_ctx)
{
	u64 temp;

	get_64bit_val(aeq_ctx, 0, &temp);
	pr_info("Intr_ID:0x%llx", FIELD_GET(GENMASK_ULL(11, 0), temp));
	pr_info("AEQ_Head:0x%llx", FIELD_GET(GENMASK_ULL(34, 13), temp));
	pr_info("Leaf_PBL_Size:0x%llx", FIELD_GET(GENMASK_ULL(36, 35), temp));
	pr_info("Virtually_Mapped:0x%llx", FIELD_GET(BIT_ULL(37), temp));
	pr_info("AEQ_Size:0x%llx", FIELD_GET(GENMASK_ULL(59, 38), temp));
	pr_info("AEQ_State:0x%llx", FIELD_GET(GENMASK_ULL(63, 60), temp));

	get_64bit_val(aeq_ctx, 8, &temp);
	pr_info("AEQ_Address:0x%llx", FIELD_GET(GENMASK_ULL(63, 0), temp));
}

int zxdh_query_aeqc(struct zxdh_sc_aeq *aeq)
{
	struct zxdh_sc_dev *dev = aeq->dev;
	struct zxdh_pci_f *rf = container_of(dev, struct zxdh_pci_f, sc_dev);
	struct zxdh_dma_mem aeqc_buf;
	struct zxdh_cqp_request *cqp_request;
	struct cqp_cmds_info *cqp_info;
	int err_code = 0;
	int status;
	...
	aeqc_buf.va = NULL;
	aeqc_buf.size = ALIGN(ZXDH_CEQ_CTX_SIZE, ZXDH_CEQC_ALIGNMENT);
	aeqc_buf.va = dma_alloc_coherent(dev->hw->device, aeqc_buf.size,
					 &aeqc_buf.pa, GFP_KERNEL);
	if (!aeqc_buf.va)
		return -ENOMEM;
	...
	pr_info("***VHCA %d HW AEQC info print start***\n", dev->vhca_id);
	zxdh_print_hw_aeqc(aeqc_buf.va);
	pr_info("****VHCA %d HW CEQC info print end****\n", dev->vhca_id);

free_rsrc:
	dma_free_coherent(dev->hw->device, aeqc_buf.size, aeqc_buf.va,
			  aeqc_buf.pa);
	aeqc_buf.va = NULL;
	return err_code;
}
