				 ...
				 struct zxdh_user_mmap_entry *entry)
{
	...
	pr_info("zxdh_mmap_hmc_for_cap remap_pfn_range end.start:%lx,size:%llx\n",
		vma->vm_start, size);
	return ret;
}
			...
			struct ib_qp_init_attr *init_attr)
{
	...
	if (iwqp->is_srq == false)
		size = sqdepth * ZXDH_QP_SQ_WQE_MIN_SIZE +
		       rqdepth * ZXDH_QP_RQ_WQE_MIN_SIZE;
	else
		size = sqdepth * ZXDH_QP_SQ_WQE_MIN_SIZE;
...
static u16 zxdh_get_udp_sport(const struct rdma_ah_attr *ah, u32 src_qp_num, u32 dest_qp_num)
{
	...
	pr_debug("%s[%d]: flow_label=%d, sport=%d, sport_offset=%d\n", __func__, __LINE__, flow_label, sport, sport_offset);
    return sport;
#else
    if (!flow_label) {
        // Without flow label
        sport = (u16)(sport_offset + ZRDMA_UDP_SPORT_BASE);
        sport_offset = (sport_offset + 1) % ZRDMA_UDP_SPORT_NUM;
        return sport;
    } else {
        // With flow label
...
int qp_remote_ip_info_process(struct ib_qp *ibqp, int op_type)
{
    ...
    pr_debug("%s[%d]: dev_addr=%pM, src_mac_addr=%x-%x-%x-%x-%x-%x\n", __func__, __LINE__, iwdev->netdev->dev_addr, src_mac_addr[0], src_mac_addr[1], src_mac_addr[2], src_mac_addr[3], src_mac_addr[4], src_mac_addr[5]);
	pr_debug("%s[%d]: ipv4=%d, name=%s, src_ip=0x%x-0x%x-0x%x-0x%x, dst_ip=0x%x-0x%x-0x%x-0x%x, src_port=0x%x, dst_port=0x%x, src_mac=0x%llx, dst_mac=0x%llx\n",
			...
			int attr_mask, struct ib_udata *udata)
{
	...
	if (attr_mask & IB_QP_AV) {
		...
		if (attr->ah_attr.ah_flags & IB_AH_GRH) {
			...
			udp_info->src_port = zxdh_get_udp_sport(
				&attr->ah_attr, iwqp->sc_qp.qp_uk.qp_id,
				attr->dest_qp_num);
		...
		if (vlan_id < VLAN_N_VID) {
			...
			udp_info->vlan_tag =
				vlan_id | ctx_info->user_pri << VLAN_PRIO_SHIFT;
		} else {
        ...
        if (remote_ip_update_hook && (dual_tor_switch == ZXDH_DUAL_TOR_SWITCH_OPEN)) {
            ...
            if (ret) {
                pr_err("%s[%d]: ipv4=%d, name=%s, op_type=%d, src_ip=0x%x-0x%x-0x%x-0x%x, dst_ip=0x%x-0x%x-0x%x-0x%x, src_port=0x%x, dst_port=0x%x\n",
	...
	iwqp->sc_qp.qp_uk.qp_8k_index =
		zxdh_get_8k_index(&iwqp->sc_qp, udp_info->dest_ip_addr[3]);
			    ...
			    bool use_pbles, bool pble_type)
{
	...
	if (use_pbles) {
		...
		status = zxdh_get_pble(pble_rsrc_com, palloc, iwmr->page_cnt,
				       b_level1_only);
		if (status)
		{
				      ...
				      struct ib_udata *udata)
{
	...
	if (IS_ERR(region)) {
		pr_err("%s[%d] VERBS: Failed to create ib_umem region, errno=%ld, start=0x%llx, len=0x%llx, access=0x%x\n", __func__, __LINE__, PTR_ERR(region), start, len, access);
	...
	if (!iwmr) {
        pr_err("%s[%d]: kzalloc failed, size=0x%lx\n", __func__, __LINE__, sizeof(*iwmr));
		ib_umem_release(region);
 ...
 */
static inline int zxdh_poll_one(struct zxdh_cq_uk *ukcq,
...
static const struct ib_device_ops zxdh_dev_ops = {
	...
	defined(RHEL_8_3) || defined(RHEL_8_4) || defined(RHEL_8_5) || defined(KYLIN_V10_4) || \
...
static void zxdh_set_device_ops(struct ib_device *ibdev)
{
	...
	defined(RHEL_8_5) || defined(KYLIN_V10_4)
...
#if KERNEL_VERSION(5, 11, 0) <= LINUX_VERSION_CODE || defined(RHEL_8_5) || \
...
int zxdh_ib_register_device(struct zxdh_device *iwdev)
{
	...
	strlcpy(iwdev->ibdev.name, "zrdma%d", IB_DEVICE_NAME_MAX);
...
#endif
	ret = kc_ib_register_device(&iwdev->ibdev, "zrdma%d",
				    iwdev->rf->hw.device);
	if (ret)
		goto error;
