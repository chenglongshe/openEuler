			    ...
			    struct zxdh_srq_init_info *info)
{
    ...
    if (rf->rdma_srq_mem_type == USER_L2D_KERNEL_L2D) {
        info->srq_db_pa = rf->srq_l2d_base_paddr + ((info->srq_uk_init_info.srq_id - dev->base_srqn) % ZXDH_PF_MAX_SRQ_NUM_USE_L2D) * 8;
        pr_debug("%s[%d]: srq use L2D memory! l2d_base_paddr=0x%llx srq_db_pa=0x%llx srq_id=%d ep_id=%d pf_id=%d vf_id=%d ftype=%d\n", __func__, __LINE__,
            rf->srq_l2d_base_paddr, info->srq_db_pa, info->srq_uk_init_info.srq_id, rf->ep_id, rf->pf_id, rf->vf_id, rf->ftype);
    } else {
        info->srq_db_pa = 0;
