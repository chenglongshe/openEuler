...
static int ah_remote_ip_info_process(struct zxdh_device *iwdev, struct zxdh_ah_info *ah_info, int op_type)
{
	...
	pr_debug("double plane %s[%d]: ipv4=%d, op_type=%d, src_ip=0x%x-0x%x-0x%x-0x%x, dst_ip=0x%x-0x%x-0x%x-0x%x, dst_mac=0x%llx, dmac=%x-%x-%x-%x-%x-%x\n",
	...
	pr_debug("double plane %s[%d]: src_mac=0x%llx, smac=%x-%x-%x-%x-%x-%x\n",
