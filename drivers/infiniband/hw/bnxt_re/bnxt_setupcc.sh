...
Usage: $(basename "$0") [OPTION]...
  ...
  -i          Ethernet Interface Name
	      (e.g. p1p1 or for bond, specify slave interfaces like -i p6p1 -i p6p2)
  -m [1-3]    1 - PFC only
	      2 - CC only
	      3 - PFC + CC mode
...
if test -f "/sys/kernel/debug/bnxt_re/$DEV_NAME/info"; then
	CNP_SERVICE_TYPE= \
`cat /sys/kernel/debug/bnxt_re/$DEV_NAME/info|
	grep fw_service_prof_type_sup|awk '{print $3}'`
fi
...
bnxt_qos_rem_app_tlvs() {
	INF_NAME=$1
	j=0
	for i in `bnxtqos -dev=$INF_NAME get_qos|grep -e "Priority:" -e
		"Sel:" -e DSCP -e UDP -e "Ethertype:"|awk -F":" '{ print $2}'`
	do
		if [ $i == 0x8915 ]
		then
			i=35093
