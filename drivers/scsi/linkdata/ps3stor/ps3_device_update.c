				...
				struct scsi_device *sdev)
{
	...
	if (p_priv_data != NULL) {
		...
		INJECT_START_AT_TIME_WAIT_REV_PRE(
						 ...
						 struct PS3Dev *dev)
{
	...
	INJECT_START(PS3_ERR_IJ_FORCE_VD_COUNT_ERR,
		     &instance->dev_context.vd_info_buf_sync->count);
				...
				struct ps3_pd_entry *local_entry)
{
	...
	INJECT_START_AT_TIME_WAIT_REV_PRE(PS3_ERR_IJ_PD_ATTR_WAIT_OS_SCAN,
					  PS3_ERR_IJ_OS_SCAN, NULL);
