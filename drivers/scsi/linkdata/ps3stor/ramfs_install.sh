...
if [ ! ${isload} ]; then
	...
	if [ ${kernel} == "5.10.134-16.2.an8.x86_64" ]; then
		imageDir="/mnt/sysimage/usr/lib/modules/4.19.91-27.8.an8.x86_64/kernel/drivers/scsi"
	fi
	if [ ${kernel} == "6.6.34-9.oc9.x86_64" ]; then
		imageDir="/mnt/sysimage/usr/lib/modules/6.6.47-12.oc9.x86_64/kernel/drivers/scsi"
	fi
