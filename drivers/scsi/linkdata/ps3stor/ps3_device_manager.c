...
static int ps3_vd_buff_alloc(struct ps3_instance *instance)
{
	...
	for (i = 0; i < PS3_VD_TABLE_NUM; i++) {
		...
		INJECT_START(PS3_ERR_IJ_PS3_VD_IDX_ARRAY_ALLOC,
			     &p_vd_table[i].vd_idxs_array);
		...
		INJECT_START(PS3_ERR_IJ_PS3_VD_ENTRIES_ARRAY_ALLOC,
			     &p_dev_ctx->vd_entries_array[i]);
		if (p_dev_ctx->vd_entries_array[i] == NULL) {
			LOG_ERROR(
	...
	INJECT_START(PS3_ERR_IJ_PS3_VD_DEVS_BUF_ALLOC, &vd_pool->devs_buffer);
	...
	p_vd_pri_data_table->vd_pri_data_idxs_array =
		(struct ps3_scsi_priv_data **)ps3_vzalloc(
			instance, sizeof(struct ps3_scsi_priv_data *) *
					  p_dev_ctx->total_vd_count);
	INJECT_START(PS3_ERR_IJ_PS3_VD_PRI_DATA_IDXS_ARRAY_ALLOC,
		     &p_vd_pri_data_table->vd_pri_data_idxs_array);
...
static int ps3_pd_buff_alloc(struct ps3_instance *instance)
{
	...
	INJECT_START(PS3_ERR_IJ_PS3_PD_ENTRIES_ARRAY_ALLOC,
		     &p_dev_ctx->pd_entries_array);
	if (p_dev_ctx->pd_entries_array == NULL) {
	...
	INJECT_START(PS3_ERR_IJ_PS3_PD_IDX_ARRAY_ALLOC,
		     &pd_table->pd_idxs_array);
	...
	INJECT_START(PS3_ERR_IJ_PS3_PD_DEVS_BUF_ALLOC, &pd_pool->devs_buffer);
			    ...
			    unsigned short pd_id)
{
	...
	INJECT_START(PS3_ERR_IJ_FORCE_MOD_DISKPOS1,
		     &pd_info->diskPos.diskDev.diskID);
...
int ps3_dev_mgr_vd_info_unsubscribe(struct ps3_instance *instance)
{
	...
	INJECT_START(PS3_ERR_IJ_WAIT_SUSPEND_VD_PENDING_FAILED, &ret);
	if (ret != PS3_SUCCESS) {
					      ...
					      unsigned short target_id)
{
	...
	INJECT_START(PS3_ERR_IJ_FORCE_MOD_DISKPOS,
		     &p_entry->diskPos.diskDev.diskID);
						...
						unsigned short target_id)
{
	...
	INJECT_START(PS3_ERR_IJ_FORCE_MOD_DISKPOS,
		     &p_entry->disk_pos.diskDev.diskID);
...
int ps3_scsi_slave_alloc(struct scsi_device *sdev)
{
	...
	sdev->hostdata = NULL;
	INJECT_START_AT_TIME_WAIT_REV_PRE(PS3_ERR_IJ_GET_PRIV_DATA_DELAY,
					  PS3_ERR_IJ_FORCE_WAIT, sdev);
