...
unload_auto(){
	...
	if [ "$install_mode" == "auto_rpm" ] ; then
		if lsinitrd "$ramfs_file" | grep -q "ps3stor" ; then
			...
			echo "The ps3stor.ko has been uninstalled."
			echo "Please execute ./compile -auto or install the RPM/DEB driver package promptly."
			echo "Keep system security and prevent unexpected power loss before the new driver is successfully installed."
		fi
	elif [ "$install_mode" == "auto_deb" ] ; then
...
case "$unload_module" in
	rpm)
		if [ "$install_mode" == "pkg_rpm" ]; then
			...
			echo "The RPM packages has been remove."
			echo "Please execute ./compile -auto or install the RPM/DEB driver package promptly."
			echo "Keep system security and prevent unexpected power loss before the new driver is successfully installed."
		else
	...
	deb)
		if [ "$install_mode" == "pkg_deb" ]; then
			...
			echo "The DEB packages has been remove."
			echo "Please execute ./compile -auto or install the RPM/DEB driver package promptly."
			echo "Keep system security and prevent unexpected power loss before the new driver is successfully installed."
		else
