...
static inline void traceIdContextIdSet(unsigned long long *traceId,
				       unsigned short contextId)
{
	*traceId &= ~(TRACE_ID_CHIP_IN_CONTEXTID_MASK
		      << TRACE_ID_CHIP_IN_CONTEXTID_SHIFT);
	*traceId |= ((unsigned long long)contextId &
		     TRACE_ID_CHIP_IN_CONTEXTID_MASK)
		    << TRACE_ID_CHIP_IN_CONTEXTID_SHIFT;
}
