...
static struct PS3Inject g_inject_list;
static struct PS3HitCmd g_hit_cmd_list;
...
void ps3_inject_list_exit(void)
{
	...
	list_for_each_entry_safe(pitem, pitem_next,
				  &g_inject_list.scsi_rw_list, list) {
		list_del(&pitem->list);
		kfree(pitem);
	}

	list_for_each_entry_safe(pitem, pitem_next,
				  &g_inject_list.scsi_task_list, list) {
		list_del(&pitem->list);
		kfree(pitem);
	}

	list_for_each_entry_safe(pitem, pitem_next, &g_inject_list.mgr_list,
				  list) {
		list_del(&pitem->list);
		kfree(pitem);
	}
...
void ps3_hit_cmd_list_exit(void)
{
	...
	ps3_mutex_lock(&g_inject_list.lock);
	list_for_each_entry_safe(pitem, pitem_next,
				  &g_hit_cmd_list.scsi_rw_list, list) {
		list_del(&pitem->list);
		kfree(pitem);
	}

	list_for_each_entry_safe(pitem, pitem_next,
				  &g_hit_cmd_list.scsi_task_list, list) {
		list_del(&pitem->list);
		kfree(pitem);
	}

	list_for_each_entry_safe(pitem, pitem_next, &g_hit_cmd_list.mgr_list,
				  list) {
		list_del(&pitem->list);
		kfree(pitem);
	}
...
unsigned char ps3_add_scsi_rw_cmd_filter(struct ps3_instance *instance,
					 struct PS3CmdWord *cmd_word)
{
	...
	p_cmd = kmalloc(sizeof(struct inject_hit_cmds_t), GFP_KERNEL);
	if (p_cmd == NULL) {
		is_find = PS3_FALSE;
		goto l_out;
	}
...
unsigned char ps3_add_scsi_task_cmd_filter(struct ps3_instance *instance,
					   struct PS3CmdWord *cmd_word)
{
	struct ps3_cmd *cmd;
	struct ps3_cmd *abort_cmd;
	unsigned short abort_cmd_id = 0;
	struct inject_cmds_t *pitem = NULL;
	struct inject_cmds_t *pitem_next = NULL;
	struct inject_hit_cmds_t *p_cmd = NULL;
	unsigned char is_find = PS3_FALSE;
	...
	if (cmd == NULL)
		return is_find;
	if (cmd->req_frame->taskReq.reqHead.cmdSubType ==
		    PS3_TASK_CMD_SCSI_TASK_ABORT &&
	    cmd->req_frame->taskReq.reqHead.cmdType ==
		...
		LOG_DEBUG("[inject] {%u:%u:%u} CFID[%u], abort CFID[%u]!\n",
			  abort_cmd->instance->host->host_no,
			  abort_cmd->scmd->device->channel,
			  abort_cmd->scmd->device->id, cmd->index,
			  abort_cmd_id);

		list_for_each_entry_safe(pitem, pitem_next,
					  &g_inject_list.scsi_task_list, list) {
			if ((abort_cmd->instance->host->host_no ==
			     pitem->item.scsi_task_cmd.host_no) &&
			    (abort_cmd->scmd->device->channel ==
			     pitem->item.scsi_task_cmd.channel) &&
			    (abort_cmd->scmd->device->id ==
			     pitem->item.scsi_task_cmd.id)) {
				if (cmd->req_frame->taskReq.reqHead.cmdType ==
					    pitem->item.scsi_task_cmd.cmd_type &&
				    cmd->req_frame->taskReq.reqHead.cmdSubType ==
					    pitem->item.scsi_task_cmd
						    .cmd_sub_type) {
					LOG_DEBUG(
						"[inject] add abort [%u:%u:%u] type[0x%x], subType[0x%x], CFID[%u], abort CFID[%u]!\n",
						pitem->item.scsi_task_cmd
							.host_no,
						pitem->item.scsi_task_cmd
							.channel,
						...
						pitem->item.scsi_task_cmd
							.cmd_sub_type,
						cmd->index, abort_cmd_id);
					is_find = PS3_TRUE;
					if (pitem->item.scsi_task_cmd.dealType ==
					    PS3_SCSI_TASK_CMD_TIMEOUT) {
					} else if (pitem->item.scsi_task_cmd
							   .dealType ==
						   PS3_SCSI_TASK_CMD_ERROE) {
						INJECT_ACTIVE(
	...
	} else if (cmd->req_frame->taskReq.reqHead.cmdSubType ==
		...
		list_for_each_entry_safe(pitem, pitem_next,
					  &g_inject_list.scsi_task_list, list) {
			scsi_priv_data = PS3_SDEV_PRI_DATA(
				pitem->item.scsi_task_cmd.device);
			if (cmd->req_frame->taskReq.reqHead.cmdType ==
				    pitem->item.scsi_task_cmd.cmd_type &&
			    cmd->req_frame->taskReq.reqHead.cmdSubType ==
				    pitem->item.scsi_task_cmd.cmd_sub_type &&
			    (cmd->req_frame->taskReq.reqHead.devID.diskID ==
			     scsi_priv_data->disk_pos.diskDev.diskID)) {
				LOG_DEBUG(
					"[inject] add reset [%u:%u:%u] type[0x%x], subType[0x%x], CFID[%u]!\n",
					pitem->item.scsi_task_cmd.host_no,
					pitem->item.scsi_task_cmd.channel,
					pitem->item.scsi_task_cmd.id,
					pitem->item.scsi_task_cmd.cmd_type,
				...
				if (pitem->item.scsi_task_cmd.dealType ==
				    PS3_SCSI_TASK_CMD_TIMEOUT) {
				} else if (pitem->item.scsi_task_cmd.dealType ==
					   PS3_SCSI_TASK_CMD_ERROE) {
					INJECT_ACTIVE(
...
unsigned char ps3_add_mgr_cmd_filter(struct ps3_instance *instance,
				     struct PS3CmdWord *cmd_word)
{
	struct ps3_cmd *cmd;
	struct inject_cmds_t *pitem = NULL;
	...
	if (cmd->req_frame->taskReq.reqHead.cmdType == PS3_CMD_SAS_MANAGEMENT ||
		...
		LOG_DEBUG(
			"[inject] hostno[%u], mgr CFID[%u], cmdType[%u], cmdSubType[%u]!\n",
			cmd->instance->host->host_no, cmd->index,
			cmd->req_frame->taskReq.reqHead.cmdType,
			cmd->req_frame->taskReq.reqHead.cmdSubType);

		list_for_each_entry_safe(pitem, pitem_next,
					  &g_inject_list.mgr_list, list) {
			if ((cmd->instance->host->host_no ==
				...
				LOG_DEBUG(
					"[inject] add mgr type[0x%x], subType[0x%x], CFID[%u]!\n",
					pitem->item.mgr_cmd.cmd_type,
					pitem->item.mgr_cmd.cmd_sub_type,
					cmd->index);
				if (pitem->item.mgr_cmd.dealType ==
				    PS3_MGR_CMD_TIMEOUT) {
				} else if (pitem->item.mgr_cmd.dealType ==
					   PS3_SCSI_TASK_CMD_ERROE) {
					INJECT_ACTIVE(
				 ...
				 struct PS3CmdWord *cmd_word)
{
	...
	if ((cmd_word->type == PS3_CMDWORD_TYPE_READ) ||
		...
		if (cmd->req_frame->taskReq.reqHead.cmdType !=
		    PS3_CMD_SCSI_TASK_MANAGEMENT) {
			is_find =
				ps3_add_scsi_rw_cmd_filter(instance, cmd_word);
		} else {
			is_find = ps3_add_scsi_task_cmd_filter(instance,
							       cmd_word);
		}
	} else {
		if (cmd->req_frame->taskReq.reqHead.cmdType ==
			...
			is_find = ps3_add_scsi_task_cmd_filter(instance,
							       cmd_word);
		} else if (cmd->req_frame->taskReq.reqHead.cmdType ==
				   PS3_CMD_MANAGEMENT ||
			   cmd->req_frame->taskReq.reqHead.cmdType ==
				   PS3_CMD_SAS_MANAGEMENT) {
...
void ps3_inject_scsi_rw_force_stop(struct ps3_cmd *cmd)
{
	...
	if (cmd->r1x_peer_cmd != NULL) {
		LOG_DEBUG(
			"[inject] hno:%u force stop r1x scsi CFID[%d] and CFID[%d]\n",
			PS3_HOST(cmd->instance), cmd->index,
			cmd->r1x_peer_cmd->index);
		if (!ps3_r1x_peer_cmd_free_nolock(cmd->r1x_peer_cmd)) {
			PS3_IOC_DRV2IOC_BACK_INC(cmd->instance,
						 cmd->r1x_peer_cmd,
...
void ps3_task_abort_force_scsi_cmd(struct ps3_cmd *cmd)
{
	...
	list_for_each_entry_safe(p_cmd, p_cmd_next,
		...
		if (abort_cmd == p_cmd->cmd) {
			LOG_DEBUG(
				"[inject] hno:%u del scsi CFID [%u] from hit_rw_list\n",
	...
	if (pitem->item.scsi_cmd.inject_count == 0) {
		LOG_DEBUG(
			"[inject] del inject_rw host_no %u channel %u id %u lba 0x%llx len %u type %u, time %u!\n",
			pitem->item.scsi_cmd.host_no,
			pitem->item.scsi_cmd.channel, pitem->item.scsi_cmd.id,
			pitem->item.scsi_cmd.lba, pitem->item.scsi_cmd.len,
			pitem->item.scsi_cmd.dealType,
			pitem->item.scsi_cmd.inject_count);
...
void ps3_task_reset_force_scsi_cmd(struct ps3_cmd *cmd)
{
	...
	list_for_each_entry_safe(p_cmd, p_cmd_next,
		...
		if (scsi_priv_data->disk_pos.diskDev.diskID ==
		    cmd->req_frame->taskReq.reqHead.devID.diskID) {
			LOG_DEBUG(
				"[inject] hno:%u del scsi CFID [%u] from hit_rw_list\n",
...
int ps3_scsi_rw_cmd_filter_handle(struct scsi_cmnd *scmd)
{
	...
	unsigned char is_find = PS3_FALSE;
	unsigned short reply_flags;
	int iRet = PS3_SUCCESS;
	struct inject_cmds_t *pitem = NULL;
	unsigned char type =
		PS3_SCSI_CMD_TYPE(ps3_scsih_cdb_rw_type_get(scmd->cmnd));
	...
	if (p_cmd->pitem->item.scsi_cmd.dealType ==
	    PS3_SCSI_CMD_TIMEOUT_FORCE_REPLY) {
		ps3_inject_scsi_rw_force_stop(cmd);
	} else if (p_cmd->pitem->item.scsi_cmd.dealType == PS3_SCSI_CMD_ERROR) {
		scmd->result =
			p_cmd->pitem->item.scsi_cmd.cmdDeal.errReply.result;
		scmd->sense_buffer[0] = p_cmd->pitem->item.scsi_cmd.cmdDeal
						.errReply.sshdr.resp_code;
		scmd->sense_buffer[1] = p_cmd->pitem->item.scsi_cmd.cmdDeal
						.errReply.sshdr.sense_key;
		scmd->sense_buffer[2] =
			p_cmd->pitem->item.scsi_cmd.cmdDeal.errReply.sshdr.asc;
		scmd->sense_buffer[3] =
			p_cmd->pitem->item.scsi_cmd.cmdDeal.errReply.sshdr.ascq;
		reply_flags = 1;
		if (cmd->cmd_receive_cb) {
			LOG_DEBUG(
				"[inject]  sense: CFID:%d, rcode:0x%x, key:0x%x, asc:0x%x, ascq:0x%x\n",
				cmd->index, scmd->sense_buffer[0],
				scmd->sense_buffer[1], scmd->sense_buffer[2],
				scmd->sense_buffer[3]);
			iRet = cmd->cmd_receive_cb(cmd, reply_flags);
		} else {
			LOG_DEBUG(
				"[inject] ps3 cmd index %d has no cmd_receive_cb\n",
				cmd->index);
		}
	} else {
...
int ps3_scsi_task_cmd_filter_handle(struct ps3_cmd *cmd)
{
	...
	if (p_cmd->pitem->item.scsi_task_cmd.dealType ==
	    PS3_SCSI_TASK_CMD_TIMEOUT) {
		LOG_DEBUG("[inject] hno:%u force task mgr CFID:%d timeout\n",
			  PS3_HOST(instance), cmd->index);
	} else if (p_cmd->pitem->item.scsi_task_cmd.dealType ==
		   PS3_SCSI_TASK_CMD_ERROE) {
		cmd->cmd_state.state = PS3_CMD_STATE_COMPLETE;
		ps3_spin_lock_irqsave(&cmd->cmd_state.lock, &cmd_lock_flags);
		...
		complete(&cmd->sync_done);
		LOG_DEBUG(
			"[inject] hno:%u force complete task mgr CFID:%d error\n",
			PS3_HOST(instance), cmd->index);
		...
		if (cmd->req_frame->taskReq.reqHead.cmdSubType ==
		    PS3_TASK_CMD_SCSI_TASK_ABORT) {
			LOG_DEBUG(
				"[inject] hno:%u force complete task abort CFID:%d normal reply\n",
				PS3_HOST(instance), cmd->index);
			ps3_task_abort_force_scsi_cmd(cmd);
		} else if (cmd->req_frame->taskReq.reqHead.cmdSubType ==
			   PS3_TASK_CMD_SCSI_TASK_RESET) {
			LOG_DEBUG(
				"[inject] hno:%u force complete task reset CFID:%d normal reply\n",
				PS3_HOST(instance), cmd->index);
			ps3_task_reset_force_scsi_cmd(cmd);
		} else {
			LOG_INFO(
				"[inject] ps3 CFID %u type %d subType %d invalid\n",
				cmd->index,
				cmd->req_frame->taskReq.reqHead.cmdType,
				cmd->req_frame->taskReq.reqHead.cmdSubType);
		}
...
int ps3_mgr_cmd_filter_handle(struct ps3_cmd *cmd)
{
	...
	list_for_each_entry_safe(p_cmd, p_cmd_next, &g_hit_cmd_list.mgr_list,
				  list) {
		if (cmd == p_cmd->cmd) {
			list_del(&p_cmd->list);
			pitem = p_cmd->pitem;
