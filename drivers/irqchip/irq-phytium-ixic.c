...
static void phytium_ixic_irq_eoi(struct irq_data *d)
{
	...
	u32 gstatus = readl(data->hpb) |
		      (readl(data->hpb + HPB_INTX_STATUS_1) << 12);
	u32 imask, istatus;
	int i;
	...
	for (i = 0; i < CTR_BANK_NUM; i++, gstatus >>= 4) {
		if (gstatus & imask)
			writel(istatus, data->ctr + CTR_BANK_SIZE*i +
					CTR_BANK_ISTATUS_LOCAL);
	}
			      ...
			      unsigned int nr_irqs, void *data)
{
	...
	WARN_ON(nr_irqs != 1);
	irq_domain_set_hwirq_and_chip(dom, virq, hwirq, &phytium_ixic_irq_chip,
				      info);
...
static struct ixic_irq_data *phytium_ixic_init(const struct fwnode_handle *fw,
					       struct resource *ctr,
					       struct resource *hpb)
{
	...
	if (fwnode_property_read_u32_array(fw, "intx-spi-base",
					   &data->spi_base, 1)) {
		err = -ENODEV;
		goto out_free;
	}
				   ...
				   struct device_node *parent)
{
	...
	d = irq_domain_add_hierarchy(pd, 0, NUM_IRQS, node, &ixic_domain_ops,
				     data);
	if (!d) {
