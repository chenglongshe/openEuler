...
static int kvm_handle_bad_page(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	/*
			   ...
			   unsigned int access)
{
	...
	if (unlikely(is_error_pfn(fault->pfn)))
		return kvm_handle_bad_page(vcpu, fault);
...
void kvm_zap_gfn_range(struct kvm *kvm, gfn_t gfn_start, gfn_t gfn_end)
{
	...
	kvm_inc_notifier_count(kvm, 0, -1ul);
	...
	kvm_dec_notifier_count(kvm, 0, -1ul);
						...
						int target_level)
{
	...
	for (level = KVM_MAX_HUGEPAGE_LEVEL; level > target_level; level--) {
		slot_handle_level_range(kvm, (struct kvm_memory_slot *)slot,
