...
static void __init __init_microcode_pages(void)
{
	pud_t *pud;
	unsigned int i;
 ...
 */
static int __init init_microcode_pages(void)
{
	if (IS_ENABLED(CONFIG_X86_32))
		return 0;

	if (zhaoxin_ucode_patch != UCODE_BSP_LOADED)
		return 0;
	...
	__init_microcode_pages();

	return 0;
}
early_initcall(init_microcode_pages);
...
{
	struct ucode_cpu_info uci;
	struct microcode_zhaoxin *patch;
	pgd_t *pgd_pgt;
	...
	if (zhaoxin_ucode_patch == UCODE_BSP_LOADED) {
		patch = get_microcode_blob(&uci, false);
		if (!patch)
			return;

		zhaoxin_ucode_patch = patch;
	}
...
/*
 * Due to the initrd being about to go away, the ucode data is
 * saved from the initrd for loading on both the BSP and APs
 * during the resume phase.
 */
int __init save_microcode_in_initrd_zhaoxin(void)
{
	struct ucode_cpu_info uci;
