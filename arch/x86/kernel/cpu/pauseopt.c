 ...
 * pauseopt max time is 100000 in TSC-quanta and C0.2 is disabled
 */
...
static ssize_t enable_p01_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 ret;
...
static ssize_t max_time_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
...
static ssize_t max_time_store(struct device *kobj, struct device_attribute *attr, const char *buf,
			      size_t count)
{
	u32 max_time, ctrl;
	int ret;
...
static int __init pauseopt_init(void)
{
	...
	ret = cpuhp_setup_state(CPUHP_AP_ONLINE_DYN, "pauseopt:online", pauseopt_cpu_online,
				pauseopt_cpu_offline);
	if (ret < 0) {
		/*
