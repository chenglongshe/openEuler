...
{
	if (!static_key_enabled(&kvm_rme_is_available))
		return -EFAULT;
...
{
	struct tmi_tec_run *run = KVM_GET_TEC_RUN(vcpu);
	if (serror_pending)
		return -EINVAL;

	if (ext_dabt_pending) {
		if (!(run->enter.flags & REC_ENTER_EMULATED_MMIO))
			return -EINVAL;
		run->enter.flags &= ~REC_ENTER_EMULATED_MMIO;
		run->enter.flags |= REC_ENTER_INJECT_SEA;
	}
	return 0;
}
...
static struct cca_operations virtcca_operations = {
	...
	.vcpu_set_events = kvm_cvm_vcpu_set_events,
	.get_rec_run = _kvm_get_rec_run,
