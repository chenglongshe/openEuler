...
int virtcca_dump_array_to_file(uint64_t *gpa_list, uint64_t *crc_result,
...
uint32_t __execute_ns_crc_dump(struct kvm *kvm, uint64_t target_ipa,
	...
	while (crc_addr < gpa_end) {
		...
		if (config->is_secure) {
			...
			for (int i = 0; i < actual_chunk_size; i++) {
				...
				crc_result[i] = __execute_ns_crc_dump(kvm, gpa_list[i],
							   ...
							   uint64_t guest_rd, bool is_src)
{
	...
	pr_info("calling %s, cid=%llu, port=%d\n", __func__, cid, sa.svm_port);
