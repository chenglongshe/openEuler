...
static inline unsigned long tsi_migvm_get_attr(unsigned long guest_rd,
                                              void *user_buf)
{
	struct arm_smccc_res res;
...
static inline unsigned long tsi_migvm_set_slot(unsigned long guest_rd,
                                             void *user_buf)
{
	struct arm_smccc_res res;
...
static inline unsigned long tsi_mig_integrity_checksum_loop(unsigned long rd)
{
	struct arm_smccc_res res;
	arm_smccc_1_1_smc(SMC_TSI_MIG_INTEGRITY_CHECKSUM_LOOP, rd, &res);
