...
DECLARE_STATIC_KEY_FALSE(has_hwcap_actlr_xcall);
...
static inline bool is_xcall_support(void)
{
	return static_branch_unlikely(&has_hwcap_actlr_xcall) ||
...
static inline int set_hw_xcall_entry(struct hw_xcall_info *xinfo,
				     unsigned int sc_no, bool disable)
{
	unsigned int i;

	xinfo->xcall_entry[sc_no] = disable ? no_xcall_entry : xcall_entry;
	if (!disable) {
...
{
	if (!static_branch_unlikely(&has_hwcap_actlr_xcall) || !tsk->xinfo)
		return;
