...
} mig_integrity_share_queue_addr_s;

struct mig_queue_device {
    struct page *send_page;
    struct page *recv_page;
    unsigned long rd;
    mig_integrity_share_queue_addr_s queue_addr;
};

static struct driver_data *driver_data_ptr = NULL;
...
static int __init migvm_queue_driver_init(void)
{
    ...
    printk(KERN_INFO "Migvm queue driver loaded. Major=%d\n", MAJOR(data->devno));
    return 0;
...
static void __exit migvm_queue_driver_exit(void)
{
    ...
    printk(KERN_INFO "Migvm queue driver unloaded\n");
}
