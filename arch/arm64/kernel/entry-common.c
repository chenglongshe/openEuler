...
#ifdef CONFIG_XCALL_SMT_QOS
static void set_return_to_vdso(struct pt_regs *regs, struct qos_data *qos_data,
			       unsigned long trampoline)
{
	qos_data->delay_cycles = sysctl_delay_cycles;
	trace_printk("delay_cycles: %ld, saved pc: 0x%llx, saved x8: 0x%llx\n",
		     qos_data->delay_cycles, regs->pc, regs->regs[8]);
			     ...
			     void (*nmi_handler)(struct pt_regs *))
{
	...
	if (current->mm && current->mm->smt_qos_page) {
		...
		trampoline = (unsigned long)VDSO_SYMBOL(current->mm->context.vdso, smt_qos_trampoline);
	}
#endif
	...
	if (system_uses_nmi() && (read_sysreg(isr_el1) & nmi_flag)) {
		...
		if (cpus_have_const_cap(ARM64_HAS_WFXT) && qos_data &&
		    trampoline && sysctl_delay_cycles > 0) {
			set_return_to_vdso(regs, qos_data, trampoline);
