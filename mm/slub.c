...
new_objects:
	...
	thisnode_gfpflags = gfpflags;
	/*
	 * When a preferred node is indicated but no __GFP_THISNODE
	 *
	 * 1) try to get a partial slab from target node only by having
	 *    __GFP_THISNODE in thisnode_gfpflags for new_slab_objects()
	...
	if (unlikely(node != NUMA_NO_NODE && !(gfpflags & __GFP_THISNODE)
		     && try_thisnode))
		thisnode_gfpflags = GFP_NOWAIT | __GFP_THISNODE;

	freelist = new_slab_objects(s, thisnode_gfpflags, node, &c);
