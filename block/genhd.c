...
static void *bpf_disk_seqf_start(struct seq_file *seqf, loff_t *pos)
{
	...
	task = get_current_level1_reaper();
	if (!task)
		task = current;
	priv->task_blkcg = css_to_blkcg(task_css(task, io_cgrp_id));
...
static void *bpf_show_partition_start(struct seq_file *seqf, loff_t *pos)
{
...
static int bpf_show_partition(struct seq_file *seqf, void *v)
{
	struct partitions_seq_priv *priv = seqf->private;
	struct gendisk *sgp = v;
	struct block_device *part;
...
static const struct seq_operations bpf_partitions_op = {
	.start	= bpf_show_partition_start,
	.next	= disk_seqf_next,
	.stop	= bpf_disk_seqf_stop,
	.show	= bpf_show_partition
};
...
static int __init proc_genhd_init(void)
{
	...
	err = bpf_iter_reg_target(&diskstats_reg_info);
	if (err)
		return err;

	partitions_reg_info.ctx_arg_info[0].btf_id = btf_partitions_ids[0];
	err = bpf_iter_reg_target(&partitions_reg_info);
#endif
	return err;
}
