...
void enfs_debug_print_name_list(void)
{
	...
	list_for_each_entry(list, &dns_cache_list, next) {
		...
		for (i = 0; i < list->inet.count; i++) {
			sockaddr_ip_to_str((struct sockaddr *)&list->inet.
					   address[i], buf, 128);
			printk(KERN_INFO "    %s\n", buf);
		}
		for (i = 0; i < list->inet6.count; i++) {
			sockaddr_ip_to_str((struct sockaddr *)&list->inet6.
					   address[i], buf, 128);
			printk(KERN_INFO "    %s\n", buf);
		}
	}
		    ...
		    int *tmp_slot)
{
	...
	for (i = 0; i < src->count && *tmp_slot != 0; i++) {
		addr = &src->address[i];
		if (enfs_insert_ip_list
		    (dst, enfs_get_config_link_count_per_mount(), addr)) {
			(*tmp_slot)--;
		}
	}
}
				  ...
				  struct nfs_ip_list *ip_list)
{
	...
	while (ip_str) {
		...
		ip_len =
		    rpc_pton(NULL, ip_str, strlen(ip_str),
		...
		if (enfs_insert_ip_list
		    (ip_list, enfs_get_config_link_count_per_mount(), &sa)) {
			(*tmp_slot)--;
		}
			   ...
			   int *tmp_slot, const char *options)
{
	...
	ip_len =
	    enfs_euler_dns_query(net, NULL, name, strlen(name), options,
				 &ip_addr, NULL, true);
#else
	enfs_log_debug("domain_name:%s option:%s\n", name, options);
	ip_len =
	    enfs_dns_query(NULL, name, strlen(name), options, &ip_addr, NULL);
#endif
	if (ip_len <= 0) {
				    ...
				    int slot, unsigned short family)
{
	...
	if (family == AF_INET6) {
		ret =
		    multi_query_dns(ip_list, name, slot, &tmp_slot,
				    "ipv6 list");
		if (ret) {
...
int enfs_dns_process_ip(DNS_QUERY_IP_INFO_SINGLE *dnsQueryIpInfo,
			DNS_QUERY_LSID_INFO **dnsQueryLsidInfo, int *lsidCount,
			int ipNumber)
{
	int i;
	int index = 0;
	...
	*lsidCount = count;
	...
	lsIdInfo =
	    (DNS_QUERY_LSID_INFO *) kmalloc(count * sizeof(DNS_QUERY_LSID_INFO),
					    GFP_KERNEL);
	if (lsIdInfo == NULL) {
...
int enfs_server_query_dns(struct rpc_clnt *clnt, NFS_ROUTE_DNS_INFO_S *dns_info,
			  ...
			  char *dnsName)
{
	...
	int lsidCount;
	ret =
	    dorado_query_dns(clnt, &dnsQueryIpInfo, ip_type, dnsNamecount,
			     dnsName, &ipNumber);
	if (ret) {
		return ret;
	}
	...
	ret =
	    enfs_dns_process_ip(dnsQueryIpInfo, &dnsQueryLsidInfo, &lsidCount,
				ipNumber);
	if (ret) {
		return ret;
	}
	...
	while (ipList->count <
	       (enfs_get_config_link_count_per_mount() <
		ipNumber ? enfs_get_config_link_count_per_mount() : ipNumber)) {
		if (dnsQueryLsidInfo[i].offset < dnsQueryLsidInfo[i].count) {
			if (i != 0) {
				offset += dnsQueryLsidInfo[i - 1].count;
			}
			ret =
			    dns_resolver_name_list(dnsQueryIpInfo
						   [offset +
						    dnsQueryLsidInfo[i].offset].
						   ipAddr, &tmpSlot, ipList);
			if (ret) {
				goto out;
			}
			dnsQueryLsidInfo[i].offset++;
			...
			struct rpc_clnt *clnt)
{
	...
	dnsName =
	    (char *)kmalloc(dns_info->dnsNameCount * EXTEND_MAX_DNS_NAME_LEN,
			    GFP_KERNEL);
	if (!dnsName) {
		kfree(ip_list);
		return -ENOMEM;
	...
	if (clnt) {
		...
		ret =
		    enfs_server_query_dns(clnt, dns_info, ip_list,
					  enfs_get_config_link_count_per_mount
					  (), ip_type, dns_info->dnsNameCount,
					  dnsName);
		if (ret != 0) {
...
void enfs_add_domain_name(struct multipath_mount_options *opt)
{
	...
	for (i = 0;
	     ...
	     i++) {
		enfs_domain_inc(opt->pRemoteDnsInfo->routeRemoteDnsList[i].
				dnsname);
	}
}
...
static int collect_clnt_name(struct nfs_client *clp, void *data)
{
	...
	for (i = 0;
	     i < MAX_DNS_SUPPORTED
	     && i < clp_info->pRemoteDnsInfo->dnsNameCount; i++) {
		enfs_domain_inc(clp_info->pRemoteDnsInfo->routeRemoteDnsList[i].
				dnsname);
	}
	return 0;
}
...
static void do_dns_update(struct work_struct *work)
{
	...
	opt.pRemoteDnsInfo = clp_info->pRemoteDnsInfo;
	error =
	    multipath_query_dns(&opt, wk->ss.ss_family, true, wk->clRpcclient);
	if (error != 0) {
		enfs_log_info("Scheduled update dns err:%d.\n", error);
		wk->query_ok = false;
	}
	...
	while (!kthread_should_stop()) {
		...
		if (interval_ms != 0 && enfs_timeout_ms(&start, interval_ms)
			...
			if (times == query_times) {
				...
				enfs_for_each_nfs_clnt(requery_clnt_dns,
						       &free_list);
