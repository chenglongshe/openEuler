			...
			int ipNumber)
{
	...
	*lsidCount = count;
	...
	lsIdInfo = (DNS_QUERY_LSID_INFO *)kmalloc(
		count * sizeof(DNS_QUERY_LSID_INFO), GFP_KERNEL);
	if (lsIdInfo == NULL) {
			...
			struct rpc_clnt *clnt)
{
	...
	dnsName = (char *)kmalloc(
		dns_info->dnsNameCount * EXTEND_MAX_DNS_NAME_LEN, GFP_KERNEL);
	if (!dnsName) {
		kfree(ip_list);
		return -ENOMEM;
	}
