...
void enfs_destroy_rpcclnt_list(struct list_head *head)
{
	...
	while (!list_empty(head)) {
		item =
		    list_entry(head->next, struct rpcclnt_release_item, node);
			       ...
			       struct rpc_xprt *xprt, void *data)
{
	...
	ret =
	    pm_ping_rpc_test_xprt_with_callback(clnt, xprt,
						pm_xprt_ping_callback,
						attach_info->data);
	LVOS_TP_END;
	if (ret != 1) {
...
static void enfs_add_xprts_to_clnt(struct rpc_clnt *clnt,
				   ...
				   int remoteTotal, int localTotal)
{
	struct rpc_xprt *xprt;
	pm_path_state state;
	int i;
	...
	if (count == 0 && localTotal == 0) {
		maxCountPerMount =
		    ...
		    1 ? remoteTotal : enfs_get_config_link_count_per_mount() -
		    1;
	}
			      ...
			      struct nfs_ip_list *remote)
{
	...
	attach_infos = (struct xprt_attach_info *)kzalloc((total_combinations) *
							  sizeof(struct
								 xprt_attach_info),
							  GFP_KERNEL);
	...
	for (i = 0; i < total_combinations; i++) {
		...
		remote_index =
		    (i + link_count / local_remote_total_lcm) % remote_total;
		...
		if (enfs_already_have_xprt
		    (clnt, &local->address[local_index],
		...
		err =
		    enfs_configure_xprt_to_clnt(xprtargs, clnt,
						&attach_infos[i]);
		if (err) {
...
int enfs_multipath_create_thread(void *data)
{
	...
	if (mount_options->pRemoteDnsInfo->dnsNameCount != 0) {
		...
		rpc_peeraddr(create_args->clnt, (struct sockaddr *)&ss,
			     sizeof(ss));
		errno =
		    multipath_query_dns(mount_options, ss.ss_family, true,
					create_args->clnt);
		if (errno != 0) {
			enfs_log_error
	...
	errno =
	    enfs_config_xprt_create_args(&xprtargs, create_args->args,
