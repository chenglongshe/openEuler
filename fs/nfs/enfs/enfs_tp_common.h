...
#define LVOS_TP_REG(name, desc, fn) \
	RegTracePoint(MY_PID, #name, desc, (FN_TRACEP_COMMON_T)(fn))

#define LVOS_TP_UNREG(name) UnregTracePoint(0, #name)
#define LVOS_TP_START(name, ...)                                          \
	do {                                                              \
		static LVOS_TRACEP_NEW_S *_pstTp;                  \
		if (unlikely(_pstTp == NULL)) {                           \
			(void)GetTracePoint(0, #name, &_pstTp);           \
			if (_pstTp == NULL) {                             \
				printk(KERN_ERR                           \
				       "tracepoint `%s` not registered",  \
				       #name);                            \
			}                                                 \
		}                                                         \
		if (NULL != _pstTp &&                                     \
		    LVOS_TRACEP_STAT_ACTIVE == _pstTp->iActive &&         \
		    LVOS_TP_TYPE_CALLBACK == _pstTp->type) {              \
			_pstTp->fnHook(&_pstTp->stParam, __VA_ARGS__);    \
			_pstTp->timeCalled++;                             \
			if (_pstTp->timeAlive > 0 &&                      \
			    0 == --(_pstTp->timeAlive)) {                 \
			...
			if (NULL != _pstTp &&                             \
			    LVOS_TRACEP_STAT_ACTIVE == _pstTp->iActive && \
			    LVOS_TP_TYPE_PAUSE == _pstTp->type) {         \
