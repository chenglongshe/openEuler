					  ...
					  int min_value, int max_value)
{
	...
	while ((token = strsep(&tmp, ",")) != NULL) {
		...
		if (sscanf
		    (token, "%llx",
		     &(g_enfs_config_info.priopity_array_wwns[count])) < 0) {
			kfree(copy);
			return -EINVAL;
		}
		count++;
					       ...
					       char *value, int valueLen)
{
	...
	while (buffer[0] != '\0') {
		...
		if (buffer[0] == '\n') {
			...
			// 处理正常字符串
			ret =
			    enfs_parse_key_value_from_config(buffer, key, val,
							     STRING_BUF_SIZE,
							     STRING_BUF_SIZE);
			if (ret < 0) {
				enfs_log_error
...
int32_t enfs_config_load(void)
{
	...
	for (i = 0; i < table_len; i++) {
		ret =
		    enfs_get_value_from_config_file(buffer,
						    g_check_and_assign_value[i].
						    field_name, value,
						    STRING_BUF_SIZE);
		if (ret < 0) {
			continue;
		...
		if (g_check_and_assign_value[i].func != NULL) {
			(*g_check_and_assign_value[i].
			 func) (g_check_and_assign_value[i].field_name, value,
				min, max);
		}
	}
...
bool enfs_whitelist_filte(char *ip_addr)
{
	...
	for (i = 0; i < count; i++) {
		if (enfs_glob_match
		    (g_enfs_config_info.local_ip_filters[i], ip_addr)) {
			return true;
		}
	}
	return false;
...
static bool enfs_file_changed(const char *filename)
{
	...
	if (timespec64_compare(&modify_time, &file_stat.mtime) == -1) {
		...
		enfs_log_debug("file change: %lld %lld\n",
			       (long long int)(modify_time.tv_sec),
			       (long long int)(file_stat.mtime.tv_sec));
		return true;
	}
