...
static bool enfs_rpc_xprt_switch_need_delete_addr(struct multipath_mount_options
						  *enfs_option,
						  ...
						  u32 protocol)
{
	...
	for (i = 0; i < local_total; i++) {
		...
		find_same_ip =
		    rpc_cmp_addr((struct sockaddr *)&enfs_option->
				 local_ip_list->address[i], srcaddr);
		if (find_same_ip) {
			break;
		}
	}
	...
	for (i = 0; i < remote_total; i++) {
		find_same_ip =
		    rpc_cmp_addr((struct sockaddr *)&enfs_option->
				 remote_ip_list->address[i], dstaddr);
		if (find_same_ip) {
			break;
		}
	}
				     ...
				     void *enfs_option)
{
	...
	rcu_read_lock();
	xps =
	    xprt_switch_get(rcu_dereference
	...
	list_for_each_entry_rcu(pos, &xps->xps_xprt_list, xprt_switch) {
		if (xprt_count < (enfs_get_config_link_count_per_mount() - 1)) {
			if (enfs_delete_xprt_from_switch(pos, enfs_option, xps)
			    == false) {
				xprt_count++;
			}
		} else {
...
int enfs_remount(struct nfs_client *nfs_client, void *enfs_option)
{
	...
	if (remount_opt->pRemoteDnsInfo->dnsNameCount != 0) {
		rpc_peeraddr(nfs_client->cl_rpcclient, (struct sockaddr *)&ss,
			     sizeof(ss));
		ret =
		    multipath_query_dns(remount_opt, ss.ss_family, false,
...
int enfs_remount_iplist(struct nfs_client *nfs_client, void *enfs_option)
{
	...
	errno =
	    enfs_config_xprt_create_args(&xprtargs, &args, servername,
					 sizeof(servername));
	if (errno) {
	...
	if (remount_opt->remote_ip_list->count == 0) {
		if (client_info->remote_ip_list->count == 0) {
			errno =
			    rpc_peeraddr(nfs_client->cl_rpcclient,
					 (struct sockaddr *)&remount_opt->
					 remote_ip_list->address[0],
