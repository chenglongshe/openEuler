...
int enfs_query_lookup_cache(struct lookupcache_work *work_info)
{
	...
	ret =
	    enfs_rpc_send(work_info->cl_rpcclient, ENFSPROC_LOOKUPCACHE, &args,
			  &enfsRes);
		...
		list_for_each_entry(pos, &nn->nfs_volume_list, master_link) {
			...
			if (memcmp(fh, &work_info->fh, sizeof(struct nfs_fh)) ==
				...
				ctx =
				    ...
				    xprt_get_reserve_context(pos->client->
							     cl_xprt);
				if (ctx == NULL) {
					enfs_log_error
...
int enfs_lookupcache_timer_init(void)
{
	...
	if (ret != 0) {
		enfs_log_error
		    ("enfs_lookupcache_timer_init workqueue init failed.\n");
		return ret;
	}
	ret = lookupcache_start();
	if (ret != 0) {
		enfs_log_error
		    ("enfs_lookupcache_timer_init work start failed.\n");
