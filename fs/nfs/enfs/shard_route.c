 ...
 */
int enfs_query_lif_info(struct rpc_clnt *clnt, FILE_UUID *file_uuid,
			uint64_t *lsid, uint32_t *cpuId)
{
	int ret;
	struct view_table *table;
...
int enfs_update_lsinfo(uint64_t devId, EXTEND_GET_LS_VERSION *ls_view,
			   int *flag)
{
	int ret;
	struct view_table *table;
...
static void update_server_enfs_capability(struct rpc_clnt *clnt, int flag)
{
	...
	for_each_net_rcu(net) {
		...
		list_for_each_entry(pos, &nn->nfs_volume_list, master_link) {
			...
			ret =
				sockaddr_ip_to_str((struct sockaddr *)&clnt->
						   cl_xprt->addr, remoteip,
						   sizeof(remoteip));
			if (ret != 0) {
			...
			ret =
				sockaddr_ip_to_str((struct sockaddr *)&pos->client->
						   cl_xprt->addr, serverip,
						   sizeof(serverip));
			if (ret != 0) {
				enfs_log_error("remoteip to str err:%d.\n",
						   ret);
...
static int query_and_update_shard(struct rpc_clnt *clnt, FILE_UUID *file_uuid,
...
static void debug_show_uuidinfo(int argc, char *argv[])
{
	...
	printk(KERN_INFO "fsidinfo devId:%llu",
	...
	list_for_each_entry(table, &shard_ctrl->view_list, next) {
		...
		list_for_each_entry(info, &table->fs_head, next) {
			...
			printk(KERN_INFO
				   "fsidinfo fsid:%u clusterId:%llu storagePoolId:%u tenantId:%u.\n",
		...
		list_for_each_entry(view, &table->shard_head, next) {
			...
			printk(KERN_INFO
				   "fsidinfo clusterId:%llu storagePoolId:%u shardNum:%u shard:%u lsid:%llu cpuId:%u.\n",
...
static void debug_show_fsinfo(int argc, char *argv[])
{
	...
	list_for_each_entry(table, &shard_ctrl->view_list, next) {
		list_for_each_entry(info, &table->fs_head, next) {
			if (fsid == info->fsId) {
				printk(KERN_INFO
					   "fsid(%u) clusterId(%llu) storagePoolId(%u) tenantId(%u).\n",
					   fsid, info->clusterId,
					   info->storagePoolId, info->tenantId);
			}
		}
			...
			if (view->clusterId == clusterId &&
				...
				for (count = 0; count < 100; count++) {
					if (count + startIndex < view->num) {
						printk(KERN_INFO
							   "shardid(%d) lsid(0x%llx) vnodeid(%u) cpuId(%u).\n",
							   count + startIndex,
							   view->entry[count +
								   startIndex].
							   lsid,
	...
	list_for_each_entry(info, &shard_ctrl->clnt_info_list, next) {
		...
		list_for_each_entry_rcu(pos, &xps->xps_xprt_list, xprt_switch) {
			...
			if (argc == 0 || strcmp(buf, argv[0]) == 0) {
				...
				printk(KERN_INFO
					   "ipaddr(%s) lsId(0x%llx) wwn(0x%llx) cpuId(%u).\n",
...
static void debug_show_shardview(int argc, char *argv[])
{
	...
	list_for_each_entry(table, &shard_ctrl->view_list, next) {
		...
		list_for_each_entry(info, &table->fs_head, next) {
			printk(KERN_INFO
				   "shardview  fsid:%u clusterId:%llu storagePoolId:%u tenantId:%u.\n",
		...
		list_for_each_entry(view, &table->shard_head, next) {
			printk(KERN_INFO
				   "shardview  clusterId:%llu storagePoolId:%u shardNum:%u.\n",
				   view->clusterId, view->storagePoolId, view->num);
		}
	}
	read_unlock(&shard_ctrl->view_lock);
...
static int enfs_recovery_nlm_lock(struct rpc_clnt *clnt)
{
	...
	for_each_net_rcu(net) {
		...
		list_for_each_entry(pos, &nn->nfs_volume_list, master_link) {
			...
			if (!strcmp(remoteip, serverip)) {
				rpc_clnt_iterate_for_each_xprt(pos->nlm_host->
								   h_rpcclnt,
								   TraverseNlmXprt,
								   (void *)pos);
