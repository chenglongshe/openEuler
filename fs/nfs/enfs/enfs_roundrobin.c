...
typedef struct rpc_xprt *(*enfs_xprt_switch_find_xprt_t) (struct rpc_xprt_switch
							  * xps,
							  const struct rpc_xprt
							  * cur);
...
static struct rpc_xprt *enfs_lb_set_cursor_xprt(struct rpc_xprt_switch *xps,
						struct rpc_xprt **cursor,
						enfs_xprt_switch_find_xprt_t
						find_next)
{
	struct rpc_xprt *pos;
	struct rpc_xprt *old;
...
static struct rpc_xprt *enfs_lb_find_next_entry_roundrobin(struct
							   rpc_xprt_switch *xps,
							   const struct rpc_xprt
							   *cur)
{
...
struct rpc_xprt *enfs_lb_switch_find_first_active_xprt(struct rpc_xprt_switch
						       *xps)
{
	struct rpc_xprt *pos;
...
static struct rpc_xprt *enfs_lb_switch_get_next_xprt_roundrobin(struct
								rpc_xprt_switch
								*xps,
								const struct
								rpc_xprt *cur)
...
static struct rpc_xprt *enfs_lb_iter_next_entry_roundrobin(struct rpc_xprt_iter
							   *xpi)
{
...
static struct rpc_xprt *enfs_lb_switch_find_singular_entry(struct
							   rpc_xprt_switch *xps,
							   const struct rpc_xprt
							   *cur)
{
...
static struct rpc_xprt *enfs_lb_iter_next_entry_sigular(struct rpc_xprt_iter
							*xpi)
{
	struct rpc_xprt_switch *xps = rcu_dereference(xpi->xpi_xpswitch);
