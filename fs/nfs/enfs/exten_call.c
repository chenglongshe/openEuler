			     ...
			     struct xdr_stream *xdrStream)
{
	...
	extend3ResLen = sizeof(uint32_t) * 3 +
			sizeof(DNS_QUERY_IP_INFO_SINGLE) * ipNumber;
	extend3Res = kmalloc(extend3ResLen, GFP_KERNEL);
	if (extend3Res == NULL)
		return -ENOMEM;
			  ...
			  FS_SHARD_VIEW **resDataOut)
{
	...
	EXTEND3args *args = kmalloc(bufLen, GFP_KERNEL);
	...
	resData = kmalloc(sizeof(FS_SHARD_VIEW) + sizeof(FS_SHARD_VIEW_SINGLE) *
...
int dorado_query_lsId(struct rpc_clnt *clnt, EXTEND_GET_LS_VERSION **resDataOut)
{
	...
	if (ret == -EOPNOTSUPP || ret == -ENOTSUPP) {
		pr_err("ENFS: %s: no support NfsExtendOp.\n",
			__func__);
		kfree(args);
		kfree(buf);
		return ret;
	...
	resData = kmalloc(sizeof(EXTEND_GET_LS_VERSION) + sizeof(EXTEND_GET_LS_VERSION_SINGLE) *
				(extend3Res->extend_res_u.lsView.num), GFP_KERNEL);
			 ...
			 LIF_PORT_INFO *lifInfo)
{
	...
	if (ret) {
		pr_err("ENFS: NfsExtendOp get lif view failed %d.\n",
			ret);
		kfree(args);
		kfree(buf);
		return ret;
