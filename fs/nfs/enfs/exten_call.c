...
int dorado_query_lsId(struct rpc_clnt *clnt, EXTEND_GET_LS_VERSION **resDataOut)
{
	...
	resData = (EXTEND_GET_LS_VERSION *)kmalloc(
		sizeof(EXTEND_GET_LS_VERSION) +
		     ...
		     int *ipNumber)
{
	...
	*ipNumber = extend3Res->extend_res_u.dnsQueryIpInfo.ipNumber;
	resData = (DNS_QUERY_IP_INFO_SINGLE *)kmalloc(
		extend3Res->extend_res_u.dnsQueryIpInfo.ipNumber *
			(sizeof(DNS_QUERY_IP_INFO_SINGLE)),
		GFP_KERNEL);
	if (resData == NULL) {
		printk(KERN_ERR "%s:kmalloc arg cfailed.\n", __func__);
		kfree(extend3Res);
