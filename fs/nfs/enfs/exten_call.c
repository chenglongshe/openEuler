...
int NfsExtendDecodeFsShardV1(EXTEND3res ** extend3ResOut, __be32 * p,
			     struct xdr_stream *xdrStream);
int NfsExtendDecodeLifInfoV1(EXTEND3res ** extend3ResOut, __be32 * p,
			     struct xdr_stream *xdrStream);
int NfsExtendDecodeDnsInfoV1(EXTEND3res ** extend3ResOut, __be32 * p,
			     struct xdr_stream *xdrStream);
int NfsExtendDecodeLsId(EXTEND3res ** extend3ResOut, __be32 * p,
			struct xdr_stream *xdrStream);

int EnfsGetFsInfoDecode(EXTEND3res ** extend3ResOut, uint32_t version,
...
int EnfsQueryDnsInfoDecode(EXTEND3res ** extend3ResOut, uint32_t version,
			   __be32 * p, struct xdr_stream *xdrStream);
int EnfsGetLsIdDecode(EXTEND3res ** extend3ResOut, uint32_t version, __be32 * p,
		      struct xdr_stream *xdrStream);

typedef int (*ENfsDecodeFunc)(EXTEND3res ** extend3ResOut, __be32 * p,
			      struct xdr_stream * xdrStream);
typedef int (*ENfsExtendOpDecode)(EXTEND3res ** extend3ResOut, uint32_t version,
				  __be32 * p, struct xdr_stream * xdrStream);
...
int NfsExtendProcInfoExtendEncode(char *pbuf, int buflen, EXTEND3args *pObj)
{
	...
	*start++ = cpu_to_be32(pObj->version);
	...
	if (opcode == NFS3_GET_LIF_VIEW_OP) {
		start =
		    xdr_reserve_space(&xdrStream,
				      8 +
				      pObj->extend_args_u.lifArgs.ipNumber *
				      MAX_IPV6_ADDR_LEN);
	...
	if (opcode == NFS_ENFS_QUERY_DNS_OP) {
		...
		start =
		    xdr_reserve_space(&xdrStream,
				      8 +
				      pObj->extend_args_u.dnsArgs.dnsNameCount *
			     ...
			     struct xdr_stream *xdrStream)
{
	...
	p = xdr_inline_decode(xdrStream,
			      shardNumber * (sizeof(uint64_t) +
					     sizeof(uint32_t)));
	if (unlikely(p == NULL)) {
		return true;
	}
	extend3ResLen =
	    sizeof(EXTEND3res) + sizeof(FS_SHARD_VIEW_SINGLE) * shardNumber;
	...
	for (i = 0; i < extend3Res->extend_res_u.fsInfo.num; i++) {
		p = xdr_decode_hyper(p,
				     &extend3Res->extend_res_u.fsInfo.
				     shardView[i].lsid);
...
int NfsExtendDecodeFsShard(EXTEND3res **extend3ResOut, __be32 *p,
			   struct xdr_stream *xdrStream)
{
	uint32_t i;
	uint32_t extend3ResLen = 0;
			     ...
			     struct xdr_stream *xdrStream)
{
	...
	extend3ResLen =
	    sizeof(EXTEND3res) + sizeof(LIF_PORT_INFO_SINGLE) * lifNum;
	extend3Res = (EXTEND3res *) kmalloc(extend3ResLen, GFP_KERNEL);
	if (extend3Res == NULL) {
		return -ENOMEM;
	}
	...
	for (i = 0; i < extend3Res->extend_res_u.lifInfo.lifNumber; i++) {
		...
		extend3Res->extend_res_u.lifInfo.lifport[i].workStatus =
		    be32_to_cpup(p++);
		p = xdr_decode_hyper(p,
				     &extend3Res->extend_res_u.lifInfo.
				     lifport[i].lsId);
		extend3Res->extend_res_u.lifInfo.lifport[i].tenantId =
		    be32_to_cpup(p++);
		p = xdr_decode_hyper(p,
				     &extend3Res->extend_res_u.lifInfo.
				     lifport[i].homeSiteWwn);
...
int NfsExtendDecodeLifInfo(EXTEND3res **extend3ResOut, __be32 *p,
			   struct xdr_stream *xdrStream)
{
	uint32_t i;
	uint32_t extend3ResLen = 0;
			     ...
			     struct xdr_stream *xdrStream)
{
	...
	extend3ResLen =
	    sizeof(uint32_t) * 3 + sizeof(DNS_QUERY_IP_INFO_SINGLE) * ipNumber;
	extend3Res = (EXTEND3res *) kmalloc(extend3ResLen, GFP_KERNEL);
	if (extend3Res == NULL) {
		return -ENOMEM;
	}
	...
	for (i = 0; i < extend3Res->extend_res_u.dnsQueryIpInfo.ipNumber; i++) {
		...
		p = xdr_decode_hyper(p,
				     &extend3Res->extend_res_u.dnsQueryIpInfo.
				     ipInfo[i].lsId);
			...
			struct xdr_stream *xdrStream)
{
	...
	p = xdr_inline_decode(xdrStream,
			      (sizeof(EXTEND_GET_LS_VERSION_SINGLE)) * lsNum);
	if (unlikely(p == NULL)) {
		return -ENOMEM;
	}
	...
	for (i = 0; i < extend3Res->extend_res_u.lsView.num; i++) {
		extend3Res->extend_res_u.lsView.lsInfo[i].lsId =
		    be32_to_cpup(p++);
		p = xdr_decode_hyper(p,
				     &extend3Res->extend_res_u.lsView.lsInfo[i].
				     lsVersion);
	}
	*extend3ResOut = extend3Res;
	return 0;
...
int EnfsGetFsInfoDecode(EXTEND3res **extend3ResOut, uint32_t version, __be32 *p,
			struct xdr_stream *xdrStream)
{
	int ret = 0;
	...
	if (version < ENFS_SERVER_VERSION_BASE) {
		...
		decode_version = ((version > ENFS_SERVER_VERSION_BASE)
		    ...
		    ? (sizeof(g_decodeFuncByVersion) /
		       sizeof(EnfsExtendProcVersionFunc) - 1) : version -
...
int EnfsExtendDecodePreCheck(uint32_t version, uint32_t opCode)
{
	if ((opCode < 0)
	    || (opCode >=
		(sizeof(g_decodeFuncByOpCode) /
		 sizeof(EnfsExtendOpDecodeProc)))) {
				  ...
				  EXTEND3res **extend3ResOut)
{
	...
	if (ret) {
		enfs_log_error
		    ("NfsExtendProcInfoExtendDecode failed, opCode:%u, version:%u, ret:%d",
		     opCode, version, ret);
		return true;
	}
...
int dorado_extend_op(struct rpc_clnt *clnt, char *buf, int *buflen)
{
	...
	status =
	    rpc_call_sync(clnt, &msg,
			  RPC_TASK_SOFT | RPC_TASK_TIMEOUT | RPC_TASK_SOFTCONN);
	LVOS_TP_END;
	LVOS_TP_END;
	if (status) {
		enfs_log_info("NFS reply status:%d resp_len:%d \n", status,
...
void nego_enfs_version(struct rpc_clnt *clnt, EXTEND3args *args)
{
	...
	ctx =
	    (struct enfs_xprt_context *)xprt_get_reserve_context(clnt->cl_xprt);
	if (ctx == NULL) {
...
int dorado_query_lsId(struct rpc_clnt *clnt, EXTEND_GET_LS_VERSION **resDataOut)
{
	...
	resData =
	    (EXTEND_GET_LS_VERSION *) kmalloc(sizeof(EXTEND_GET_LS_VERSION) +
					      sizeof
					      (EXTEND_GET_LS_VERSION_SINGLE) *
					      (extend3Res->extend_res_u.lsView.
					       num), GFP_KERNEL);
	...
	memcpy(resData->lsInfo, extend3Res->extend_res_u.lsView.lsInfo,
	       sizeof(EXTEND_GET_LS_VERSION_SINGLE) *
...
void NfsExtendDnsQuerySetArgs(EXTEND3args *args, uint32_t ip_type,
...
void NfsExtendDnsQuerySetRes(EXTEND3res *extend3Res,
		     ...
		     int *ipNumber)
{
	...
	*ipNumber = extend3Res->extend_res_u.dnsQueryIpInfo.ipNumber;
	resData =
	    (DNS_QUERY_IP_INFO_SINGLE *) kmalloc(extend3Res->extend_res_u.
						 dnsQueryIpInfo.ipNumber *
						 (sizeof
