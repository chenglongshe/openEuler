			       ...
			       enum nfsmultipathoptions type)
{
	...
	pr_info("NFS:   parsing nfs mount option '%s' type: %d[%s]\n", cursor,
		type, __FUNCTION__);
	len =
	    rpc_pton(net_ns, cursor, strlen(cursor), (struct sockaddr *)&addr,
		     sizeof(addr));
	if (!len)
		return -EINVAL;
	// 判断是否IPV4/IPV6混杂
	if (addr.ss_family != ip_list->address[ip_list->count - 1].ss_family) {
		pr_info
		    ("NFS:   parsing nfs mount option type: %d fail. both have ipv4 and ipv6 address[%s]\n",
		     type, __FUNCTION__);
		return -EINVAL;
	}
	// 判断范围IP是否为同一个IP
	if (rpc_cmp_addr
	    ((const struct sockaddr *)&ip_list->address[ip_list->count - 1],
	...
	while (true) {
		...
		case AF_INET:{
				...
				sin4->sin_addr.s_addr =
				    htonl(ntohl(sin4->sin_addr.s_addr) +
					  add_num);
				pr_info
				    ("NFS:   parsing nfs mount option ip %08x type: %d ipcont %d [%s]\n",
				     ntohl(sin4->sin_addr.s_addr), type,
				     ip_list->count, __FUNCTION__);
				break;
			}
		case AF_INET6:
			...
			pr_info
			    ("NFS:   parsing nfs mount option ip %08x%08x%08x%08x type: %d ipcont %d [%s]\n",
			     ntohl(sin6->sin6_addr.in6_u.u6_addr32[0]),
			     ntohl(sin6->sin6_addr.in6_u.u6_addr32[1]),
			     ntohl(sin6->sin6_addr.in6_u.u6_addr32[2]),
		...
		for (i = 0; i < ip_list->count; i++) {
			...
			if (rpc_cmp_addr
			    ((const struct sockaddr *)&ip_list->address[i],
			     (const struct sockaddr *)&tmp_addr)) {
				pr_info
				    ("NFS:   parsing nfs mount option type: %d index %d,same as before %d, add_num %d [%s]\n",
		...
		if (duplicate_flag == false) {
			...
			if ((type == LOCALADDR
				...
				pr_info
				    ("[MULTIPATH:%s] iplist for type %d reached %d, more than supported limit %d\n",
				     __func__, type, ip_list->count,
				     type ==
				     LOCALADDR ? MAX_SUPPORTED_LOCAL_IP_COUNT :
				     enfs_get_config_link_count_per_mount());
				ip_list->count = 0;
				return -ENOSPC;
			}
	...
	for (i = 0; i < ip_list->count; i++) {
		if (rpc_cmp_addr
		    ((const struct sockaddr *)&ip_list->address[i],
		     (const struct sockaddr *)&addr)) {
			pr_info
			    ("NFS:   parsing nfs mount option '%s' type: %d index %d same as before index %d [%s]\n",
...
bool enfs_valid_ip(char *str, struct net *net)
{
	...
	len =
	    rpc_pton(net, str, strlen(str), (struct sockaddr *)&addr,
		     sizeof(addr));
	if (!len) {
		return false;
	}
				...
				enum nfsmultipathoptions type)
{
	...
	while (ptr != NULL) {
		...
		if (prev_range == false) {
			ret =
			    enfs_parse_ip_single(ip_list_tmp, net_ns, cursor,
						 type);
			if (ret) {
				goto out;
			}
			...
			ret =
			    enfs_parse_ip_range(net_ns, cursor, ip_list_tmp,
						type);
			if (ret != 0) {
				goto out;
			}
				 ...
				 struct multipath_mount_options *options)
{
	...
	while (ptr) {
		...
		strcpy(dns->routeRemoteDnsList[dns->dnsNameCount].dnsname,
		       cursor);
		dns->dnsNameCount++;
	}
...
int nfs_multipath_parse_options_check_ip_valid(struct sockaddr_storage *address)
{
	...
	if (address->ss_family == AF_INET) {
		rc = nfs_multipath_parse_options_check_ipv4_valid((struct
								   sockaddr_in
								   *)address);
	} else if (address->ss_family == AF_INET6) {
		rc = nfs_multipath_parse_options_check_ipv6_valid((struct
								   sockaddr_in6
								   *)address);
	} else {
		rc = -EINVAL;
...
int nfs_multipath_parse_options_check_valid(struct multipath_mount_options
					    *options)
{
	int rc;
	int i;
	...
	for (i = 0; i < options->local_ip_list->count; i++) {
		rc = nfs_multipath_parse_options_check_ip_valid(&options->
								local_ip_list->
								address[i]);
		if (rc != 0)
			return rc;
						...
						*options)
{
	...
	for (i = 0; i < options->local_ip_list->count; i++) {
		for (j = 0; j < options->remote_ip_list->count; j++) {
			if (rpc_cmp_addr
			    ((const struct sockaddr *)&options->local_ip_list->
			     address[i],
			     (const struct sockaddr *)&options->remote_ip_list->
			     address[j])) {
				printk
				    ("local_addr index %d as same as remote_addr index %d \n.",
				     i, j);
				return -ENOTSUPP;
			}
		}
...
int nfs_multipath_alloc_options(void **enfs_option)
{
	...
	options->pRemoteDnsInfo =
	    kzalloc(sizeof(NFS_ROUTE_DNS_INFO_S), GFP_KERNEL);
