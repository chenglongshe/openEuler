				 ...
				 size_t count, loff_t *pos)
{
	...
	for_each_cpu(cpu, cpu_online_mask) {
		...
		*per_cpu_ptr(&xcall_cache_wait, cpu) = 0;
	}
...
{
	unsigned long hit = 0, miss = 0, wait = 0;
	unsigned int cpu;
	u64 percent;
	...
	for_each_cpu(cpu, cpu_online_mask) {
		...
		wait = *per_cpu_ptr(&xcall_cache_wait, cpu);

		if (hit == 0 && miss == 0)
			continue;

		percent = (hit * 10000ULL) / (hit + miss);
		seq_printf(m, "cpu%d epoll cache_{hit,miss,wait}: %ld,%ld,%ld, hit ratio: %3llu.%02llu%%\n",
			   cpu, hit, miss, wait, percent / 100, percent % 100);
	}
	return 0;
}
