...
//	struct fuse_ipc_info *ipc_info2;
	//cpumask_t old_mask;
	//cpumask_t new_mask;
	...
	//ipc_info2 = this_cpu_ptr(fc->percpu_ipc_info);
	//while (ipc_info != ipc_info2) {
	...
	while (count) {
		...
		fuse_release_user_pages(&ia->ap, should_dirty);
...
static ssize_t fuse_direct_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	if (!is_sync_kiocb(iocb) && iocb->ki_flags & IOCB_DIRECT) {
		...
		if (fc->use_fastpath)
			res = __fuse_direct_read_fast(iocb, to, &iocb->ki_pos);
		else
#endif
		{
