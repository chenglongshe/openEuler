		...
		struct fuse_ipc_info *ipc_info)
{
	...
	ret = kernel_ipc_do_call(ipc_info->bind_info, tsk);

	FUSE_DEBUG("[cpu/%d][%s/%d] endddddd\n", smp_processor_id(), current->comm,
			   current->pid);

	if (ret) {
		pr_warn("[cpu/%d][%s/%d] fuse_simple_request send failed: "
				"unique: %lld, opcode: %d, return value: %ld\n",
				smp_processor_id(), current->comm, current->pid,
...
static void fuse_wakeup_server(struct fuse_conn *fc)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (ipc_info && ipc_info->bind_info) {
			kernel_ipc_wakeup_server_task(ipc_info->bind_info);
		}
	}
}
#endif
...
#ifdef CONFIG_FUSE_FASTPATH
	if (fm && fm->fc && fm->fc->use_fastpath && args->opcode != 26) {
...
static long fuse_ipc_unbind(struct fuse_conn *fc, struct task_struct *tsk)
{
	...
	//mutex_lock(&ipc_info->mutex_lock);
	kernel_ipc_unbind(ipc_info->bind_info, tsk);
		...
		struct task_struct *tsk, struct fuse_ipc_io *ipc_in_data)
{
	...
	FUSE_DEBUG("[cpu/%d][%s/%d] wait call slow start\n", smp_processor_id(),
			   tsk->comm, tsk->pid);
	ret = kernel_ipc_wait_call(ipc_info->bind_info, tsk);
	if (ret < 0) {
		pr_err("error[%d/%s]: kernel_ipc_wait_call error: %ld\n",
		...
		struct fuse_ipc_io *ipc_out_data)
{
	...
	if (oh->len != (nbytes + sizeof(struct fuse_out_header))) {
		pr_warn(
		"[cpu/%d][%s/%d]failed %s:oh.unique:%lld, oh.len: %d nbytes: %ld\n",
		...
		struct task_struct *tsk, struct fuse_ipc_io *ipc_out_data)
{
	...
	ret = kernel_ipc_ret_call(ipc_info->bind_info, tsk);
	if (ret) {
		pr_err("error: kernel_ipc_ret_call error: %ld\n", ret);
		return ret;
	}
			   ...
			   unsigned long arg)
{
	...
	if (cmd == FUSE_DEV_IOC_CLONE) {
		...
		err = fuse_ipc_ioctl(file, cmd, arg);
#endif
	return err;
}
