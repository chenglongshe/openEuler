...
static void *_read_data_inner(struct super_block *sb, const char *path)
{
	...
	off = 0;
	ret = kernel_read(filp, data, size, &off);
	if (ret < 0) {
		erofs_err(sb, "read failed for size:%lld, ret:%lld", size, ret);
		vfree(data);
		data = NULL;
		goto close_data;
	}
...
int erofs_register_trio(struct super_block *sb)
{
	...
	sbi->buffer = _read_data_inner(sb, sbi->trio_data);
	if (!sbi->buffer)
		return ret;

	meta_buffer = _read_data_inner(sb, sbi->trio_meta);
	if (!meta_buffer)
		goto free_obj;

	while ((item = strsep(&meta_buffer, PATH_PATH_DELIMIT)) != NULL) {
