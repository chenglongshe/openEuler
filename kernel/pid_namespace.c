	...
	struct pid_namespace *parent_pid_ns)
{
	...
	// additional 1 refcount for the list
	list_add_tail(&get_pid_ns(ns)->loadavg->list, &init_pidns_loadavg.list);
...
static void pidns_list_reset(void)
{
	...
	list_for_each_safe(pos, tmp, &init_pidns_loadavg.list) {
		...
		struct pid_namespace *pidns = entry->pidns;

		/*
		 * Where the actual releasing of pidns is triggered:
		 *
		 *   refcount == 1 means the pidns is only referred by this list,
		 *   which should be released.
		 */
		if (refcount_read(&pidns->ns.count) == 1) {
			list_del(pos);
			put_pid_ns(pidns);
			continue;
		}

		pidns->loadavg->load_tasks = 0; // reset
	}
	spin_unlock(&pidns_list_lock);
}
