...
static void xcu_cfs_root_cg_init(void)
{
	...
	for_each_active_xcu(xcu, id) {
		...
		root_xcg->perxcu_priv[id].rq = &xcu->xrq.cfs;
 ...
 */
static void xcu_cfs_cg_init(struct xsched_group *xcg,
			    struct xsched_group *parent_xg)
{
	...
	for_each_active_xcu(xcu, id) {
		...
		xcg->perxcu_priv[id].rq = sub_cfs_rq;
		xcg->perxcu_priv[id].rq->ctx_timeline = RB_ROOT_CACHED;
...
static void xcu_cfs_cg_deinit(struct xsched_group *xcg)
{
	...
	for_each_active_xcu(xcu, id) {
		mutex_lock(&xcu->xcu_lock);
		fair_xsched_class.dequeue_ctx(&xcg->perxcu_priv[id].xse);
		mutex_unlock(&xcu->xcu_lock);
		kfree(xcg->perxcu_priv[id].rq);
	}
 ...
 * @parent_xg: parent's group
 */
static void xcu_cg_init(struct xsched_group *xcg,
			struct xsched_group *parent_xg)
{
	...
	case XSCHED_TYPE_CFS:
		...
		break;
	default:
		pr_debug("xcu_cgroup: init RT group css=0x%lx\n", (uintptr_t)&xcg->css);
		break;
	}
}
...
static void xcu_css_free(struct cgroup_subsys_state *css)
{
	...
	if (xcg->parent != NULL) {
		...
		case XSCHED_TYPE_CFS:
			...
			break;
		default:
			pr_debug("xcu_cgroup: deinit RT group css=0x%lx\n", (uintptr_t)&xcg->css);
			break;
		}
	}
...
static int xcu_can_attach(struct cgroup_taskset *tset)
{
	...
	cgroup_taskset_for_each(task, dst_css, tset) {
		struct cgroup_subsys_state *old_css =
			task_css(task, xcu_cgrp_id);
		struct xsched_group *dst_xcg = xcu_cg_from_css(dst_css);
		   ...
		   struct xsched_group *new_xcg)
{
	...
	list_for_each_entry_safe(xse, tmp, &old_xcg->members, group_node) {
		if (xse->owner_pid == task_pid_nr(task)) {
			...
			WARN_ON_ONCE(old_xcg != xse->parent_grp);
...
int xcu_cg_set_sched(struct xsched_group *xg, int type)
{
	...
	if (xg->parent != root_xcg || !list_empty(&xg->members))
		return -EINVAL;
	...
	if (type == XSCHED_TYPE_CFS) {
		xg->sched_type = XSCHED_TYPE_CFS;
		xcu_cfs_cg_init(xg, xg->parent);
	}
...
static int xcu_stat(struct seq_file *sf, void *v)
{
	...
	if (xcucg->sched_type == XSCHED_TYPE_RT) {
		seq_puts(sf, "RT group stat is not supported\n");
		return 0;
	}
