...
void xcu_grp_shares_update(struct xsched_group *xg)
{
	...
	list_for_each_entry((xgi), &(parent)->children_groups, group_node) {
		if ((xgi)->sched_type == XSCHED_TYPE_CFS) {
			...
			xgi->shares_cfg_red = div64_u64(xgi->shares_cfg, sh_gcd);
			div64_u64_rem(sh_prod_red, xgi->shares_cfg_red, &rem);
			if (rem)
	...
	list_for_each_entry((xgi), &(parent)->children_groups, group_node) {
		if ((xgi)->sched_type == XSCHED_TYPE_CFS) {
			xgi->weight = div64_u64(sh_prod_red, xgi->shares_cfg_red);
	...
	list_for_each_entry((xgi), &(parent)->children_groups, group_node) {
		if ((xgi)->sched_type == XSCHED_TYPE_CFS) {
			xgi->weight = div64_u64(xgi->weight, w_gcd);
			for_each_active_xcu(xcu, id) {
				mutex_lock(&xcu->xcu_lock);
