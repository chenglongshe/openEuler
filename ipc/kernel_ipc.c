...
#ifndef pr_fmt
# define pr_fmt(fmt) "kernel_ipc: " fmt
#endif
		...
		struct task_struct *tsk)
{
	...
	if (!bind_info) {
		pr_err("error: no found bind_info\n");
		return -ENOENT;
	}
...
void *kernel_ipc_bind(struct task_struct *server_task)
{
	...
	if (!bind_info) {
		pr_err("error: alloc kernel_ipc_bind_info failed\n");
		...
		struct task_struct *tsk)
{
	...
	if (ret) {
		pr_err("kernel ipc call check and init failed, errno: %d\n", ret);
		return ret;
	}
	...
	if (bind_info->is_calling) {
		...
		pr_err("[cpu/%d][%s/%d] show_stack end in %s\n",
